...
i32 main() {
    ...
    vector<vector<vector<ll>>> dp(1+n, vector<vector<ll>>(1+n, vector<ll>(m+1, INF)));

    vvl mindp(1+n, vl(1+n, INF));
        ...
        for (int l = 1; l +len -1<= n; ++l) {
            ...
            for (int i = l; i <= r; ++i) {
                for (int curColor = 1; curColor <= m; ++curColor) {
                    ll c1 = dp[l][i][curColor];
                    if (mindp[l][i] + p[curColor] < c1) {
                        c1 = mindp[l][i] + p[curColor];
                    }
                    ll c2 = 0;
                    if (i < r) {
                        c2 = dp[i+1][r][curColor];
                        if (mindp[i+1][r] + p[curColor] < c2) {
                            c2 = mindp[i+1][r] + p[curColor];
                        }
                    }
                    dp[l][r][curColor] = min(dp[l][r][curColor], c1 + c2);
                    mindp[l][r] = min(mindp[l][r],  dp[l][r][curColor]);
                }
            }
        }
    }
