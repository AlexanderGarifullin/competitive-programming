...
i32 main() {
    ...
    vvi left(n,vi(m)), right(n,vi(m)), down(n, vi(m));
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            if (v[i][j]) {
                left[i][j]++;
                if (j - 1 >= 0 && v[i][j - 1]) left[i][j] += left[i][j - 1];
            }
        }
        for (int j = m-1; j >=0 ; --j) {
            if (v[i][j]) {
                right[i][j]++;
                if (j + 1 < m && v[i][j + 1]) right[i][j] += right[i][j + 1];
            }
        }
    }
    for (int i = n-1; i >=0; --i) {
        for (int j = 0; j < m; ++j) {
            if (v[i][j]) {
                down[i][j]++;
                if (i + 1 < n && v[i + 1][j]) down[i][j] += down[i + 1][j];
            }
        }
    }

    function<void(vvi)> print = [](vvi dp){
   ...
   // print(down);
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            r = max(r, down[i][j] + min(left[i][j], right[i][j]) * 2 - 2);
        }
    }
    cout << r;
}
