...
void multiSolve() {
    int t_; cin >> t_; while(t_--) {
        ...
        vl a(n); fin(a);
        sort(all(a));
        vector<tuple<ll,int,int>> dif;
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < i; ++j) {
                dif.pb(make_tuple(a[i] - a[j], i, j));
            }
        }
        int ans = 0;

        function<bool(ll)> ok = [](ll v) {
            ll x = mysqrt(v);
            return x * x == v;
        };
        ...
        function<void(ll,int,int,ll)> add = [&](ll dif, int j, int i, ll d) {
            ll p = (dif / d - d) / 2;
            ll q = (dif / d + d) / 2;
            if (dif == (q - p) * (q + p)) {
                ll x1 = q * q - a[j];
                ll x2 = p * p - a[i];
                if (x1 >= 0 && x1 <= 1e18 && x1 == x2) {
                    int c=0;
                    for (int k = 0; k < n; ++k) {
                        ll v = a[k] + x1;
                        c += ok(v);
