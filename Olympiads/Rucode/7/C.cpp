...
i32 main() {
    ...
    for (int i = 0; i < l; ++i) {
        ...
        str mm; cin >> mm;
        str mi;
        for (int j = 0; j < isz(mm); ++j) {
            if (isdigit(mm[j])) mi += mm[j];
        }
        int minv = stoi(mi);
        cin >> mm;
        str ma;
        for (int j = 0; j < isz(mm); ++j) {
            if (isdigit(mm[j])) ma += mm[j];
        }
        int maxv = stoi(ma);
        cin >> mm;
        str name;
        for (int j = 1; j < isz(mm) - 1; ++j) {
            if (isalpha(mm[j])) name += mm[j];
        }

        // cout << minv << ' ' << maxv << ' ' << name << en;
        minNumber[i] = minv;
        maxNumber[i] = maxv;
        names[i] = name;
    }
    ...
    vb wasMin(l), wasMax(l);
    for (int i = 1; i <= t; ++i) {
        for (int j = 0; j < l; ++j) {
            int x; cin >> x;
            if (x < minNumber[j] || x > maxNumber[j]) {
                cout << "Error" <<en;
                cout << i << ' ' << names[j];
                return 0;
            }
            if (x == minNumber[j]) wasMin[j] = true;
            if (x == maxNumber[j]) wasMax[j] = true;
        }
    }

    vi wa1, wa2;
    for (int i = 0; i < l; ++i) {
        if (!wasMin[i]) wa1.pb(i);
        if (!wasMax[i]) wa2.pb(i);
    }
