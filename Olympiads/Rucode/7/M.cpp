...
i32 main() {
    ...
    if (l == r) {
       ...
       while(true) {
           ...
           if (isz(curodd) < 2 && isz(curven) < 2) {
               for (int x : curodd) odd.push(x);
               for (int x : curven) even.push(x);
               break;
           }
           int evenMed = -1, oddMed = -1;
           if (isz(curodd) == 2) {
               oddMed = (curodd[0] + curodd[1]) / 2;
           }
           if (isz(curven) == 2) {
               evenMed = (curven[0] + curven[1]) / 2;
           }
           if (evenMed != -1 && oddMed != -1) {
               if (evenMed < oddMed) {
                   if (evenMed % 2) odd.push(evenMed);
                   else even.push(evenMed);
                   for (int x : curodd) odd.push(x);

               } else if (evenMed > oddMed) {
                   if (oddMed % 2) odd.push(oddMed);
                   else even.push(oddMed);
                   for (int x : curven) even.push(x);
               } else {
                   if (evenMed % 2) odd.push(evenMed);
                   else even.push(evenMed);
        ...
        int x = -1;
        int y = - 1;
        if (!even.empty()) x = even.top();
        if (!odd.empty()) y = odd.top();
        if (x != -1 && y != -1) cmo;
        else if (x == -1) cout << y << en;
        else if (y == -1) cout << x << en;
    }
}
