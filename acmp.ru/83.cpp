...
//    freopen("output.txt", "wt", stdout);
    ...
    // 0 ... 2^m -1
    vvl dp(n, vl(1<<m));
    // первую строчку заполняем всеми вариантами от 0 до 2^m -1
    dp[0] = vl(1<<m, 1);
    for (int row = 1; row < n; ++row) {
        for (int curRow = 0; curRow < (1<<m); ++curRow) {
            for (int predRow = 0; predRow < (1<<m); ++predRow) {
                bool ok = true;
                for (int col = 0; col + 1 < m; ++col) {
                    int leftUp = getbit(predRow, col);
                    if (leftUp == getbit(predRow,col+1) &&
                            leftUp == getbit(curRow, col) &&
                            leftUp == getbit(curRow, col+1)) {
                        ok = false;
                        break;
                    }
                }
                if (ok) dp[row][curRow] += dp[row-1][predRow];
            }
        }
    }
