...
//    freopen("output.txt", "wt", stdout);
    ...
    // количество способов где n шаров и максимум высота k
    map<pii,int> mem;

    function<int(int, int)> solve = [&](int n, int k){
        if (mem.contains(pii(n, k))) return mem[pii(n,k)];
        if (n == 0) {
            return mem[pii(n,k)] = 1;
        }
        int res =0;
        for (int cur = 1; cur <= min(n,k); ++cur) {
            res += solve(n-cur, cur-1);
        }
