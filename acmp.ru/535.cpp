...
//    freopen("output.txt", "wt", stdout);
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            for (int k = 0; k < 3; ++k) {
                ...
                str s  = sum(sum(v[i],v[j]), v[k]);
                st.ins(s);
            }
        }
    }
    for (str ss : st) vec.pb(ss);
    sort(all(vec), [](str s1, str s2){
        if (isz(s1) < isz(s2)) return true;
        if (isz(s2) > isz(s2)) return false;
    ...
    ifyn(isz(st) > 1);
    for (const str& ss : vec) cout << ss << en;
}
