...
i32 main() {
    ...
    int n,k; cin >> n >> k;
    vi a(n); fin(a);
    vpii prefmax(n),sufmax(n);
    for (int i = 0; i < n; ++i) {
        prefmax[i] = {a[i], i+1};
        if (i > 0) if (a[i] <= prefmax[i-1].first) prefmax[i] = prefmax[i-1];
    }
    for (int i = n-1; i >=0; --i) {
        sufmax[i] = {a[i], i+1};
        if (i < n-1) if (a[i] < sufmax[i+1].first) sufmax[i] = sufmax[i+1];
    }
    int ans=-1;
    int l=-1,r=-1;
    for (int i = 0; i < n - k -1; ++i) {
        if (prefmax[i].first + sufmax[i+k+1].first > ans) {
            ans = prefmax[i].first + sufmax[i+k+1].first;
            l = prefmax[i].second;
            r = sufmax[i+k+1].second;
        }
    }
    cout << l << ' ' << r;
}
