...
i32 main() {
   ...
   int n; cin >> n;
   // move num disk from a -> c with help from b;
   function<void(int,int,int,int,int)> move=[&](int num, int a, int b, int c, int cnt){
       if (cnt == 1) {
           cout << num << ' ' << a << ' ' << c<< en;
       } else {
           move(num, a, c, b, cnt-1);
           cout << cnt - num +1 << ' ' << a << ' ' << c<< en;
           move(num, b, a, c, cnt-1);
       }
   };
    move(1,1,2,3,n);
}
