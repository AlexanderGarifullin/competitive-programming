...
i32 main() {
    ...
    int t; cin >> t; while(t--) {
        int n, m; cin >> n >> m;
        vvi v(n, vi(m)), up(n, vi(m)), down(n, vi(m)), left(n, vi(m)), right(n, vi(m));
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < m; ++j) {
                char c; cin >> c;
                if (c == '#') v[i][j] = 0;
        ...
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < m; ++j) {
                if (v[i][j]) {
                    down[i][j]++;
                    if (i > 0 && v[i - 1][j]) down[i][j] += down[i - 1][j];
                }
            }
        }
        // up
        for (int i = n-1; i >=0 ; --i) {
            for (int j = 0; j < m; ++j) {
                if (v[i][j]) {
                    up[i][j]++;
                    if (i < n - 1 && v[i + 1][j]) up[i][j] += up[i + 1][j];
                }
            }
        }
        // left;
        ...
        for (int i = 0; i < n; ++i) {
            for (int j = m-1; j >=0 ; --j) {
                if (v[i][j]) {
                    right[i][j]++;
                    if (j < m - 1 && v[i][j + 1]) right[i][j] += right[i][j + 1];
                }
            }
        }
        function<void(vvi)> print = [](vvi v) {
            for (int i = 0; i < isz(v); ++i) {
                for (int j = 0; j < isz(v[i]); ++j) {
                    cout << v[i][j] << ' ';
                }
...
//        print(right);
        ...
        if (n > 1) {
            int predMax = 0;
            for (int i = 0; i < m; ++i) {
                predMax = max(predMax, down[0][i]);
                predMax = max(predMax, left[0][i]);
                predMax = max(predMax, right[0][i]);
            }
            for (int i = 1; i < n; ++i) {
                int curMax = 0;
                int curPredMax = 0;
                for (int j = 0; j < m; ++j) {
                    curMax = max(curMax, up[i][j]);
                    curPredMax = max(curPredMax, down[i][j]);
                    curPredMax = max(curPredMax, left[i][j]);
                    curPredMax = max(curPredMax, right[i][j]);
                    curMax = max(curMax, left[i][j]);
                    curMax = max(curMax, right[i][j]);
                }
                res =max(res, predMax + curMax);
                predMax = max(predMax, curPredMax);
            }
        }
        ...
        if (m > 1) {
            int predMax = 0;
            for (int i = 0; i < n; ++i) {
                predMax = max(predMax, left[i][0]);
                predMax = max(predMax, up[i][0]);
                predMax = max(predMax, down[i][0]);
            }
            for (int i = 1; i < m; ++i) {
                int curPredMax = 0;
                int curMax = 0;
                for (int j = 0; j < n; ++j) {
                    curMax = max(curMax, right[j][i]);
                    curPredMax = max(curPredMax, left[j][i]);
                    curPredMax = max(curPredMax,down[j][i]);
                    curPredMax = max(curPredMax, up[j][i]);
                    curMax = max(curMax,down[j][i]);
                    curMax = max(curMax, up[j][i]);
                }
                res =max(res, predMax + curMax);
                predMax = max(predMax, curPredMax);
            }
