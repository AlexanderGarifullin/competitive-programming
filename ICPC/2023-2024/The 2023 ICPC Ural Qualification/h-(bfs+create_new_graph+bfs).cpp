                        ...
                        for (int dy = -1; dy <=1 ; ++dy) {
                            if (dx * dx + dy * dy == 1) {
                                if (x + dx >= 0 && x + dx < n && y + dy >= 0 && y + dy < m) {
                                    if (dp[x+dx][y+dy] == 0 && !usebfs[x+dx][y+dy])  {
                                        q.push({x+dx,y+dy});
                                        usebfs[x+dx][y+dy] = 1;
                                    }
                                }
...
//        nline;
//    }
    vector<set<int>> g(curComponent + 1);

    vpii steps = {{2,0}, {-2,0}, {0,-2}, {0,2}};
        ...
        for (int y = 0; y < m; ++y) {
            if (dp[x][y] != -1) {
                for (auto [dx, dy]: steps) {
                    if (x + dx >= 0 && x + dx < n && y + dy >= 0 && y + dy < m) {
                        if (dp[x + dx][y + dy] != -1 && dp[x + dx][y + dy] != dp[x][y]) {
                            g[dp[x][y]].ins(dp[x + dx][y + dy]);
                            g[dp[x + dx][y + dy]].ins(dp[x][y]);
                        }
                    }
                }
            }
    ...
    queue<pii> q;
    q.push({dp[x1][y1], 0});
    vb use(1+curComponent);
    use[dp[x1][y1]] = true;
    while (!q.empty()) {
        auto [curV, dist] =q.front();
        q.pop();
        if (curV == dp[x2][y2]) {
            len = dist;
            break;
