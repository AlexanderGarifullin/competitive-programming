...
bool calc(int l, int r) {
    if (l > r) return true;
    bool canDelFromSide = false;
    int mx = *max_element(a.begin()+l, a.begin()+r+1);
    int len = r - l + 1;
    if (l-1>= 0 && a[l-1] > mx) canDelFromSide = true;
    if (r+1<n && a[r+1] > mx) canDelFromSide = true;
    if (len < k && !canDelFromSide) return false;
    int needY = len % k;
    ans += needY * 1ll * y;
    len -= needY;
    if (1ll * y * k >= x) {
        ans += 1ll * len / k * x;
    } else if (canDelFromSide) {
        ans += 1ll * len * y;
    } else {
        ans += 1ll * (len - k) * y + x;
    }
