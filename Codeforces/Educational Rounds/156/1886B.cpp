...
//        // only a
//        ld res1 = max(sqrtl(ax * ax + ay * ay), sqrtl((ax-px) * (ax-px) + (ay-py) * (ay-py)));
//        // only b
//        ld res2 = max(sqrtl(bx * bx + by * by), sqrtl((bx-px) * (bx-px) + (by-py) * (by-py)));
//        // a = O, b = P
//        ld res3 =0;
//        ld between =
//        // a = p, b = 0
//        ld res4=0;
//
//        cout << min({res1,res2,res3,res4})<<en;

        ld lenAP = sqrtl((ax - px) * (ax - px) + (ay - py) * (ay - py));
        ld lenBP = sqrtl((bx - px) * (bx - px) + (by - py) * (by - py));
        ld lenAO = sqrtl((ax) * (ax) + (ay ) * (ay));
        ld lenBO = sqrtl((bx) * (bx) + (by ) * (by ));
        ld lenAB = sqrtl((bx - ax) * (bx - ax) + (by - ay) * (by - ay));



        function<bool(ld)> ok = [&](ld w){
            // A - P
            if (lenAP <= w && lenAO <= w) return true;
            if (lenBP <= w && lenBO <= w) return true;
            if (lenAP <= w && lenBO <= w && 2 * w >= lenAB) return  true;
            if (lenAO <= w && lenBP <= w && 2 * w >= lenAB) return  true;
            return false;
        };
