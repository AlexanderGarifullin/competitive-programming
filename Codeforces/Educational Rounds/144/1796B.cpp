...
void voidsolve(){
    ...
    if (min_s.back() == max_s.back()){
        ans += '*';
        ans += min_s.back();
        fl =1;
    }
    else if (min_s.front() == max_s.front()){
        ans += min_s.front();
        ans += '*';
        fl =1;
    }
    else {
        for (int i = 0; i < isz(max_s) - 1; ++i) {
            if (fl) break;
            string cur1 = max_s.substr(i,2);
            for (int j = 0; j < isz(min_s) - 1; ++j) {
                string cur2 = min_s.substr(j,2);
                if (cur1 == cur2){
                    ans = '*' + cur1 + '*';
                    fl = 1;
                }
