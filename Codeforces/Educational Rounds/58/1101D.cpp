...
void calc(int v, int p = -1) {
    ...
    sort(all(child));
    for (int i = 0; i < isz(child); ++i) {
        int j = i - 1;
        int mx1 = 0;
        int mx2 = 0;
        while(j + 1 < isz(child) && child[j+1].first == child[i].first) {
            j++;
            if (child[j].second >= mx1) {
                mx2 = mx1;
                mx1 = child[j].second;
            } else if (child[j].second > mx2) {
                mx2 = child[j].second;
            }
        }
        if (a[v] % child[i].first == 0) {
            ans = max(ans, mx1 + mx2 + 1);             
            dp[v].emplace_back(child[i].first, mx1 + 1);
            while (a[v] % child[i].first == 0) {
                a[v] /= child[i].first;
            }
        } else {
            ans = max(ans, mx1);
