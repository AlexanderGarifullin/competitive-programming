...
//    freopen("output.txt", "wt", stdout);
    ...
    int t_; cin >> t_; while(t_--) {
        ...
        int s,w; cin >> s >> w;
        int r = 0;
        function<int(int,int,int,int,int)> get = [](int weight1, int weight2, int cnt1, int cnt2, int volume) {
            int val=0;
            int c = min(cnt1, volume/ weight1);
            val += c;
            volume -= c * weight1;
            val += min(cnt2, volume/weight2);
            return val;
        };
        // try to feed some s to first;
        for (int i = 0; i <= min(cnts, p/s); ++i) {
            int cur = i;
            int haveFirst = p - i * s;
            int canAdd = min(cntw, haveFirst/w);
            cur += canAdd;
            int cnts2 = cnts - i;
            int cntw2 = cntw - canAdd;
            ll x = get(s,w,cnts2,cntw2, f);
            ll y = get(w,s,cntw2,cnts2,f);
            cur += max(x,y);
            r= max(r,cur);
        }
        // try to feed some w to first;
        for (int i = 0; i <= min(cntw, p/w); ++i) {
            int cur = i;
            int haveFirst = p - i * w;
            int canAdd = min(cnts, haveFirst/s);
            cur += canAdd;
            int cnts2 = cnts - canAdd;
            int cntw2 = cntw - i;
            cur += max(get(s,w,cnts2,cntw2, f), get(w,s,cntw2,cnts2,f));
            r= max(r,cur);
        }
        // try to feed some s to second;
        for (int i = 0; i <= min(cnts, f/s); ++i) {
            int cur = i;
            int haveFirst = f - i * s;
            int canAdd = min(cntw, haveFirst/w);
            cur += canAdd;
            int cnts2 = cnts - i;
            int cntw2 = cntw - canAdd;
            cur += max(get(s,w,cnts2,cntw2, p), get(w,s,cntw2,cnts2,p));
            r= max(r,cur);
        }
        // try to feed some w to second;
        for (int i = 0; i <= min(cntw, f/w); ++i) {
            int cur = i;
            int haveFirst = f - i * w;
            int canAdd = min(cnts, haveFirst/s);
            cur += canAdd;
