...
i32 main() {
    ...
    while(t--){
        str s; cin >> s;
        int x; cin >> x;
        str w = string(isz(s), '1');

      // make w string
        for (int i = 0; i < isz(s); ++i) {
            if (s[i] == '0') {
                if (i - x >= 0) w[i - x] = '0';
                if (i + x < isz(s)) w[i+x]='0';
            }
        }
      // make s string by a string w  
        str new_s = string(isz(s),'0');
        for (int i = 0; i < isz(s); ++i) {
            if (i + x < isz(s) && w[i+x] == '1') new_s[i] = '1';
            else if (i - x >= 0 && w[i-x] == '1') new_s[i] = '1';
        }
      // compare old s and new s 
        if (new_s == s) cout << w << en;
        else cmo;
    }
