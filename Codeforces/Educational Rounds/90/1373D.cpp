    ...
    while(t--){
        ...
        for (int i = 0; i < n; ++i) {
            if (i == 0) {
                ...
                dp[0][i] = dp[0][i - 1] + (i % 2 == 0 ? v[i] : 0);
                if (i == 1) {
                    dp[1][i] = v[1];
                }
                else {
                    dp[1][i] = max(dp[1][i-2],dp[0][i-2])+ (i % 2 == 1 ? v[i] : v[i-1]);
                }
                dp[2][i] = max({dp[0][i-1],dp[1][i-1],dp[2][i-1]}) + (i % 2 == 0 ? v[i] : 0);
            }
        }

        cout << max({dp[0].back(), dp[1].back(), dp[2].back()}) << en;
    }
