    ...
    int t_; cin >> t_; while(t_--){
        ...
        for (int i = 0; i < n; ++i) {
            if (s[i] == '+') {
                ...
                cnt--;
                maxSortedPref = min(maxSortedPref, cnt);
                if (cnt < minNotSortedPref) minNotSortedPref = 0;
            } else if (s[i] == '1') {
                maxSortedPref = max(cnt, maxSortedPref);
            } else {
                if (cnt <= 1) {
                    ok = false;
                    break;
                }
                if (minNotSortedPref == 0 || minNotSortedPref > cnt) minNotSortedPref = cnt;
            };
