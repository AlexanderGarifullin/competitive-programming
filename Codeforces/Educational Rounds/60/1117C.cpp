...
void CBO() {
    ...
    for (int i = 0; i < n; ++i) {
        if (s[i] == 'U') prefy[i]++;
        else if (s[i] == 'D') prefy[i]--;
        else if (s[i] == 'L') prefx[i]--;
        else prefx[i]++;
    ...
    while(left+1<right) {
        ...
        ll ost = mid % n;
        ll dist = 0;
        ll x = x1 + full * prefx.back() + (ost>0?prefx[ost-1] : 0);
        ll y = y1 + full * prefy.back() + (ost>0?prefy[ost-1] : 0);;
        dist = abs(x - x2)  +  abs(y - y2);

        if (dist <= mid) right = mid;
        else left = mid;
    }

    if (right > 5e17) right=-1;
    cout << right<<en;
}
