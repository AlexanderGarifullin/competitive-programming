...
void CBO(){
    ll l,r; cin >> l >> r;
    cout << upper_bound(good.begin(), good.end(), r) -
            lower_bound(good.begin(), good.end(), l)<< en;
}

void takeAll(int pos, int cnt, ll value) {
    if (pos == 18) {
        good.pb(value);
        return;
    }
    takeAll(pos+1, cnt, value*10);
    if (cnt<3) {
        for (int i = 1; i <= 9; ++i) {
            takeAll(pos+1, cnt+1, value*10+i);
        }
    }
}
