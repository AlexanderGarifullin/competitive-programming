...
//    freopen("output.txt", "wt", stdout);
    ...
    int t_; cin >> t_; while(t_--) {
        ...
        vi a(n); fin(a);
        ll r = 0;
        // check len 3 and len 4
        function<bool(int,int,int)> check1 = [&](int i,int j, int k) {
            int x=a[i],y=a[j],z=a[k];
            return !(x <= y && y <= z || x >= y && y >= z);
        };
        function<bool(int,int,int,int)> check2 = [&](int i,int j, int k, int r) {
            int x=a[i],y=a[j],z=a[k],c=a[r];
            bool ok1 = x <= y && y <= max(z,c);
            bool ok2 = x >= y && y >= min(z,c);
            bool ok3 = min(x,y) <= z && z <= c;
            bool ok4 = max(x,y) >= z && z >= c;
