    ...
    function<bool(ll)> ok = [&](ll mid) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (r < mid) {
                ...
                medium.emplace_back(l, r);
            }
        }
        sort(all(medium));
        int needMore = max(0, (n+1) / 2 - cntMore);
        if (needMore > isz(medium)) return false;
        need += needMore * mid;
        for (int i = 0; i < isz(medium) - needMore; ++i) {
            need += medium[i].first;
        }
        return need <= s;
    };
