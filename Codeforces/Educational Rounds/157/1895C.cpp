...
ll stupid(int n, vs v) {
    ll c = 0;
    set<pii> st;
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j <= i; ++j) {
            if ((isz(v[j]) + isz(v[i])) % 2 == 0) {
                str x = v[j] + v[i];
                int sum = 0;
                for (int k = 0; k < isz(x); ++k) {
                    sum += x[k] - '0';
                }
                int half = 0;
                for (int k = 0; k < isz(x) / 2; ++k) {
                    half += x[k] - '0';
                }
                if (half * 2 == sum) {
                    st.ins({j,i});
...
ll smart(int n, vs v){
    ll c = n;
    sort(all(v));
    map<int,map<int,int>> len_sum_cnt;
    for (int i = 0; i < n; ++i) {
        int ss = 0;
        for (int j = 0; j < isz(v[i]); ++j) {
            ss += v[i][j] - '0';
        }
        len_sum_cnt[isz(v[i])][ss]++;
    }
    for (int i = 0; i < n; ++i) {
        int sum = 0;
        for (int j = 0; j < isz(v[i]); ++j) {
        ...
        for (int j = 1; j <= last; ++j) {
            int size_left_path = isz(v[i]) - j;
            int needSize = size_left_path - j;
            int needSum = sum - add;
            if (len_sum_cnt[needSize][needSum] > 0) c += len_sum_cnt[needSize][needSum];
            add += v[i][j] - '0';
            sum -= v[i][j] - '0';
        }
        len_sum_cnt[isz(v[i])][ss]++;
        ...
        sum -= add;
        last = isz(v[i]) / 2;
        for (int j = isz(v[i]) - 2; j >= last; --j) {
            int size_right_path = j + 1;
            int needSize = -(isz(v[i]) - size_right_path);
            needSize += size_right_path;
            int needSum = sum - add;
            if (len_sum_cnt[needSize][needSum] > 0) c += len_sum_cnt[needSize][needSum];
            add += v[i][j] - '0';
            sum -= v[i][j] - '0';
    ...
    for (auto [len, sum_cnt] : len_sum_cnt) {
        for (auto [sum, cnt] : sum_cnt) c += 1ll * cnt * (cnt-1) ;
    }
    return c;
}
