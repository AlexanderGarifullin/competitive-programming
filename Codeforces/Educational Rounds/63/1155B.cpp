...
i32 main() {
   ...
   cin.tie(0); ios_base::sync_with_stdio(false);
    ...
    int cntPetya= (n-11)/2;
    int cntVasyan=n-11-cntPetya;
    str s;
    cin >> s;

    int start8 = s.find('8');
    if (start8 == string::npos) {
        cno;
        return 0;
    }
    int cntNeed = start8;
    int last8 = 0;
    for (int i = start8; i < n; ++i) {
        if (s[i]=='8') {last8 = i;cntPetya--;}
        else {
            cntNeed++;
        }
        if (cntPetya == 0) break;
    ...
    if (cntPetya > 0 ||cntPetya ==0 && cntNeed > cntVasyan ) cno;
    else {
        if (last8 + 1 =='8') cyes;
        else {
            cntVasyan-=cntNeed;
            int cntcur = 0;
            for (int i = last8 + 1; i < n; ++i) {
                if (s[i]  =='8') break;
                cntcur++;
            }
            ifyn(cntcur <=cntVasyan);
        }
    }
