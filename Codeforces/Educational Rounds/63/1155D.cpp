...
void CBO() {
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        left[i] = max(0ll, pref[i] - mins);
    }
    ll maxs = pref[n];
    for (int i = n; i >= 1 ; --i) {
        maxs = max(maxs, pref[i]);
        right[i] = max(0ll, maxs - pref[i-1]);
    }

    ll ans = 0;
    ll bestLeft = 0;
    for (int i = 1; i <= n; ++i) {
        ans = max(ans, left[i] * (x>0?x:1));
        ans = max(ans, bestLeft + right[i+1] + x * pref[i]);
        bestLeft = max(bestLeft, left[i] - x * pref[i]);
    }
    cout << ans << en;
}
