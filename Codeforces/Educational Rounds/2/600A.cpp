...
int main() {
   ...
   //  freopen("output.txt", "wt", stdout);
    ...
    V<string> b;
    string word = "";
    bool isIntegerNumber = 1;
    bool startWithZero = 0;
    for (int i = 0; i < isz(s); ++i) {
        if (s[i] == ',' || s[i] == ';'){
            if (!word.empty() &&isIntegerNumber && (!startWithZero || isz(word) == 1)){
                a.pb(word);
            }
            else
                b.pb(word);
        ...
        if (!word.empty() && isIntegerNumber && (!startWithZero || isz(word) == 1) ) a.pb(word);
        else b.pb(word);
    if (isz(a) == 0) {
        cout << '-' << en;
    }
