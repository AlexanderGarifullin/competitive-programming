...
bool check(){
    ...
    if (p2==-1)return 1;
    pll v = make_pair(x[p2] - x[p1], y[p2] - y[p1]);
    for (int i = 0; i < n; ++i) {
        if (used[i])continue;
        pll v2 = make_pair(x[i] -  x[p1], y[i] - y[p1]);
        if (cross(v, v2)!=0) return 0;
    }
    return 1;
}


bool good(int p1, int p2) {
    fill(all(used), false);
    pll v = make_pair(x[p2] - x[p1], y[p2] - y[p1]);
    for (int i = 0; i < n; ++i) {
        pll v2 = make_pair(x[i] -  x[p1], y[i] - y[p1]);
        if (cross(v, v2)==0) used[i]=true;
    }
    return check();
}
