...
//    freopen("output.txt", "wt", stdout);
    ...
    cin >> cntRed >> cntGreen >> cntBlue;
    vi r(cntRed), g(cntGreen), b(cntBlue);
    fin(r); fin(g); fin(b);
    sort(rall(r));
    sort(rall(g));
    sort(rall(b));
    vvvl dp(cntRed+1, vvl(cntGreen+1, vl(cntBlue+1)));
    ll res = 0;
    for (int curCntRed = 0; curCntRed <= cntRed; ++curCntRed) {
        for (int curCntGreen = 0; curCntGreen <= cntGreen; ++curCntGreen) {
            for (int curCntBlue = 0; curCntBlue <= cntBlue; ++curCntBlue) {

                if (curCntRed < cntRed && curCntGreen < cntGreen) {
                    dp[curCntRed+1][curCntGreen+1][curCntBlue] =
                            max(dp[curCntRed+1][curCntGreen+1][curCntBlue],
                                dp[curCntRed][curCntGreen][curCntBlue] +
                                r[curCntRed] * g[curCntGreen]);
                }

                if (curCntRed < cntRed && curCntBlue < cntBlue) {
                    dp[curCntRed+1][curCntGreen][curCntBlue+1] =
                            max(dp[curCntRed+1][curCntGreen][curCntBlue+1],
                                dp[curCntRed][curCntGreen][curCntBlue] +
                                r[curCntRed] * b[curCntBlue]);
                }

                if (curCntBlue < cntBlue && curCntGreen < cntGreen) {
                    dp[curCntRed][curCntGreen+1][curCntBlue+1] =
                            max(dp[curCntRed][curCntGreen+1][curCntBlue+1],
                                dp[curCntRed][curCntGreen][curCntBlue] +
                                b[curCntBlue] * g[curCntGreen]);
                }


                res = max(res, dp[curCntRed][curCntGreen][curCntBlue]);
            }
        }
    }
