...
void solve() {
    ...
    function<bool(int,int,int,int)> check = [&](int val, int c1, int c2, int c3) {
        for (int i = 0; i <= c1; ++i) {
            for (int j = 0; j <= c2; ++j) {
                if (i + 2 * j > val) continue;
                if ((val - i - 2 * j) % 3) continue;
                if ((val - i - 2 * j) / 3 <= c3) {
                    return true;
                }
            }
    ...
    function<bool(int,int,int)> can = [&](int c1, int c2, int c3) {
        for (int v : a) {
            if (!check(v, c1,c2,c3)) return false;
        }
        return true;
    };
