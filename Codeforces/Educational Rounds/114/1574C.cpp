...
i32 main() {
    ...
    // cout.setf(ios::fixed); cout.precision(20);
        ...
        for (int i = 0; i < m; ++i) {
            ll x,y; cin >> x >> y;
            auto itHeroToFight = lower_bound(all(a), x);
            if (itHeroToFight == a.end()) {
                ll hero = a.back();
                ll answ = max(0ll, x - hero);
                answ += max(0ll, y + hero - sum);
                cout << answ << en;
            }
            else if (*itHeroToFight == x || itHeroToFight == a.begin()) {
                ll hero = *itHeroToFight;
                ll answ = max(0ll, x - hero);
                answ += max(0ll, y + hero - sum);
                cout << answ << en;
            }
                ...
                ll hero = *itHeroToFight;
                ll answ1 = max(0ll, x - hero);
                answ1 += max(0ll, y + hero - sum);
                itHeroToFight--;
                ll hero2 = *itHeroToFight;
                ll answ2 = max(0ll, x - hero2);
                answ2 += max(0ll, y + hero2 - sum);
                cout << min(answ1,answ2) << en;
            }
        }
}
