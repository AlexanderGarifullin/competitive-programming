...
void CBO() {
    ...
    it = prev(it);
    vvi ans;
    pqsmalleststart<pii> pq;
    while(true) {
        auto [value, cn] = *it;

        int maxCan = c[value] - 1;
        while(!pq.empty() && cn > 0 && pq.top().first <= maxCan) {
            auto [sz, id] = pq.top(); pq.pop();
            int add = min(cn, maxCan + 1 - sz);
            for (int i = 0; i < add; ++i) {
                ans[id].pb(value);
            }
            cn -= add;
            pq.push(make_pair(isz(ans[id]), id));
        }
        ...
        if (cn > 0) {
            ...
            while(cn > 0) {
                int id = isz(ans);
                ans.pb({});
                for (int i = 0; i < min(sz, cn); ++i) {
                    ans[id].pb(value);
                }
                int tt = isz(ans[id]);
                pq.push(make_pair(tt, id));
                cn -= min(sz, cn);
            }
        }
