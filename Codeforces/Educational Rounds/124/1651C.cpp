...
i32 main() {
    ...
    int t; cin >> t; while(t--) {
        ...
        vl b(n); fin(b);
        ll res = 1e18;
        ll bestA1=1e18,bestAn=1e18, bestB1=1e18, bestBn= 1e18;
        for (int i = 0; i < n; ++i) {
            bestA1= min(bestA1, abs(a[0]-b[i]));
            bestAn= min(bestAn, abs(a.back()-b[i]));
            bestB1= min(bestB1, abs(b[0]-a[i]));
            bestBn= min(bestBn, abs(b.back()-a[i]));
        }
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 3; ++j) {
                ll cur = 0;
                if (i == 0) cur += abs(a[0] - b[0]);
                else if (i == 1) cur += abs(a[0]-b.back());
                else cur += bestA1;
                if (j == 0) cur += abs(a.back() - b[0]);
                else if (j == 1) cur += abs(a.back()-b.back());
                else cur += bestAn;
                set<int> st = {i,j};
                if (st.find(0)==st.end()) {
                    cur += bestB1;
