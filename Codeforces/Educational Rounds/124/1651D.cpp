...
void solve() {
    ...
    for (auto [x,y] : cords) {
        for (int i = 0; i < 4; ++i) {
            int nx = x + dx[i];
            int ny = y + dy[i];
            if (st.contains(make_pair(nx,ny))) continue;
            ans[{x,y}] = {nx,ny};
            q.push({x,y});
            break;
        }
    ...
    while(!q.empty()){
        ...
        for (int i = 0; i < 4; ++i) {
            int nx = x + dx[i];
            int ny = y + dy[i];
            if (!st.contains({nx,ny}) || ans.contains({nx,ny})) continue;
            ans[{nx,ny}] = ans[{x,y}];
            q.push({nx,ny});
        }
    }

    for (int i = 0; i < isz(cords); ++i) {
        cout << ans[cords[i]].first << ' ' << ans[cords[i]].second<<en;
    }
}
