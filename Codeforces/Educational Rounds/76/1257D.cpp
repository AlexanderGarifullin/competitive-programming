...
int main() {
  ...
  //  freopen("joseph.out", "wt", stdout);
    ...
    while (t--) {
        ...
        vi monsters(n + 1);
        vi max_power(n + 1);
        int max_power_monster = 0;
        int max_power_hero = 0;
        for (int i = 1; i <= n; ++i) {
            cin >> monsters[i];
            setmax(max_power_monster, monsters[i]);
        }
        int m;
        cin >> m;
        for (int i = 1; i <= m; ++i) {
            int p, s;
            cin >> p >> s;
            setmax(max_power[s], p);
            setmax(max_power_hero, p);
        }

        for (int i = n - 1; i > 0; --i) {
            setmax(max_power[i], max_power[i + 1]);
        }

        // Если макс сила монстра > макс силы героя => не может убить этого монстра.
        if (max_power_monster > max_power[1]) {
            cout << -1 << en;
        } else {
            int now = 0, len = 0, ans = 0;
            for (int i = 1; i <= n; ++i) {
                now = max(now, monsters[i]);
                len++;
                if (max_power[len] >= now) continue;
                ans++;
                len = 1;
                now = monsters[i];
            }
