...
string norm(const string& v) {
    int cnt = 0;
    while(cnt < isz(v) && v[cnt] == '0') cnt++;
    if (cnt == isz(v)) return "0";
    return v.substr(cnt, isz(v) - cnt);
}

string operator |(const string& a, const string& b) {
    int sz = max(isz(a), isz(b));
    str ans(sz, '0');
    for (int i = 0; i < isz(a); ++i) {
        if (a[i] == '1') ans[i + sz - isz(a)] = '1';
    }
    for (int i = 0; i < isz(b); ++i) {
        if (b[i] == '1') ans[i + sz - isz(b)] = '1';
    }
    return norm(ans);
}

bool better(const string& a, const string& b) {
    if (isz(a) != isz(b)) return isz(a) > isz(b);
    else return a > b;
}
