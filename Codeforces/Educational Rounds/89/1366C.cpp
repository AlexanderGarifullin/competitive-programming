...
i32 main() {
    ...
    while(t--){
        ...
        /* Find len from (1;1)
         ...
         1 2 3 4
         2 3 4 5
         we want that i and k - i group was equal
         all from group 0 = all from group 5
         1 = 4
         2 = 3
         if (n + m % 2 == 0)
         0 1 2
         1 2 3
         2 3 0
         Center group haven't pair. We skip it
         */
        ...
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < m; ++j) {
                if (i == 0 &&  j == 0) {
                    if (field[i][j]) cntOne[len[i][j]]++;
                    else cntZero[len[i][j]]++;
                    continue;
                }
                else if (i == 0) len[i][j] = len[i][j-1] + 1;
                else if (j == 0) len[i][j] = len[i-1][j] + 1;
        ...
        ll answ = 0;
        if ((n + m) % 2 == 0){
            for (int i = 0; i < isz(cntZero)  / 2 ; ++i) {
                // want 1
                int dif1 = cntZero[i] + cntZero[isz(cntZero) - 1 - i];
                // want 0
                int dif0 = cntOne[i] + cntOne[isz(cntZero) - 1 - i];
                // what smaller
                answ += min(dif1, dif0);
            }
        }
