...
ll get(int p, int l, int r) {
    ...
    int lenRight = r - p - 1;
    int lenLeft = p - l - 1;
    ll ans = 1 + lenRight + lenLeft;
    ans += 1ll * lenLeft * lenRight;
    return ans;
}
...
void solve() {
    ...
    ll sumMinimum = 0;
    for (int i = 1; i <= n; ++i) {
        sumMinimum += 1ll * a[i] * get(i, lft[i], rgt[i]);
    }
    for (int i = 1; i <= n; ++i) {
        a[i] = -a[i];
    }
    ...
    ll sumMaximum = 0;
    for (int i = 1; i <= n; ++i) {
        sumMaximum += -1ll * a[i] * get(i, lft[i], rgt[i]);
    }
    cout << sumMaximum - sumMinimum;
}
