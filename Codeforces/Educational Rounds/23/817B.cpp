...
i32 main() {
    ...
    int n; cin >> n;
    vi a(n); fin(a);
    int min1 = Min(a[0],a[1],a[2]);
    int min3 = Max(a[0],a[1],a[2]);
    int min2 = a[1]+ a[0] + a[2] - min1 - min3;
    for (int i = 3; i < n; ++i) {
        if (a[i] < min1) {
            min2 = min1;
            min3 = min2;
    ...
    if (min1 == min2 && min2 == min3) {
        cout << max(1ll,perm(cntMin1 , 3)) << en;
    }
    else if (min1 == min2){
        cout << max(1ll,cntMin3  * perm(cntMin1, 2)) << en;
    }
    else {
        if (min2 == min3) {
            cout << max(1ll, cntMin1 * perm(cntMin3, 2)) << en;
        }
        else
            cout << max(1ll,cntMin1 * cntMin2 * cntMin3) << en;
    }
