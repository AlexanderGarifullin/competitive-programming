...
vl solve(int n, int m, vi a, vi b) {
    int sz = n + m + 1;
    vl res(sz);
    vvi cntTake(2, vi(sz));
    vl resOnPref(sz);
    ...
    for (int i = 0; i < sz-1; ++i) {
        ...
        cntTake[1][i] = cntb;
        if (i>0) {
            resOnPref[i] += resOnPref[i-1];
        }
    }
    res.back() = resOnPref[sz-2];
    vi posRigthA(sz), posRigthB(sz);
    for (int i = sz-1; i >=0; --i) {
        if (i < sz-1) {
            posRigthA[i] = posRigthA[i+1];
            posRigthB[i] = posRigthB[i+1];
        }
        if (a[i] > b[i]) {
            posRigthA[i] = i;
        } else {
    ...
    for (int i = sz-2; i >=0; --i) {
        ...
        if (was[i] == 1) {
            ...
            cur -= b[i];
            if (posLastGoodB != -1) {
                if (posLastGoodB == sz - 1) {
                    cur += b[posLastGoodB];
                } else {
                    cur += b[posLastGoodB];
                    cur -= a[posLastGoodB];
                    if (was[posLastGoodB] == 1) {
                        cur += a.back();
                    } else  {
                        cur += b.back();
                    }
