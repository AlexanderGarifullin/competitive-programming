...
void svo(){
    ...
    for (int i = 0; i < n; ++i) {
        ll aa = v[i].first;
        ll bb = v[i].second;
        pref[i] = make_pair( aa, bb - aa);
        if (i > 0) {
            if (pref[i-1].second > pref[i].second) {
                pref[i] = pref[i-1];
            } else if (pref[i-1].second == pref[i].second) {
    ...
    for (int i = 0; i < m; ++i) {
        if (c[i] < minValA) continue;
        ll add = 0;
        ll w = c[i];
        ll oldw = w;
        vl wstates, addstates;
        ...
        while (w >= minValA) {
            ...
            while (l + 1 < r) {
                ll mid = (l + r) / 2;
                bool ok = true;
                ok &= (w + mid * good[pos].second) >= good[pos].first;
                if (ok) l = mid;
                else r = mid;
            }
            add += 2 * l + 2;
            addstates.pb(add);
            w = w + l * good[pos].second;
            ll aa = good[pos].first;
            ll bb = good[pos].second + aa;
            w = w - aa + bb;
        }
        for (int j = 0; j < isz(addstates); ++j) {
            w_add[wstates[j]] = addstates.back() -
                                (j>0? addstates[j-1] : 0);
        }
