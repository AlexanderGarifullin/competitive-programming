...
i32 main() {
    ...
    vi r(n + 1), p(n+1), minEl(n+1), maxEl(n+1), cnt(n+1);
    for (int i = 1; i <= n; ++i) {
        p[i] =i; minEl[i] = i; maxEl[i] = i, cnt[i] = 1;
    }
    ...
    function<void(int,int)> Union = [&](int x, int y) {
        ...
        int u2 = Find(y);
        if (u1 == u2) return ;
        if (r[u1] > r[u2]) swap(u1,u2);
        p[u1] = u2;
        cnt[u2] += cnt[u1];
        minEl[u2] = min(minEl[u2], minEl[u1]);
        maxEl[u2] = max(maxEl[u2], maxEl[u1]);
        if (r[u1] == r[u2]) r[u2]++;
    };
