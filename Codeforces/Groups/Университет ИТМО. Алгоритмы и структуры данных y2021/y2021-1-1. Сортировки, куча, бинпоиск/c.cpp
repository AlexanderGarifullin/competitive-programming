...
i32 main() {
    ...
    function<vi(int, int)> Find = [&](int l, int r) {
        if (l + 1 == r) {
            vi cur = { v[l]};
            return cur;
        }
        ...
        for (int i = 0; i < isz(v2); ++i) {
            while(ii < isz(v1) && v1[ii] < v2[i]) {
                ii++;
            }
            if (ii == isz(v1)) break;
            cnt += isz(v1) - ii;
        ...
        vi cur;
        cur.reserve(isz(v1) + isz(v2));
        int i =0, j = 0;
        while(i < isz(v1) || j < isz(v2)) {
            if (i < isz(v1) && j < isz(v2)) {
                if (v1[i] < v2[j]) cur.push_back(v1[i++]);
                else cur.push_back(v2[j++]);
            }
            else if (i < isz(v1)) cur.push_back(v1[i++]);
            else cur.push_back(v2[j++]);
        }
        return cur;
    };
