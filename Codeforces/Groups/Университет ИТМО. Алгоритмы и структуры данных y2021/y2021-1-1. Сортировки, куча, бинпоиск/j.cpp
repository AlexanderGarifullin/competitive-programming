...
//    freopen("output.txt", "wt", stdout);
    ...
    function<bool(ld)> ok = [&](ld R) {
        vld vv(n);
        for (int i = 0; i < n; ++i) {
            vv[i]=v[i].first - R * v[i].second;
        }
        sort(rall(vv));
        ld s = accumulate(vv.begin(),vv.begin()+k,0.0l);
        return s>=0;
    };
    ld left = 0;
    ld right = 1e8;
    ...
    vector<pair<ld,int>> v2(n);
    for (int i = 0; i < n; ++i) {
        v2[i].second=i+1;
        v2[i].first = v[i].first - left * v[i].second;
    }
    sort(rall(v2));
    for (int i = 0; i < k; ++i) {
        cout << v2[i].second << en;
