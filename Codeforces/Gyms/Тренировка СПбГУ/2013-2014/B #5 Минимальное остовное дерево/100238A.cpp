...
int get(int u, vi &p) {
    ...
    if (p[u] != u) p[u] = get(p[u], p);
    return p[u];
};

void unite(int a, int b, vi & p){
    int p1 = get(a, p);
    int p2 = get(b, p);
    p[p1] = p2;
}
...
ll dist(int u, int v, vpii & cords, ll p1, ll p2) {
    return (cords[u].first * cords[v].first +
            p1 * cords[u].second * cords[v].second)% p2;
}
    ...
    for (int i = 1; i <= n; ++i) {
        for (int j = 1; j <= m; ++j) {
            if (i+1<=n) {
                ...
                edges.pb(make_tuple(dist(id[i][j], id[i+1][j], cords, p1, p2), id[i][j], id[i+1][j]));
            }
            if (j+1<=m) {
                if (id[i][j+1]==-1) {
                    id[i][j+1] = isz(cords);
                ...
                edges.pb(make_tuple(dist(id[i][j], id[i][j+1], cords, p1, p2), id[i][j], id[i][j+1]));
            }
            if (i+1<=n && j+1<=m) {
                if (id[i+1][j+1]==-1) {
                    id[i+1][j+1] = isz(cords);
                    cords.pb({i+1,j+1});
                }
                edges.pb(make_tuple(dist(id[i][j], id[i+1][j+1], cords, p1, p2), id[i][j], id[i+1][j+1]));
            }
        }
    }
