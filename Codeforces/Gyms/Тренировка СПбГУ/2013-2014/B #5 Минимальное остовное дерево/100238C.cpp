...
i32 main() {
    freopen("connect.in", "rt", stdin);
    freopen("connect.out", "wt", stdout);
    ...
    function<ld(int,int)> dist = [&](int u, int v){
        return sqrtl((cords[u].first - cords[v].first)*(cords[u].first - cords[v].first)
        +
        (cords[u].second - cords[v].second) * (cords[u].second - cords[v].second));
    };

    vector<vector<pair<ld, int>>> g(k);
    ...
    function<ld(int,int)> calc = [&](int cur, int from) {
        ld res = 0;
        for (auto [w, next] : g[cur]) {
            if (next != from) {
                res += w + calc(next, cur);
            }
        }
    ...
    cout << calc(0,0) << en;
    cout << k - 1<< en;
    function<void(int,int)> print = [&](int cur, int from) {
        for (auto [w, next] : g[cur]) {
            if (next != from) {
                cout << cur +1<< ' ' <<next + 1 << en;
                print(next, cur);
            }
        }
    };
