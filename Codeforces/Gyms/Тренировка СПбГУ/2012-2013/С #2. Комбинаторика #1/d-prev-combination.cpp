...
i32 main() {
     ...
     freopen("generation.in", "rt", stdin);
     freopen("generation.out", "wt", stdout);
    ...
    fill_fact(fact);
    long long cnt = fact[n] / (fact[n-k] *fact[k]);

    vector<vector<int>> answ;

    for (int i = 0; i < cnt; ++i) {
        answ.pb(combination);
        combination = prev_combination(combination, n, k);
    }

    sort(answ.begin(), answ.end(),[](vector<int>& v1, vector<int> &v2){
        for (int i = isz(v1) - 1; i >= 0; --i) {
