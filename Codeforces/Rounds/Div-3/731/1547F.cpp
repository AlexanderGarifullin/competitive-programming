...
int get(int v, int lv, int rv, int l, int r) {
    ...
    int m = (lv+rv)/2;
    int gg = 0;
    gg = __gcd(gg, get(v*2, lv, m, l, r));
    gg = __gcd(gg, get(v*2+1, m, rv, l, r));
    return gg;
}
...
void CBO() {
    ...
    tree = vi(4*n);
    build(1, 0, n);
    function<bool(int,int)> ok = [&](int pos, int move) {
        int pos2 = pos + move;
        int gg = get(1, 0, n, pos, min(n-1, pos2) + 1);
        if (pos2 >= n) {
            gg = __gcd(gg, get(1, 0, n, 0, (pos2 % n) + 1));
        }
        return gg == g;
    };
    vi cnt(n);
