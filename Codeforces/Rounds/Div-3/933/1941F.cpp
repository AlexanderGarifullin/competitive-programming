...
void multiSolve(){
    int t_; cin >> t_; while(t_--) {
        ll n,m,k; cin >> n >> m >> k;
        vl a(n); fin(a);
        vl d(m); fin(d); sort(all(d));
        vl f(k); fin(f); sort(all(f));
        ll m1=0, m2=0;
        ll ind = -1;
        for (ll i = 0; i + 1< n; ++i) {
            ll dif = a[i+1] - a[i];
            if (dif > m1) {
                ...
                m1 = dif;
                ind = i;
            } else if (dif > m2) m2 = dif;
        }
        ll ans = m1;
        ll median = (a[ind] + a[ind+1]) / 2;
        for (int x: d) {
            ll l = 0;
            ll r = k - 1;
            while (l + 1 < r) {
            ...
            ll v1 = x + f[l];
            ans = min(ans, Max(m2, abs(v1 - a[ind]), abs(v1 - a[ind+1])));
            ll v2 = x + f[r];
            ans = min(ans, Max(m2, abs(v2 - a[ind]), abs(v2 - a[ind+1])));
        }
        cout << ans << en;
    }
}
