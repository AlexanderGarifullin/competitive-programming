...
i32 main() {
    ...
    while(t--){
        ...
        map<vi, int> types;
        vi vertexTypes(n+1, -1);
        function<void(int,int)> dfs = [&](int u, int p){
            vi temp;
            for (auto & v : adj[u]){
                if (v == p) continue;
                dfs(v, u);
                temp.pb(vertexTypes[v]);
            }
            sortsmalltolarge(temp);
            auto it = types.insert({temp, isz(types)});
            vertexTypes[u] = it.first->second;
        };
        dfs(1,0);
        ...
        function<bool(int,int)> check = [&](int u, int p){
            ...
            vi odd;
            for (auto & [type, cntType] : map)
            {
                if (cntType % 2) odd.pb(type);
            }
            if (isz(odd) == 0) return true;
            if (isz(odd) > 1) return false;
            for (int &v : adj[u])
            {
                if (v == p) continue;
                if (odd.front() == vertexTypes[v])
                    return check(v, u);
            }
        };
