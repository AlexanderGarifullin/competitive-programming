...
void CBO() {
    ...
    for (int i = 0; i < n; ++i) {
        if (i == 0) {
            best=min(best, (c+a[0]-1)/a[0]);
        } else {
            ll cur = (max(0ll, c - ost[i-1]) + a[i] - 1) / a[i];
            best = min(best, need[i-1] + cur);
        }

        if (i < n-1) {
            int needHave = b[i] - (i>0?ost[i-1]:0);
            if (needHave <= 0) {
                ost[i] = ost[i - 1] - b[i];
                need[i] += need[i-1];
            } else {
                ll k = (needHave + a[i] - 1) / a[i];
                need[i] = k;
                ost[i] = k * a[i] - needHave;
                if (i>0) {
                    need[i] += need[i-1];
                }
            }
