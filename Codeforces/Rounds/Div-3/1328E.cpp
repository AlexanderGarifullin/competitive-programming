...
int t = 0;
vi tin,tout, deep,p;
void dfs(int curV, int pred = -1) {
    if (pred != -1) {
        deep[curV] = 1 + deep[pred];
    }
    p[curV] = pred;
    tin[curV] = t++;
    for (int next:g[curV]) {
...
bool isanc(int asc, int des) {
    return tin[asc] <= tin[des] && tout[des] <= tout[asc];
}

void CBO(){
    int n,m; cin >> n >> m;
    g = vvi(n+1);
    tin = tout = deep = p = vi(n+1);
    for (int i = 0; i < n-1; ++i) {
        int u,v; cin >> u >> v;
        g[u].pb(v);
        g[v].pb(u);
    ...
    for (int i = 0; i < m; ++i) {
        ...
        int deepest = v[0];
        for (int j = 0; j < k; ++j) {
            if (deep[v[j]] > deep[deepest]) {
                deepest = v[j];
            }
        }
        for (int j = 0; j < k; ++j) {
            if (v[j] == deepest || p[v[j]] == -1) continue;
            v[j] = p[v[j]];
        }

        bool ok = true;
        for (int j = 0; j < k && ok; ++j) {
            ok &= isanc( v[j], deepest);
        }
        ifyn(ok);
    }
}
