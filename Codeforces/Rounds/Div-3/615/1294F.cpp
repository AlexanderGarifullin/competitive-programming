...
void solve() {
    ...
    dp1[1] = 0;
    dfs1(1,  dp1);
    int FirstEnd = max_element(all(dp1)) - dp1.begin();
    dp1 = vi(n+1, -1);
    dp1[FirstEnd] = 0;
    par = vi(n+1,-1);
    dfs1(FirstEnd, dp1);
    int SecondEnd = max_element(all(dp1)) - dp1.begin();
    if (dp1[SecondEnd] == n-1) {
        cout << n-1 << en;
        cout << FirstEnd << ' ' << SecondEnd << ' ';
        for (int i = 1; i <= n; ++i) {
            if (i != FirstEnd && i != SecondEnd) {
                cout << i << en;
                break;
            }
        }
        ...
        cout << dp1[SecondEnd] + best.first << en;
        cout << FirstEnd << ' ' << SecondEnd << ' ' << best.second << ' ' << en;
    }
}
