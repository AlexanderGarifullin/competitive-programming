...
i32 main() {
    ...
    int t; cin >> t; while(t--) {
        ...
        bool ok = true;
        for (int i = 0; i < n; ++i) {
            if (cords[i].second < curHeight) {
                ok = false;
                break;
            } else {
                if (curX < cords[i].first) {
                    answ += string(cords[i].first - curX, 'R');
                    curX = cords[i].first;
                }
                if (curHeight < cords[i].second) {
                    answ += string(cords[i].second - curHeight,'U');
                    curHeight = cords[i].second;
                }
            }
        }
        if (ok) {
