...
//    freopen("bees.out", "wt", stdout);
    ...
    for (int i = 0; i < q; ++i) {
        int val; cin >> val;
        val %= x;
        int oldcnt = val_cnt[val];
        cnt_values[oldcnt].erase(val);
        if (cnt_values[oldcnt].empty()) {
            cnt_values.erase(oldcnt);
        }
        val_cnt[val] = oldcnt + 1;
        cnt_values[oldcnt+1].ins(val);
        int mincnt = cnt_values.begin()->first;
        int minvalue = *(cnt_values.begin()->second).begin();
        cout << 1ll*x*mincnt+minvalue<< en;
    }
}
