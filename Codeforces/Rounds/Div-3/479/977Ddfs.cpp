...
i32 main() {
    ...
    int n; cin >> n;
    vl a(n); fin(a);
    ll minnumber  = *min_element(all(a));
    ...
    for (int i = 0; i < n; ++i) {
        if (a[i] == minnumber) {
            ...
            q.push({a[i], 0});
            while(!q.empty()) {
                auto [curNumber, type] = q.front();
                q.pop();
                if (type == 0) {
                    ans.push_back(curNumber);
                    bool curFind = false;
                    for (int j = 0; j < n; ++j) {
                        if (!use[j] && a[j] == curNumber * 3) {
                            use[j] = true;
                            curFind = true;
                            q.push({a[j], 1});
                            break;
