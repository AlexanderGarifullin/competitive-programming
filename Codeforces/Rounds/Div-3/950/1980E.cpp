...
bool solve(int n, int m, vvi &a, vvi &b) {
    ...
    for (int i = 0; i < m; ++i) {
        swap(b[0][i], b[bStart][i]);
    }

    vb used(m);
    for (int i = 0; i < m; ++i) {
        if (a[0][i] != b[0][i]) {
            if (bpos.find(a[0][i]) == bpos.end()) return false;
            int posWant = bpos[a[0][i]];
            if (used[posWant]) return false;
            int oldV = b[0][i];
            for (int j = 0; j < n; ++j) {
                swap(b[j][i],b[j][posWant]);
            }
            bpos[oldV] = posWant;
            bpos[a[0][i]] = i;
        }
