...
bool solve(int n, vi &a, vi &b, int m, vi &d) {
    multiset<int> setd;
    for (int i = 0; i < m; ++i) {
        setd.insert(d[i]);
    }
    ...
    for (int i = 0; i < n; ++i) {
        if (a[i] != b[i]) {
            if (setd.find(b[i]) == setd.end()) return false;
            setd.erase(setd.find(b[i]));
        }
        setb.ins(b[i]);
    }

    for (int i = m-1; i >= 0; --i) {
        if (setd.find(d[i]) != setd.end()) {
            if (i == m - 1) {
                if (setb.find(d[i]) == setb.end()) return false;
            }
            else  {
                return true;
            }
