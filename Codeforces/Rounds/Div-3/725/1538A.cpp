...
i32 main() {
    ...
    while(t--){
        int n; cin >> n;
        vi a(n); fin(a);
        auto mmax = max_element(all(a)) - a.begin();
        auto mmin = min_element(all(a)) - a.begin();
        int ll = min(mmax,mmin);
        int rr = max(mmax, mmin);
        int cntlleft = ll+1;
        int cnrright = n - rr;
        int dif = rr - ll;
        cout << Min(cntlleft + cnrright, cntlleft + dif , cnrright + dif)<< en;
    }
