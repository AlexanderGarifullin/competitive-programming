...
int get(vvi v, vvi pref, int a, int b, int c) {
    int res =0;
    while (pref[6][0] <= a && pref[6][1] <= b && pref[6][2] <= c) {
        a -= pref[6][0]; b -= pref[6][1]; c -= pref[6][2];
        res+=7;
    }
    for (int i = 0; i < 7; ++i) {
...
//    freopen("output.txt", "wt", stdout);
    ...
    for (int startDay = 0; startDay < 7; ++startDay) {
        int cur = 0;
        int aa =a, bb = b, cc =c;
        int needA= pref[6][0], needB =pref[6][1], needC = pref[6][2];
        if (startDay > 0) {
            needA -= pref[startDay-1][0];
            needB -= pref[startDay-1][1];
            needC -= pref[startDay-1][2];
        }
        if (needA <= aa & needB <= bb && needC <= c) {
            cur = 7 - startDay;
            int add = get(v, pref, aa - needA,bb - needB,cc - needC);
            res = max(res,cur + add);
        } else {
            for (int i = startDay; i < 7; ++i) {
                if (v[i][0] <= aa && v[i][1] <= bb && v[i][2] <= cc) {
                    aa -= v[i][0]; bb -= v[i][1]; cc -= v[i][2];
                    cur++;
                } else break;
            }
            res = max(res, cur);
