...
i32 main() {
  ...
  //  ios_base::sync_with_stdio(false); cin.tie(nullptr); cout.tie(nullptr);
    // cout.setf(ios::fixed); cout.precision(20);
        ...
        map<int, char> map1;
        map1[0] = 'R'; map1[1] = 'B'; map1[2] = 'G';

        map<char,int> map2;
        map2['R'] = 0; map2['B'] = 1; map2['G'] = 2;

        vvi dp(3, vi(n)), pred(3, vi(n, -1));
        for (int i = 0; i < n; ++i) {
            int curIndex = map2[s[i]];
            for (int j = 0; j < 3; ++j) {
                if (i == 0) dp[j][i] = (curIndex != j);
                else {
                    int mmin = 1e9, jmin = -1;
                    for (int k = 0; k < 3; ++k) {
                        if (k != j && dp[k][i-1] < mmin) {
                            mmin = dp[k][i-1];
                            jmin = k;
                        }
                    }
                    pred[j][i] = jmin;
                    dp[j][i] = mmin + (curIndex != j);
                }
            }
        }
//        for (int i = 0; i < 3; ++i) {
