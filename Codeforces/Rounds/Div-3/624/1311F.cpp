...
void upd(vl & v, int pos, int val) {
    for (; pos < isz(v); pos |= pos + 1) {
        v[pos] += val;
    }
}
...
void solve() {
    ...
    sort(all(cv));
    reuniq(cv);
    vl cntSum(isz(cv) + 1), valueSum(isz(cv) + 1);
    ll ans = 0;
    for (int i : ord) {
        int pos = lower_bound(all(cv), v[i]) - cv.begin();
        ans += get(cntSum, pos) * x[i] - get(valueSum, pos);
        upd(cntSum, pos, 1);
        upd(valueSum, pos, x[i]);
    }
    cout << ans << en;
}
