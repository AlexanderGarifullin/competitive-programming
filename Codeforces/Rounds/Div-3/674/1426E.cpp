...
int minsolve(int n, vi a, vi b) {
    ...
    do {
        vi a1 = a;
        vi b1 = b;
        for (int i = 0; i < isz(ord); ++i) {
            int c = min(a1[ord[i].first], b1[ord[i].second]);
            a1[ord[i].first] -= c;
            b1[ord[i].second] -= c;
        }
...
int maxsolve(int n, vi a, vi b) {
    int r = min(a[0],b[1]) + min(a[1],b[2]) + min(a[2],b[0]);
    return r;
}
...
//    freopen("output.txt", "wt", stdout);
    ...
    vi a(3); fin(a);
    vi b(3); fin(b);
    cout << minsolve(n,a,b)<< ' ' << maxsolve(n,a,b) << en;
}
