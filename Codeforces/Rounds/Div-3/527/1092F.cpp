...
i32 main() {
    ...
    //timer.start();
     ...
     //  freopen("absum.in", "rt", stdin); freopen("absum.out", "wt", stdout);
    ...
    function<void(int,int)> find_best = [&](int u, int p) {
        bestSum = max(bestSum, curSum);
        for (int &v : g[u]){
            if (v == p) continue;
            ll delta = sumInTree[1] - 2 * sumInTree[v];
            curSum += delta;
            find_best(v, u);
            curSum -= delta;
        }
