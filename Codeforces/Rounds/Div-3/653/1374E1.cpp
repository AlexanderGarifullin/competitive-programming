...
//    freopen("bees.out", "wt", stdout);
    ...
    //  cout.setf(ios::fixed); cout.precision(20);
        ...
        sort(all(t1));
        sort(all(t2));
        vl p1(isz(t1)+1),p2(isz(t2)+1),p3(isz(t3)+1);
        for (int i = 0; i < isz(t1); ++i) {
            p1[i+1] = p1[i] + t1[i];
        }
        for (int i = 0; i < isz(t2); ++i) {
            p2[i+1]=p2[i]+t2[i];
        }
        for (int i = 0; i < isz(t3); ++i) {
            p3[i+1]=p3[i]+t3[i];
        }

        ll res = 1e18;
        for (int i = 0; i < isz(p3); ++i) {
            int cur = k - i;
            if (cur < 0) break;
            if (cur > isz(t1) || cur > isz(t2)) continue;
            ll s = p3[i] + p1[cur] + p2[cur];
            res = min(res, s);
        }
        if (res == 1e18) res = -1;
        cout << res << en;
