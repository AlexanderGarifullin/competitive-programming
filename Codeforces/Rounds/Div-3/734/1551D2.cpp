...
vs get(int n, int m, int k) {
    ...
    if (n % 2 == 0 && m % 2 == 0) {
        ...
        pair<char,char> p2 = {'v','e'};
        bool d = false;
        for (int col = 0; col + 1 < m && k; col += 2) {
            for (int row = 0; row +1 < n && k; row +=2) {
                if (d) {
                    v[row][col] = v[row][col+1]= p1.first;
                    v[row+1][col] = v[row+1][col+1] = p1.second;
                } else {
                    v[row][col] = v[row][col+1]= p2.first;
                    v[row+1][col] = v[row+1][col+1] = p2.second;
                }
                k -= 2;
            }
            d = !d;
        ...
        for (int col = 0; col < m; ++col) {
            bool d = col % 2;
            for (int row = n-1; row - 1 >=0 ; row-=2) {
                if (v[row-1][col] != '-') break;
                if (d) {
                    v[row-1][col] = v[row][col] = 'd';
                } else {
                    v[row-1][col] = v[row][col] = 'c';
                }
                d = !d;
            }
        }
        ...
        pair<char,char> p2 = {'v','e'};
        bool d = false;
        for (int col = 0; col + 1 < (m-1) && k; col += 2) {
            for (int row = 0; row +1 < n && k; row +=2) {
                if (d) {
                    v[row][col] = v[row][col+1]= p1.first;
                    v[row+1][col] = v[row+1][col+1] = p1.second;
