...
// РЁР°Р±Р»РѕРЅ РЅР° РїРѕРёСЃРє РјР°РєСЃРёРјСѓРјР° СЃСЂРµРґРё РЅРµСЃРєРѕР»СЊРєРёС… СЌР»РµРјРµРЅС‚РѕРІ.
template<typename T>
T Max(T a)
{
    return a;
...
// РЁР°Р±Р»РѕРЅ РЅР° РїРѕРёСЃРє РјРёРЅРёРјСѓРјР° СЃСЂРµРґРё РЅРµСЃРєРѕР»СЊРєРёС… СЌР»РµРјРµРЅС‚РѕРІ.
template<typename T>
T Min(T a)
{
    return a;
...
// РЈРґР°Р»РµРЅРёРµ РїРѕРІС‚РѕСЂСЏСЋС‰РёС…СЃСЏ СЌР»РµРјРµРЅС‚РѕРІ РІ РѕС‚СЃРѕСЂС‚РёСЂРѕРІР°РЅРЅРѕРј РјР°СЃСЃРёРІРµ. 1 2 2 3 3 4 6 = 1 2 3 4 6.
...
// РћР±СЂР°С‚РЅРѕРµ С‡РёСЃР»Рѕ a^(-1) = a^(p-2)
// a^(-1) = binpow(a, mod - 2, mod)
// 1) q, q^2, ... , q^k, 0, 0 ... - РќРµРѕР±СЂР°С‚РёРјС‹Р№ СЌР»РµРјРµРЅС‚. binpow(q, 10^18, mod) == 0. q^(k+1) % mod == 0. РќРµР»СЊР·СЏ РЅР°Р№С‚Рё q^(-1)
// 2) q, q^2, ... , q^k, 1, q,  q^2, ... , q^k, ... РЎСѓС‰РµСЃС‚РІСѓРµС‚ РѕР±СЂР°С‚РЅС‹Р№ СЌР»РµРјРµРЅС‚. РўРѕРіРґР° => a^(-1) = a^(p-2) (mod p)
// a^(-1) = binpow(a, mod - 2, mod) - Р•СЃР»Рё mod - РїСЂРѕСЃС‚РѕРµ.
// a^(-1) = binpow(a, f(m) - 1, m) - Р’ РѕР±С‰РµРј СЃР»СѓС‡Р°Рµ (Р•СЃР»Рё m РЅРµ РїСЂРѕСЃС‚РѕРµ).  f(m) - Р¤СѓРЅРєС†РёСЏ РР№Р»РµСЂР° - РєРѕР»-РІРѕ С‡РёСЃРµР» РѕС‚ 1 РґРѕ m - 1, РіРґРµ РёС… РќРћР” СЃ m = 1
// Р¤СѓРЅРєС†РёСЏ СЌР№Р»РµСЂР°: f(p1^k1, p2^k2, ... , pn^kn) =  (p1^k1 - p1^(k1-1)) * (p2^k2 - p2^(k2-1)) (pn^kn - pn^(kn-1));
...
str solveB(vvc &v){
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        if (ok && v[i][0] != '.') { answ += v[i][0]; return answ;}
    }
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        if (ok && v[0][i] != '.') { answ += v[0][i]; return answ;}
    }

    if (v[1][1] == '.') return "DRAW";
    if (v[0][0] == v[1][1] && v[1][1] == v[2][2] || v[0][2] == v[1][1] && v[1][1] == v[2][0]) { answ += v[1][1]; return answ;}
    return "DRAW";
}
