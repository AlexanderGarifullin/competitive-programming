...
void CBO() {
    ...
    for (int i = 0; i < m; ++i) {
        int x,y; cin >> x >> y;
        ll w; cin >> w;
        e[i] = make_tuple(x,y,w);
    }
    int mp = min_element(a.begin()+1, a.end()) - a.begin();
    for (int i = 1; i <= n; ++i) {
        if (i == mp) continue;
        e.emplace_back(mp, i, a[mp] + a[i]);
    }
    sort(all(e), [](tuple<int,int,ll> t1,
            tuple<int,int,ll> t2) {
       auto [from1, to1, w1] = t1;
       auto [from2, to2, w2] = t2;
       return w1 < w2;
    });
    ll ans = 0;
    vi id(n+1); iota(all(id), 0);
    vvi list(n+1);
    for (int i = 1; i <= n; ++i) {
        list[i] = {i};
    }
    ...
    function<void(int,int)> unite = [&](int a, int b) {
        ...
        int id2 = get(b);
        if (id1 == id2) return;
        if (isz(list[id1]) < isz(list[id2])) swap(id1,id2);
        for (int v : list[id2]) {
            list[id1].pb(v);
            id[v] = id1;
        }
