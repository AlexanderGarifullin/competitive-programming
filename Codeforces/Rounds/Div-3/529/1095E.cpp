...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (pref[i]>=0) canPref[i] = true;
        if (i) canPref[i] =canPref[i] && canPref[i-1];
    }
    int bal = 0;
    for (int i = n-1; i >=0; --i) {
        if (s[i] == '(') bal++;
        else bal--;
        if (bal <= 0) canSuf[i] = true;
        if (i + 1 < n) canSuf[i] = canSuf[i] & canSuf[i+1];
    }

    int ans = 0;
    for (int i = 0; i < n; ++i) {
        bool can = 1;
        if (i) can &= canPref[i-1];
        if (i+1<n) can &= canSuf[i+1];
        if (!can) continue;
        int bal = 0;
        if (i) bal += pref[i-1];
        bal += pref[n-1] - pref[i];
