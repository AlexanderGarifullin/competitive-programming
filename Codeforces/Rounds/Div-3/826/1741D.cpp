...
i32 main() {
    ...
    int t; cin >> t; while(t--) {
        ...
        int distinsidepair = 1;
        for (int distbetweenpair = 2; distbetweenpair <= n; distbetweenpair*=2) {
            for (int i = 0; i <= n- distbetweenpair; i+=distbetweenpair) {
                if (p[i] > p[i+distinsidepair]) {
                    for (int j = i; j < i + distinsidepair; ++j) {
                        swap(p[j], p[j+distinsidepair]);
                    }
                    res++;
                }
            }
