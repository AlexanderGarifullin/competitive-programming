...
i32 main() {
    ...
    while(t--){
        ...
        if (isz(color_it) == 1) cno;
        else {
            for (auto [c, v]: color_it) colors.pb(c);

            for (int i = 0; i < isz(colors)-1; ++i) {
                int next_color = colors[i + 1];
                for(int u : color_it[colors[i]]) {
                    g[u].pb(color_it[next_color].front());
                    g[color_it[next_color].front()].pb(u);
                }
            }
            if (isz(color_it[colors.back()]) > 1)
                for (int u : color_it[colors.back()]) {
                        if (u != color_it[colors.back()].front()) {
                            g[u].pb(color_it[colors.front()].front());
                            g[color_it[colors.front()].front()].pb(u);
                    }
                }
