...
//    freopen("output.txt", "wt", stdout);
    ...
    int t_; cin >> t_; while(t_--) {
        ...
        vector<tuple<int,int,int>> v2(n);
        vpii v(n);
        vpii pref1(n), pref2(n);
        for (int i = 0; i < n; ++i) {
            int h,w; cin >> h >> w;
            v[i] = {h,w};
            v1[i] = {h, w, i};
        ...
        for (int i = 0; i < n; ++i) {
            ...
            pref2[i] = {h2, i2};
            if (i > 0) {
                pref1[i] = min(pref1[i], pref1[i-1]);
                pref2[i] = min(pref2[i], pref2[i-1]);
            }
        }
        vi res(n,-1);
        for (int i = 0; i < n; ++i) {
            auto [hi,wi] = v[i];
            auto it = lower_bound(all(v1), tiii(hi, -1, -1));
            if (it != v1.begin()) {
                it--;
                int j = it - v1.begin();
                if (pref1[j].first < wi) res[i] = pref1[j].second + 1;
            }
            it = lower_bound(all(v2), tiii(wi, -1, -1));
            if (it != v2.begin()) {
                it--;
                int j = it - v2.begin();
                if (pref2[j].first < hi) res[i] = pref2[j].second + 1;
            }
            swap(hi, wi);
            it = lower_bound(all(v1), tiii(hi, -1, -1));
            if (it != v1.begin()) {
                it--;
                int j = it - v1.begin();
                if (pref1[j].first < wi) res[i] = pref1[j].second + 1;
