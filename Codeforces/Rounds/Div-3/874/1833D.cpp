...
i32 main() {
    ...
    int t; cin >>t; while(t--) {
        int n; cin >> n;
        vi p(n); fin(p);
        int jmax = max_element(all(p)) - p.begin();
        vi res(n,-1);
        if (jmax == 0) {
           int jsecMax=max_element(p.begin()+1,p.end()) - p.begin();
            if (jsecMax == 1) {
                for (int i = 1; i < n; ++i) {
                    res[i-1]=p[i];
                }
                ...
                for (int l = 0; l < jsecMax; ++l) {
                    vi cur;
                    for (int j = jsecMax; j < n; ++j) {
                        cur.pb(p[j]);
                    }
                    for (int j = jsecMax - 1; j >= l ; --j) {
                        cur.pb(p[j]);
                    }
                    for (int j = 0; j < l; ++j) {
                        cur.pb(p[j]);
            ...
            for (int l = 0; l < jmax; ++l) {
                vi cur;
                cur.reserve(n);
                for (int i = jmax; i < n; ++i) {
                    cur.pb(p[i]);
                }
                for (int j = jmax - 1; j >= l ; --j) {
                    cur.pb(p[j]);
                }
                for (int i = 0; i < l; ++i) {
                    cur.pb(p[i]);
