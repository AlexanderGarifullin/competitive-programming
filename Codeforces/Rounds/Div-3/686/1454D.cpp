  ...
  //  cout.setf(ios::fixed); cout.precision(20);
    ...
    int t; cin >> t; while(t--){
        ...
        if (isz(map) == 0) {
            ...
            v.reserve(isz(map));
            for (auto [del, cnt] : map) {
                v.pb({del,cnt});
            }
            sort(all(v),[](pair<ll,int> p1, pair<ll,int> p2){return p1.second > p2.second;});
            vl answ(v[0].second);
            ...
            for (int i = 1; i < isz(v); ++i) {
                int cnt = v[i].second;
                for (int j = isz(answ)-1; j > isz(answ)-cnt - 1; --j) {
                    answ[j] *= v[i].first;
                }
            }
            cout << isz(answ) << en;
