...
void multiSolve() {
    int t_; cin >> t_; while(t_--) {
        int n;cin >> n;
        vi minValue, maxValue, minSuf, maxSuf;
        minValue.pb(0);  minSuf.pb(0);
        maxValue.pb(0);  maxSuf.pb(0);

        minValue.pb(0); minSuf.pb(0);
        maxValue.pb(1); maxSuf.pb(1);
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (c == '+') {
                ...
                int curMinSuf = min(0, minSuf[u] + x);
                int curMaxSuf = max(0, maxSuf[u] + x);

                int curMinValue = min(minValue[u], curMinSuf);
                int curMaxValue = max(maxValue[u], curMaxSuf);

                minValue.pb(curMinValue);
                maxValue.pb(curMaxValue);
                ...
                int u,v; cin >> u >> v;
                int k; cin >> k;
                ifyn(minValue[v] <= k && k <= maxValue[v]);
            }
        }
    }
}
