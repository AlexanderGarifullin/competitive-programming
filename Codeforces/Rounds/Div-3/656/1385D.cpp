...
i32 main() {
    ...
    int t; cin >> t; while(t--){
        int n; cin >> n;
        str s; cin >> s;
        function<ll(int,int,int)> func = [&](int left, int right, int number){
            if (left + 1 == right) {
                return (ll)(s[left] != v[number]);
            }
            int c1 = 0;
            int c2 = 0;
            ...
            ll x1 = func((left+right)/2, right, number+1);
            ll x2 = func(left,(left+right)/2, number+1);
            return min(c1 + x1, c2 + x2);
        };
        cout << func(0, n, 0) << en;
    }
}
