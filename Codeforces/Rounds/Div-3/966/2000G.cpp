...
bool ok(int start) {
    ...
    while(!q.empty()) {
        auto [sum, v] = *q.begin();
        q.erase(q.begin());
        for (auto [next, x, y] : g[v]) {
            // хотим пройти по автобусу x
            ll nextTime = sum + x;
            // можем зайти
            if (nextTime <= t1 || sum >= t2) {
                if (nextTime < dp[next]) {
                    q.erase(make_pair(dp[next], next));
                    dp[next] = nextTime;
                    q.insert(make_pair(dp[next], next));
                }
            } else {
                nextTime = t2 + x;
                if (nextTime < dp[next]) {
