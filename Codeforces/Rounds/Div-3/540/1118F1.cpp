...
//    freopen("output.txt", "wt", stdout);
    ...
    int cnt = 0;
    vi cntRed(n+1), cntBlue(n+1);
    function<void(int,int)> dfs1 = [&](int cur, int pred) {
        if (a[cur] == 1) cntRed[cur]++;
        else if (a[cur] == 2) cntBlue[cur]++;
        for (int next: g[cur]) {
            if (next != pred) {
                dfs1(next, cur);
                cntRed[cur] += cntRed[next];
                cntBlue[cur] += cntBlue[next];
            }
        }
    };
    dfs1(1,1);
    function<void(int,int)> dfs2 =[&](int cur, int pred) {
        for (int next : g[cur]) {
            if (next != pred) {
                int pRed = cntRed[1] - cntRed[next];
                int pBlue = cntBlue[1] - cntBlue[next];
                int c = 0;
                if (pRed > 0) c++;
                if (pBlue > 0) c++;
                if (c != 2) {
