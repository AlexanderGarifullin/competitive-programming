...
ll get(vi even, vi odd, int n, bool isEvenStart) {
    ll s = 0;
    int l1 = 0, l2 = 0;
    while(l1 <=isz(even) && l2 <= isz(odd)) {
        if (isEvenStart) {
            if (l1 < isz(even)) l1++;
            else break;
        } else {
        ...
        isEvenStart = !isEvenStart;
    }
    for (int i = l1; i < isz(even); ++i) {
        s += even[i];
    }
    for (int i = l2; i < isz(odd); ++i) {
        s += odd[i];
    }
    return s;
}
...
//    freopen("output.txt", "wt", stdout);
    ...
    ll res = 1e18;
    ll r1 = get(even, odd, n, 0);
    ll r2 = get(even, odd, n, 1);
    res=min(r1,res);
    res=min(r2,res);
    cout<<res;
}
