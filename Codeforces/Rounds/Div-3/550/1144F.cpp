...
//    freopen("output.txt", "wt", stdout);
    ...
    vb output(n+1);
    vi colors(n+1);
    function<void(int,int,int)> dfs = [&](int v, int t, int pred) {
        colors[v] = 1;
        for (int nextV: g[v]) {
            if (nextV == pred) continue;
            if (colors[nextV] == 0) {
                if (t==0){
                    input[nextV] = 1;
                    output[v]=1;
                    if (e_pos.find({v,nextV}) == e_pos.end()) {
                        res[e_pos[{nextV,v}]]++;
                    }
                } else {
                    input[v] = 1;
                    output[nextV]=1;
                    if (e_pos.find({nextV,v}) == e_pos.end()) {
                        res[e_pos[{v,nextV}]]++;
                    }
                }
                dfs(nextV, 1-t, v);
            } else if (colors[nextV] == 1) {
                if (t==0){
                    input[nextV] = 1;
                    output[v]=1;
                    if (e_pos.find({v,nextV}) == e_pos.end()) {
