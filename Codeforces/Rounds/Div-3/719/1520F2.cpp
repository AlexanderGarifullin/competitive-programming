...
//    freopen("bees.out", "wt", stdout);
    ...
    for (int i = 0; i < t; ++i) {
        int kk; cin >> kk;
        function<int(int,int,int)> func = [&](int l, int r, int k) {
            int mid = (l + r)/2;
            pii range = pii(l,mid);
            int sum;
            if (cache.find(range) == cache.end()) {
                ...
                cache[range] = mid - l + 1 - sum;
            }
            int cntzero = cache[range];
            if (r == l && cntzero == k) return l;
            if (cntzero >= k) {
                return func(l, mid, k);
            } else {
                return func(mid + 1, r, k - cntzero);
            }
        };
        int res = func(1, n, kk);
        cout << "! " << res << endl;
        function<void(int,int,int)> dec = [&](int pos, int l, int r) {
            cache[{l,r}]--;
            if (l == r) return;
            int m = (l+r)/2;
            if (pos <= m) {
