...
//    freopen("output.txt", "wt", stdout);
    ...
    int t_; cin >> t_; while(t_--) {
        int n; cin >> n;
        str s; cin >> s;
        int cn = count(all(s), 'N');
        int cs = count(all(s), 'S');
        int ce = count(all(s), 'E');
        int cw = count(all(s), 'W');
        str res = string(n,'-');
        bool ok = 1;
        ...
        else if (!(cn % 2 == cs % 2 && ce % 2 == cw % 2)) {
            ok = 0;
        } else {
            if (n == 2) {
                if (cn == 1 || cs == 1 || ce == 1 || cw == 1) ok = 0;
            }
        }
        ...
        int cntN1, cntN2, cntS1, cntS2, cntE1, cntE2, cntW1, cntW2;

        cntN1 = cn / 2; cntN2 = cn - cntN1;
        cntS1 = cs /2 ; cntS2 = cs - cntS1;

         cntE2 = ce / 2; cntE1 = ce - cntE2;
         cntW2 = cw / 2; cntW1 = cw  - cntW2;
