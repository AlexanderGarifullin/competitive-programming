...
//    freopen("output.txt", "wt", stdout);
    ...
    int t_; cin >> t_; while(t_--) {
        ...
        int n = isz(days);
        ll inf = 1e18;
        vvl dp(n, vl(sum+1, inf));
        dp[0][0] = 0;
        ll res = 0;
        // сколько денег нам надо чтобы получить dp[n][s] = s счастья на n дне
        for (int i = 1; i < n; ++i) {
            auto [curCost, curHappy, day] = days[i];
            ll haveMoney = 1ll * (day-1) * x;
            for (ll wantHappy = sum; wantHappy >= 0; --wantHappy) {
                dp[i][wantHappy] = dp[i-1][wantHappy];
            }
            if (haveMoney >= curCost) {
                for (ll wantHappy = sum; wantHappy >= curHappy; --wantHappy) {
                    ll oldHappy = wantHappy - curHappy;
                    if (dp[i-1][oldHappy] != inf) {
                        ll cost = dp[i-1][oldHappy];
                        ll have = haveMoney - cost;
                        if (have >= curCost) {
                            dp[i][wantHappy] = min(cost + curCost, dp[i][wantHappy]);
                        }
                    }
                }
            }
