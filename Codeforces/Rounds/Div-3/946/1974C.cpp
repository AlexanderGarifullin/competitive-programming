...
//    freopen("output.txt", "wt", stdout);
    ...
    int t_; cin >> t_; while(t_--) {
        ...
        map<pii, map<int,int>> cnt1; map<pii, int> sum1;  // a b _
        map<pii,map<int,int>> cnt2; map<pii, int> sum2; // _ b c
        map<pii,map<int,int>> cnt3; map<pii, int> sum3; // a _ c
        ...
        for (int i = 0; i < n-2; ++i) {
            ...
            r += sum1[pii(a,b)] - cnt1[pii(a,b)][c];
            r += sum2[pii(b,c)] - cnt2[pii(b,c)][a];
            r += sum3[pii(a,c)] - cnt3[pii(a,c)][b];
