...
//    freopen("output.txt", "wt", stdout);
    ...
    int t_; cin >> t_; while(t_--) {
        ...
        vector<tuple<int,int,int,int>> minMaxCords(m+1);
        int minX=1,maxX=a,minY=1,maxY=b;
        minMaxCords[0] = {minX,maxX,minY,maxY};
        for (int i = 0; i < m; ++i) {
            ...
            minMaxCords[i+1] = {minX,maxX,minY,maxY};
        }
        function<bool(int, int, int)> ok = [&](int numOp, int x, int y) {
            auto [curMinX,curMaxX,curMinY,curMaxY] = minMaxCords[numOp];
            if (curMinX <= x && x <= curMaxX && curMinY <= y && y <= curMaxY) return true;
            return false;
        };
        vi maxCan(n);
        for (int i = 0; i < n; ++i) {
