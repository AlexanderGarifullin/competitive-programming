...
//    freopen("bees.out", "wt", stdout);
    ...
    int t; cin >> t; while(t--) {
        ...
        while(!q.empty()) {
            ...
            if (cnt % 2 == 0) {
                a[(l+r-1)/2] = i;
                if (cnt > 2) q.push(make_tuple(-((l+r-1)/2-1 - l + 1), l, (l+r-1)/2-1));
                q.push(make_tuple(-(r - ((l+r-1)/2+1) + 1), (l+r-1)/2+1, r));
            } else {
                a[(l+r)/2]=i;
                if (cnt > 1) {
                    q.push(make_tuple(-((l+r)/2-1 - l + 1), l,(l+r)/2-1));
                    q.push(make_tuple(-(r - ( (l+r)/2+1) + 1), (l+r)/2+1, r));
                }
            }
        }
        fout(a);
