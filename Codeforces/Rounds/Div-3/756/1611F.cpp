    ...
    for (int len = 0; len <= c; ++len) {
        for (int i = 0; i + (1<<len) - 1 < n; ++i) {
            if (len == 0) {
                ...
                sp[len][i] = min(sp[len-1][i], sp[len-1][i + (1<<len) - (1<<(len-1))]);
            }
        }
    }
}
...
ll get(int left, int right){
    ...
    return min(sp[t][left],sp[t][right - (1<<t)]);
}
...
//    freopen("output.txt", "wt", stdout);
    ...
    int t_; cin >> t_; while(t_--) {
        ...
        function<bool(int, int, ll)> ok = [&](int start, int len, ll minus) {
            start--;
            int end = start + len - 1;
            ll minValue = get(start, end);
            minValue -= minus;
            minValue += s;
            return minValue >= 0;
        };
        ...
        int len = -1;
        for (int i = 1; i <= n; ++i) {
            if (a[i-1] >= 0 || abs(a[i-1]) <= s) {
                ll minus = 0;
                if (i-2 >=0) minus = pref[i-2];
                int ll = 1;
                int rr = n - i + 2;
                while(ll+1<rr) {
                    int mm = (ll+rr)/2;
                    if (ok(i, mm, minus)) ll = mm;
                    else rr = mm;
                }
                if (ll > len) {
                    left = i;
