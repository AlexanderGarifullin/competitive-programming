...
i32 main() {
    ...
    int t; cin >> t; while(t--){
        ...
        function<void(int,int, int)> dfs = [&](int left, int right,int depth){
            if (left + 1 == right) {
                d[left] = depth;
                return;
            }
            int curRoot = max_element(a.begin()+left, a.begin() + right) - a.begin();
            d[curRoot] = depth;
            if (left + 1 <= curRoot) dfs(left, curRoot, depth + 1);
            if (curRoot + 1 <= right - 1) dfs(curRoot + 1, right, depth + 1);
        };
