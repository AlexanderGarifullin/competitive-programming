...
//    freopen("output.txt", "wt", stdout);
    ...
    int t_; cin >> t_; while(t_--) {
        ...
        while(q--){
            int l,r; cin >> l >> r;
            bool ok = false;
            if (b[r] == b[l-1]) ok = true;
            else if (r-l+1 > 2){
                auto itT = map[b[l-1]].lower_bound(r);
                if (itT != map[b[l-1]].end() && *itT < r || itT != map[b[l-1]].begin()) {
                    if (itT == map[b[l-1]].end() || *itT>= r) {
                        itT--;
                    }
                    auto itS = map[b[r]].lower_bound(l);
                    if (itS != map[b[r]].end()) {
                        ok = *itS < *itT;
                    }
                }
