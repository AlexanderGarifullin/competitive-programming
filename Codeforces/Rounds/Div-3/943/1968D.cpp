...
#define foutpairs(x) for (auto &it: x) cout << it.first << ' '<<it.second << en; cout << en
...
ll get(vpii & path, int k, int n) {
    ll r =0;
    int maxValue = 0;
    ll s = 0;
    for (int i = 0; i < isz(path); ++i) {
        auto [needCnt, value] = path[i];
        if (needCnt > k) break;
        s += value;
        maxValue = max(maxValue, value);
        ll cur = s + 1ll * maxValue * (k - needCnt);
        r = max(r, cur);
    }
    return r;
}
...
//    freopen("output.txt", "wt", stdout);
    ...
    int t_; cin >> t_; while(t_--) {
        int n,k, posB, posS; cin >> n >> k >> posB >> posS;
        vi p(n); fin(p);
        vi a(n); fin(a);
        vpii path1, path2;
        int pp = posB - 1;
        ...
        ll s1 = get(path1, k, n);
        ll s2 = get(path2, k, n);
        if (s1 > s2) cout << "Bodya" << en;
        else if (s1 < s2) cout << "Sasha" << en;
        else cout << "Draw" << en;
    }
}
