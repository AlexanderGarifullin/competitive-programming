...
vi get(int lenPrefix, str s) {
    s = s.substr(0,lenPrefix) + "#" + s;
    int n = isz(s);
    vi p(n);
    for (int i = 1; i < n; ++i) {
        int cur = p[i-1];
...
bool ok(int lenPrefix, str s, int cntSegments) {
    vi pref = get(lenPrefix, s);
    int c = 0;
    int last = -1;
    for (int i = lenPrefix; i < isz(pref); ++i) {
        if (pref[i] == lenPrefix) {
            if (i - last >= lenPrefix) {
                c++;
                last = i;
