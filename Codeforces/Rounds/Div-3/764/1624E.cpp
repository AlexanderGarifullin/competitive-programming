...
void solve() {
    int n,m; cin >> n >> m;
    vector<vector<tiii>> f2(10, vector<tiii>(10));
    vector<vector<vector<tiii>>> f3(10, vector<vector<tiii>>(10, vector<tiii>(10)));
    for (int i = 0; i < n; ++i) {
        str s; cin >> s;
        for (int j = 0; j < m; ++j) {
            if (j + 1 < m) {
                f2[s[j]-'0'][s[j+1]-'0'] = make_tuple(j+1, j+2, i+1);
            }
            if (j + 2 < m) {
                f3[s[j]-'0'][s[j+1]-'0'][s[j+2]-'0'] = make_tuple(j+1, j+3,i+1);
            }
        }
    }
    str s; cin >> s;
    ...
    for (int i = 0; i < m; ++i) {
        if (!dp[i]) continue;
        if (i + 2 <= m && f2[s[i]-'0'][s[i+1]-'0'] != make_tuple(0,0,0)) {
            dp[i+2] = true;
        }
        if (i + 3 <= m && f3[s[i]-'0'][s[i+1]-'0'][s[i+2]-'0'] != make_tuple(0, 0,0)) {
            dp[i+3] = true;
        }
    }
    if (!dp[m]) {
