...
bool can (int cntPlus, int cntMinus, int cntTwo) {
    int t = min(cntPlus, cntMinus);
    cntPlus -= t;cntMinus -= t;
    if (cntPlus) {
        if (cntPlus > 2 * cntTwo) return false;
        if (cntPlus % 2) {
            cntMinus++;
            cntTwo--;
        }
        cntTwo -= cntPlus/2;
        if (cntTwo < 0) return false;
        if (cntMinus) {
            if (cntTwo == 0) return false;
            cntTwo--;
        }
        ...
        if (cntMinus > cntTwo) return false;
        cntTwo -= cntMinus;
        return (cntTwo % 3 == 0);
    }
}
    ...
    int t_; cin >> t_; while(t_--) {
        ...
        for (int r = 0; r < n; ++r) {
            ...
            for (int l = r-1; l >=0; --l) {
                if (s[l] == '+') {
                    cntPlus++;
                    cntMinus += lastMinus % 2;
                    cntTwo += lastMinus/2;
                    lastMinus=0;
                } else {
                    lastMinus++;
                }
                res += can(cntPlus, cntMinus + (lastMinus%2), cntTwo + (lastMinus/2));
            }
        }
        cout << res << en;
    }
