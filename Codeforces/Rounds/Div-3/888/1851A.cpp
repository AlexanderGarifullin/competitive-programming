...
i32 main() {
    ...
    while(t--){
        ...
        vi h(n); fin(h);
        set<int>fr;
        for (int stepH = 0; stepH < m; ++stepH) {
            for (int curFriend = 0; curFriend < n; ++curFriend) {
                for (int stepFriend = 0; stepFriend < m; ++stepFriend) {
                    if (stepFriend != stepH) {
                        ll dif = abs(stepH - stepFriend) * k;
                        if (dif == abs(H - h[curFriend])) {
                            fr.insert(curFriend);
                            break;
                        }
                    }
