...
//    freopen("output.txt", "wt", stdout);
    ...
    for (int i = 1; i < isz(pref); ++i) {
        pref[i] = pref[i-1];
        if (i - 1 < isz(s) && need < isz(t) && s[i-1] == t[need]) {
            pref[i]++; need++;
        }
    }
    need = isz(t) - 1;
    for (int i = isz(s); i >= 0; --i) {
        suf[i] = suf[i+1];
        if (i -1 >= 0 && need >= 0 && s[i-1] == t[need]) {
            need--; suf[i]++;
        }
    }
    int res = 0;
    for (int r = 1; r <= isz(s); ++r) {
        int need = isz(t) - suf[r+1];
        auto it = lower_bound(pref.begin(), pref.begin()+r, need);
        int j = it - pref.begin();
        res=max(res, r - j);
    }
    cout << res << en;
