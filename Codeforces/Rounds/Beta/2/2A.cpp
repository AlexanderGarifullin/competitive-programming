...
i32 main() {
    ...
    ll m = -1e18;
    int n; cin >> n;
    map<str, ll> finishPoints, curPoints;
    vs names(n);
    vl points(n);
    for (int i = 0; i < n; ++i) {
        str s; ll add; cin >> s >> add;
        finishPoints[s] += add;
        names[i] = s;
        points[i] = add;
    }
    for (auto [str, point] : finishPoints) m = max(m, point);
    for (int i = 0; i < n; ++i) {
        curPoints[names[i]] += points[i];
        if (curPoints[names[i]] >= m && finishPoints[names[i]] == m) {
            cout << names[i];
            return 0;
        }
    }
