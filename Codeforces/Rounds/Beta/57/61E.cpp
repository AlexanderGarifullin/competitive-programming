...
int get(int v, int ls, int rs, int left, int right) {
    if (rs <= left || ls >= right) return 0;
    if (left >= ls) {
        return tr[v];
    }
    int mid = (left+right)/2;
    ...
    int rightV = 2*v+1;
    int value = 0;
    if (ls < mid) value += get(leftV, ls, rs, left, mid);
    if (mid < rs) value += get(rightV, ls, rs, mid, right);
    return value;
}

int get2(int v, int ls, int rs, int left, int right) {
    if (rs <= left || ls >= right) return 0;
    if (right <= rs) {
        return tr[v];
    }
    ...
    int rightV = 2*v+1;
    int value = 0;
    if (ls < mid) value += get2(leftV, ls, rs, left, mid);
    if (mid < rs) value += get2(rightV, ls, rs, mid, right);
    return value;
}

void update(int v, int pos, int left, int right) {
    if (left + 1 == right) {
        tr[v]++;
        return;
    }
    ...
    int rightV = 2*v+1;
    if (pos < mid) {
        update(leftV, pos, left, mid);
    } else {
        update(rightV, pos, mid, right);
    }
    tr[v] = tr[leftV] + tr[rightV];
}
...
void CBO(){
    ...
    tr = vi(4*n);
    for (int i = 0; i < n; ++i) {
        gr[i] = get(1, a[i]+1, n+1, 1, n+1);
        update(1, a[i], 1, n+1);
    }

    tr = vi(4*n);
    for (int i = n-1; i >=0 ; --i) {
        ls[i] = get2(1, 1, a[i], 1, n+1);
        update(1, a[i], 1, n+1);
    }
    ll res = 0;
    for (int i = 0; i < n; ++i) {
