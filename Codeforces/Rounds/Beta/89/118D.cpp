...
int get(int curN1, int curN2, int type) {
    if (type == 2) {
        // ans
        int res = 0;
        for (int i = 0; i <= min(curN1-1, k1-1); ++i) {
            res += get(curN1-1-i, curN2, 1);
            res%=mod;
        }
        for (int i = 0; i <= min(curN2-1, k2-1); ++i) {
            res += get(curN1, curN2-1-i, 0);
            res %=mod;
        }
        return res % mod;
    } else  {
        // 0 1
        int key = curN1 * prime * prime * prime + curN2 * prime * prime + type * prime;
        if (cache.find(key) != cache.end()) return cache[key];
        int res = 0;
        if (curN1 == 0 && curN2 == 0 || curN1 == 0 && curN2 > k2 || curN2 == 0 && curN1 > k1) {
            res = 0;
        }
        else if (curN1 == 1 && curN2 == 0 || curN1 == 0 && curN2 == 1)  {
            res = 1;
        }
        else if (type == 1) {
            // start with 2;
