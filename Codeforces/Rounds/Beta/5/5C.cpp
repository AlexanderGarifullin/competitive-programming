...
//    freopen("output.txt", "wt", stdout);
    ...
    st.push({-1e9, 0});
    st.push({0, -1});
    int bestLen=0, cntLen = 1;
    int n =isz(s);
    int bal=0;
    for (int i = 0; i < n; ++i) {
        if (s[i] == '(') bal++;
        else bal --;
        while(st.top().first > bal) {
            st.pop();
        }
        if (st.top().first == bal) {
            if (i - st.top().second > bestLen) {
                bestLen = i - st.top().second;
                cntLen = 1;
            }
            else if (i - st.top().second == bestLen) cntLen++;
         }
        if (bal > st.top().first) st.push({bal, i});
    }
    cout << bestLen << ' ' << cntLen;
}
