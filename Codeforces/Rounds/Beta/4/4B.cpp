...
i32 main() {
    ...
    int d, sum, mintime,maxtime; cin >> d >> sum ;
    int mins =0,maxs = 0;
    vpii times(d);
    for (int i = 0; i < d; ++i) {
    ...
    if (!(mins <= sum && sum <= maxs)) cno;
    else {
        vi answ(d);
        for (int i = 0; i < d; ++i) {
            answ[i] = times[i].first;
            sum -= answ[i];
        }
        for (int i = 0; i < d && sum > 0; ++i) {
            answ[i] += min(times[i].second - times[i].first, sum);
            sum -= min(times[i].second - times[i].first, sum);
        }
        cyes;
        fout(answ);
    }
