...
vi slow(int n, int x, vi a, vi b) {
    sort(all(b));
    do {
        if (func(a, b) == x) return b;
    } while(next_permutation(all(b)));
    return {};
}

vi smart(int n, int x,  vi a, vi b) {
    vpii aa(n);
    vi bb = b;
    for (int i = 0; i < n; ++i) {
        aa[i]={a[i],i};
    ...
    sort(all(bb));
    for (int i = 0; i < x; ++i) {
        b[aa[x-i-1].second]=bb[i];
    }
    for (int i = x; i < n; ++i) {
        b[aa[n-(i-x)-1].second]=bb[i];
    }
    if (x == func(a, b)) {
        return b;
    }
...
void stress(){
    ...
    for (int n = 1; n <= 5; ++n) {
        for (int x = 0; x <= n; ++x) {
            ...
            vi v1 = smart(n,x,a,b);
            vi v2 = slow(n, x, a, b);
            if (func(a,v1) != func(a,v2))  {
                cout << "n = " << n <<  " WA" << en;
                fout(a);
                fout(b);
                cout << x << ' ' << en << string(20,'-') << en;
                fout(smart(n,x,a,b));
                fout(slow(n, x, a, b));
                exit(0);
            }
