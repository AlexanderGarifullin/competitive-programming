...
i32 main() {
    ...
    int t; cin >> t; while(t--) {
        ...
        set<int> st;
        vpii color_index;
        color_index.reserve(n);
        for (int i = 1; i <= n; ++i) {
            cin >> a[i];
            st.insert(i);
            color_index.pb({a[i], i});
        }
        sort(all(color_index), [](pii p1, pii p2){
            return p1.first < p2.first;
        });
        for (int i = 0; i < isz(color_index); ++i) {
            auto [color, index] = color_index[i];
            if (ans[color] != 0) {
                st.erase(index);
                continue;
            }
            auto itStart = st.begin();
            auto itLast = st.end();
            itLast--;
            int firstFree = index - *itStart;
            int lastFree = *itLast - index;
            ans[color] = 2 + 2 * firstFree + lastFree * 2;
            st.erase(index);
        }
