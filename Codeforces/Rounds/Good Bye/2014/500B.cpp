...
//    freopen("output.txt", "wt", stdout);
    ...
    map<int,vi> ind_value;
    map<int, vi> ind_indexes;
    for (int i = 1; i <= n; ++i) {
        if (id[i] == -1) {
            v.clear();
            ind.clear();
            id[i] = idd++;
            dfs(i);
            sort(all(v)); sort(all(ind));
            ind_indexes[id[i]] = ind;
            ind_value[id[i]] = v;
        }
    }

    for (auto [curId, values] : ind_value) {
        for (int i = 0; i < isz(values); ++i) {
            p[ind_indexes[curId][i]] = values[i];
        }
    }
