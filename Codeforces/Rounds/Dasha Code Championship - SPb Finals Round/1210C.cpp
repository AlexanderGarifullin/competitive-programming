...
void solve() {
    ...
    ll ans =0;
    function<void(int,int, vpll)> dfs = [&](int v, int p, vpll predNumbers) {
        for (pll & pp : predNumbers) {
            pp.first++; pp.second = __gcd(pp.second, a[v]);
        }
        predNumbers.emplace_back(0, a[v]);
        vpll nw;
        for (pll pp : predNumbers) {
            if (!nw.empty() && nw.back().second == pp.second) continue;
            nw.pb(pp);
        }
        int ls = -1;
        for (int i = isz(nw) - 1; i >= 0; i--) {
            ans += (nw[i].first - ls) * nw[i].second;
            ans %= mod;
            ls = nw[i].first;
        }
        for (int next : g[v]) {
            if (next != p) dfs(next, v, nw);
        }
    };
    dfs(0,-1,{});
    cout << ans << en;
