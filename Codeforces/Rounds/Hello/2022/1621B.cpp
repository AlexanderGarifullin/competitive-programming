...
i32 main() {
    ...
    int t; cin >> t; while(t--) {
        int n; cin >> n;
        int mostleft = 2e9, leftcost = 2e9;
        int mostright = 0, rightcost = 2e9;
        int cnt = 0, cost = 2e9;
        for (int i = 0; i < n; ++i) {
            int l,r,c; cin >> l >> r >> c;
            if (l < mostleft || l == mostleft && c < leftcost) {
                mostleft = l;
                leftcost = c;
            }
            if (r > mostright || r == mostright && c < rightcost) {
                mostright = r;
                rightcost = c;
            }
            if (r - l + 1 > cnt) {
                cnt = r - l + 1;
                cost = c;
            } else if (r - l + 1  == cnt && c < cost) {
                cost = c; cnt = r - l + 1;
            }
            if (mostright - mostleft + 1 > cnt) {
                cout << leftcost + rightcost << en;
            } else cout << min(leftcost + rightcost, cost) << en;
        }
    }
}
