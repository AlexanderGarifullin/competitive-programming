...
i32 main() {
    ...
    int t; cin >> t; while(t--) {
        ...
        pqmaximumtstart<pii> pq;
        for (pii p : leaves) {
            pq.push(p);
        }
        while(!pq.empty()) {
            auto [curDeep, curV] = pq.top();
            pq.pop();
            if (use[curV]) continue;
            use[curV] = true;
            if (s[curV] == 'W') cntwhite[curV]++;
            else cntblack[curV]++;
            if (cntwhite[curV] == cntblack[curV]) dp[curV]++;
            if (pred[curV] != -1) {
                dp[pred[curV]] += dp[curV];
                cntblack[pred[curV]] += cntblack[curV];
                cntwhite[pred[curV]] += cntwhite[curV];
                pq.push({curDeep-1, pred[curV]});
            }
        }
