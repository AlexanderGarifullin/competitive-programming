...
i32 main() {
    ...
    int t; cin >> t; while(t--) {
        ...
        index.pb(start);
        int finish =  n- 1;
        while(finish >= 0 && s[finish] == 'B') finish--;
        if (finish != n - 1) finish = finish + 1;

        for (int i = start + 1; i < finish; ++i) {
            if (s[i] == 'B') {
                int cnt = 1;
                int j = i + 1;
                while(j < finish && s[j] == 'B') {
                    cnt++;
                    j++;
                }
                if (cnt > 1) {
       ...
       //  fout(index);
        for (int i = 0; i < isz(index); i+=2) {
            int left = index[i];
            int right = index[i+1];
            int cur = 0;
            vi z;
            if (s[left] == 'B' || s[right] == 'B') z.pb(0);
            int cntA = 0;
