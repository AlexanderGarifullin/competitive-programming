    ...
    int t; cin >> t; while(t--) {
        ...
        while(!q.empty()) {
            ...
            for (int nextV : g[curV]) {
                if (nextV != a && dist1[nextV] == 0) {
                    q.push(nextV);
                    dist1[nextV] = dist1[curV] + 1;
                }
            }
        }
        q.push(b);
        while(!q.empty()) {
            ...
            q.pop();
            for (int nextV : g[curV]) {
                if (nextV != b && dist2[nextV] == 0) {
                    q.push(nextV);
                    dist2[nextV] = dist2[curV] + 1;
                }
            }
        }
        ...
        int needVertex=-1;
        vi color(n+1);
        function<void(int,int)> dfs = [&](int curV, int predV) {
            color[curV] = 1;
            for (int nextV: g[curV]) {
                if (nextV == predV) continue;
                if (color[nextV] == 0) dfs(nextV, curV);
                else if (color[nextV] == 1) {
                    if (needVertex == -1) needVertex = nextV;
                    return;
                }
            }
            color[curV] = 2;
        ...
        dfs(b,b);
        ifyn(dist2[needVertex] < dist1[needVertex]);
    }
}
