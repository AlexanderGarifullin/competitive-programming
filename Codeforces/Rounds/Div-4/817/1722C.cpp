...
i32 main() {
    ...
    while(t--){
        ...
        ll fir = 0, sec = 0, thir = 0;

        for (auto word : st1) {
            if (st2.find(word) == st2.end() && st3.find(word) == st3.end()){
                fir += 3;
            }
            else if (st2.find(word) != st2.end() && st3.find(word) != st3.end()) {
                st2.erase(word);
                st3.erase(word);
            }
            else if (st2.find(word) != st2.end()) {
                st2.erase(word);
                fir++;
                sec++;
            }
