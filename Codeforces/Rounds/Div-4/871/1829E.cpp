    ...
    while(t--) {
     int n, m; cin >> n >> m;
        ...
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < m; ++j) {
                cin >> g[i][j];
                if (g[i][j] == 0) used[i][j] = true;
            }
        }
        vl components;

        function<void(int,int)> bfs =[&](int x, int y) {
            components.back() += g[x][y];
            used[x][y] = true;
            for (int i = -1; i <= 1; ++i) {
                for (int j = -1; j <= 1; ++j) {
                    if (i *i + j * j == 1) {
                        int dx = x + i;
                        int dy = y + j;
                        if ( dx < 0 || dy < 0 || dx >= n || dy >= m || used[dx][dy]) continue;
                        bfs(dx,dy);
                    }
                }
            }
        ...
        if (isz(components) == 0) cout << 0 << en;
        else
        cout << *max_element(all(components)) << en;
    }
