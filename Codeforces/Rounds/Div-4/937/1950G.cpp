...
void solve() {
    ...
    vi na(n), nb(n);
    for (int i = 0; i < n; ++i) {
        na[i] = lower_bound(all(values), aa[i]) - values.begin();
        nb[i] = lower_bound(all(values), bb[i]) - values.begin();
    }
    int maxSize = 0;
    vvi dp(1<<n, vi(n));
    for (int i = 0; i < n; ++i) {
