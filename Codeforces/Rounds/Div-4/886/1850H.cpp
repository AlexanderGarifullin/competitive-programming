...
i32 main() {
    ...
    while(t--){
        ...
        function<void(int)> dfs =[&](int curSoldier){
            visited[curSoldier] = true;
            for (auto [next, dist] : g[curSoldier]) {
                if (!visited[next]) {
                    soldiers[next] = soldiers[curSoldier] + dist;
                    dfs(next);
                }
            }
        };
        ...
        bool ok = 1;
        for (int i = 0; i < m; ++i) {
            if (soldiers[conditions[i][1]] - soldiers[conditions[i][0]] != conditions[i][2]){
                ok = 0;
                break;
            }
        }
