...
ll add(ll a, ll b, ll c, ll x, ll y, ll z) {
    ll r = 0;
    vl v = {a,b,c};
    vl v2 {x,y,z};
    vi pos = {0,1,2};
    do {
        ll c1 = max(0ll, v[pos[0]] - x + 1);
        ll c2 = max(0ll, v[pos[1]] - y + 1);
        ll c3 = max(0ll, v[pos[2]] - z + 1);
        ll cur = c1 * c2 * c3;
//        if (cur == 6) {
//            cout << x << ' ' << y << ' ' << z << en;
//            cout <<  v[pos[0]] << ' ' << v[pos[1]] << ' ' << v[pos[2]] << en;
//            nline;
//        }
        r =max(r, cur);
...
ll solve(ll a, ll b, ll c, ll k) {
    ...
    for (ll i = 1; i <= len[2]; ++i) {
        if (k % i) continue;
        for (ll j = 1; j <= len[2]; ++j) {
            if (k % j) continue;
            ll z = k / i / j;
            if (z <= 0 || z > len[2] || i * j * z != k) continue;
            r = max(r, add(a,b,c,i,j,z));
        }
    }
    return r;
}
