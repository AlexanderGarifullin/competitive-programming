...
void solve() {
    str s; cin >> s;
    int n = isz(s);
    function<bool(int,int)> is_palindrome = [&](int l, int r) {
    ...
    if (!is_palindrome(0, first) && !is_palindrome(first+1, n-1)) {
        cyes;
        cout << 2 << en << s.substr(0,first+1) << ' ' << s.substr(first+1)<<  en;
    } else if (first == 1 || first == n/2) {
        cno;
    } else {
        cyes;
        cout << 2 << en << s.substr(0, first + 2) << ' ' << s.substr(first + 2) << en;
    }
