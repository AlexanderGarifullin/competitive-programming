...
ll solve(int n, vl a) {
    ...
    vvl cntVariantov(2, vl(n));
    dp[0][0] = abs(a[0]);
    dp[1][0] = a[0];
    cntVariantov[0][0] = cntVariantov[1][0] = 1;
    if (a[0] >= 0) cntVariantov[0][0] = cntVariantov[1][0] = 2;
    ...
    for (int i = 1; i < n; ++i) {
        ...
        vpll v1 = {
                {dp[0][i-1] + a[i], cntVariantov[0][i-1]},
                {abs(dp[0][i-1] + a[i]),  cntVariantov[0][i-1]},
        };
        ...
        if (dp[0][i-1]  != dp[1][i-1] ) {
            v1.pb( {dp[1][i-1] + a[i],  cntVariantov[1][i-1]});
            v1.pb( {abs(dp[1][i-1] + a[i]),  cntVariantov[1][i-1] });
            v2.pb(   {dp[1][i-1] + a[i], cntVariantov[1][i-1]});
            v2.pb(     { abs(dp[1][i-1] + a[i]), cntVariantov[1][i-1]});
        }

        for (auto [v, c] : v1) {
            if (dp[0][i] == v) {
                cntVariantov[0][i] += c;
                cntVariantov[0][i] %= mod;
            }
        }
        ...
        for (auto [v, c] : v2) {
            if (dp[1][i] == v) {
                cntVariantov[1][i] += c;
                cntVariantov[1][i] %= mod;
            }
        }
    }
    return cntVariantov[0][n-1] % mod;
}


ll solveDp(int n, vl a) {
    vvl dp(2, vl(n));
    dp[0][0] = abs(a[0]);
    dp[1][0] = a[0];
    for (int i = 1; i < n; ++i) {
...
void dfs(int pos, int n, vl & a, ll c) {
    if (pos + 1 == n) {
        res = max(res, max(c + a[pos], abs(c + a[pos])));
        return;
    } else {
        dfs(pos + 1, n, a, c + a[pos]);
        dfs(pos + 1, n, a, abs(c + a[pos]));
    }
}

void dfs2(int pos, int n, vl & a, ll c, str s) {
    if (pos + 1 == n) {
        ll r1 = c + a[pos];
        ll r2 = abs(c + a[pos]);
        if (r1 == res) {
        ...
        dfs2(pos + 1, n, a, c + a[pos], s + "0");
        dfs2(pos + 1, n, a, abs(c + a[pos]), s + "1");
    }
}
