...
void svo(){
    ...
    for (int i = 0; i < n; ++i) {
        int a,b; cin >> a >> b;
        if (a>b) v1.pb(make_tuple(a,b,i+1));
        else v2.pb(make_tuple(a,b,i+1));
    }
    sort(all(v1),[](tuple<int,int,int> t1, tuple<int,int,int> t2) {
        auto [a1,b1,i1] = t1;
        auto [a2,b2,i2] = t2;
        return b1 < b2;
    });
    sort(all(v2),[](tuple<int,int,int> t1, tuple<int,int,int> t2) {
        auto [a1,b1,i1] = t1;
        auto [a2,b2,i2] = t2;
        return b1 > b2;
    });
