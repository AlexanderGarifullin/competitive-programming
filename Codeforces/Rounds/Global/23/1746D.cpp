...
ll DP(int v, int k) {
    if (mem.contains(make_pair(v,k))) return mem[make_pair(v,k)];
    ll ans = 1ll * k * s[v];
    ll cntChild = isz(g[v]);
    if (cntChild == 0) return ans;
    if (k % cntChild == 0) {
        for (int next : g[v]) {
            ans += DP(next, k / cntChild);
        }
    } else {
        vl dp1(cntChild);
        vl dp2(cntChild);
        ...
        for (int i = 0; i < cntChild; ++i) {
            dp1[i] = DP(g[v][i], k / cntChild);
            dp2[i] = DP(g[v][i], k / cntChild + 1);
            diff[i] = dp2[i] - dp1[i];
        }
        for (int i = 0; i < cntChild; ++i) {
            ans += dp1[i];
        ...
        sort(rall(diff));
        for (int i = 0; i < k % cntChild; ++i) {
            ans += diff[i];
        }
    }
    mem[make_pair(v,k)] = ans;
    return ans;
}
