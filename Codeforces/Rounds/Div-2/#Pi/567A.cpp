...
//      freopen("output.txt", "wt", stdout);
   ...
   vi v(n); fin(v);
    for (int i = 0; i < n; ++i) {
        int minLeftNeigbor = 3e9 ;
        int minRightNeigbor = 3e9;
        if (i + 1 < n) minRightNeigbor = v[i + 1] - v[i];
        if (i - 1 >= 0) minLeftNeigbor = v[i] - v[i - 1];
        int mmax = v.back() - v[i];
        mmax = max(mmax, v[i] - v[0]);
        cout << min(minLeftNeigbor, minRightNeigbor) <<' ' << mmax << en;
    }
}
 
