...
i32 main() {
    ...
    int m, s; cin >> m >> s;
    // max number where m digits and sum = s
    // min number where m digits and sum = s
    if (s == 0) {
        if (m == 1) cout << 0 << ' ' << 0;
        else cout << -1 << ' ' << -1;
        return 0;
    ...
    bool ok = true;
    //
    str minnumber = string(m, '0');
    str maxnumber = string(m, '0');

    minnumber[0] = '1';
    int summinnumber = 1;
    int j = m - 1;
    while(summinnumber != s) {
        if (j > 0) {
            int minus = -1;
            char c;
            for (int i = 1; i < 10 ; ++i) {
    ...
    for (int i = 0; i < m; ++i) {
        ...
        summaxnumber += add;
        maxnumber[i] = c;
        if (summaxnumber == s) break;
    }
    if (summaxnumber != s) ok = false;
    if (ok) {
