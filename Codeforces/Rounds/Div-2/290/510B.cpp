...
i32 main() {
    ...
    function<bool(int,int,int,int)> dfs = [&](int curX, int curY, int predX, int predY){
        bool have = false;
        use[curX][curY] = true;
       // cout << curX << ' ' << curY << en;
        for (int dx = -1; dx <= 1; ++dx) {
            for (int dy = -1; dy <=1 ; ++dy) {
                if (dx*dx+dy*dy==1) {
                    int nextX = curX + dx;
                    int nextY = curY + dy;
                    if (predX == nextX && predY == nextY) continue;
                    if (0 <= nextX && nextX < n && 0 <= nextY && nextY < m) {
                        if (field[curX][curY] != field[nextX][nextY]) continue;
                        if (use[nextX][nextY]) {
                            return true;
                        }
                        have |= dfs(nextX,nextY,curX,curY);
                    }
                }
            }
        }
