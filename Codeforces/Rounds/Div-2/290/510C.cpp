...
void solve(int n, vs v) {
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < i; ++j) {
            str pref = v[j]; str word = v[i];
            int pos = 0;
            while(pos < min(isz(word), isz(pref)) && word[pos]==pref[pos]) pos++;
            if (pos == min(isz(word), isz(pref))) {
                if (isz(pref) > isz(word)) {
                    cout << "Impossible" << en;
                    return;
                }
            }
    ...
    vvb g(26,vb(26));
    for (int i = 0; i < n-1; ++i) {
        str pref = v[i]; str word = v[i+1];
        int pos = 0;
        while(pos < min(isz(word), isz(pref)) && word[pos]==pref[pos]) pos++;
        if (pos != min(isz(word), isz(pref))) {
            g[pref[pos]-'a'][word[pos]-'a']=1;
        }
    }
    vb used(26);
    bool ok = 1;
