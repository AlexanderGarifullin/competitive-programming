...
void CBO() {
    int r,s,p; cin >> r >> s >> p;
    vector<vector<vector<long double>>> dp(r+1,vector<vector<ld>>(s+1,vector<ld>(p+1, 0)));
    dp[r][s][p] =1;
    ...
    for (int sum = r + s + p; sum > 0 ; --sum) {
        for (int cr = r; cr >=0; --cr) {
            for (int cs = s; cs >=0; --cs) {
                ...
                int wp = cp * cr;
                int tot = wr + ws + wp;
                if (cr > 0) dp[cr-1][cs][cp] += cur * wp / tot;
                if (cs > 0) dp[cr][cs - 1][cp] += cur * wr / tot;
                if (cp > 0) dp[cr][cs][cp - 1] += cur * ws / tot;
            }
        }
    }
