...
i32 main() {
    ...
    s1 = accumulate(all(v1),0ll);
    s2 = accumulate(all(v2),0ll);
    if (s1 > s2) cout << fir << en;
    else if (s1 < s2) cout << sec << en;
    else {
        bool eq = true;
        for (int i = 0; i < min(isz(v1), isz(v2)); ++i) {
            if (v1[i] < v2[i]) {
                eq = false;
                cout << sec << en;
                break;
