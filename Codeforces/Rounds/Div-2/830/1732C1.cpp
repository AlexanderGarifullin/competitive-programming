...
ll get(int lpos, int rpos, vl &prefSum, vl &prefXor) {
    ll sum = prefSum[rpos];
    if (lpos > 0) {
        sum -= prefSum[lpos - 1];
    }
...
pii solve(int n, int q,vi a,int l,int r) {
    vl prefSum(n);
    vl prefXor(n);
    for (int i = 0; i < n; ++i) {
        prefSum[i] = prefXor[i] = a[i];
        if (i>0) {
    ...
    while(q--) {
        ...
        r--;
        p = make_pair(l, r);
        ll val = get(l, r, prefSum, prefXor);
        int right = l;
        for (int i = l; i <= r; ++i) {
            ll cur = get(i, right, prefSum, prefXor);
            while(right < i || (right  + 1<= r && cur < val)) {
                right++;
                cur = get(i, right, prefSum, prefXor);
            }
            if (get(i, right, prefSum, prefXor) == val &&
            p.second - p.first > right - i) {
                p = make_pair(i, right);
            }
        }
    }
...
pii slow(int n, int q,vi a,int l,int r) {
    l--; r--;
    pii p = {l,r};
    ll s = 0;
    ll x =0;
    ...
    for (int left = l; left <= r; ++left) {
        for (int right = left; right <= r; ++right) {
            ...
            ll res = s - x;
            if (res >= ans) {
                if (right - left + 1 <= p.second - p.first + 1) {
...
void test() {
    ...
    while(q--) {
        int l,r; cin >> l >> r;
        pii p = solve(n,1,a,l,r);
        cout << p.first + 1<< ' ' << p.second + 1<< en;
    }
}
...
void brute() {
    for (int n = 1; n <= 10; ++n) {
        for (int i = 0; i < 10; ++i) {
            ...
            for (int l = 1; l <= n; ++l) {
                for (int r = l; r <= n; ++r) {
                    pii my = solve(n,q,a,l,r);
                    pii sl = slow(n,q,a,l,r);
                    ll sum1 = 0;
                    ll sum2 =0;
                    ll x1 = 0;
                    ll x2 = 0;
                    ll v1 = 0;
                    ll v2 = 0;
                    for (int j = my.first ; j <= my.second ; ++j) {
                        sum1 += a[j];
                        x1 ^= a[j];
                    }
                    for (int j = sl.first ; j <= sl.second ; ++j) {
                        sum2 += a[j];
                        x2 ^= a[j];
                    }
                    ...
                    int len1 = my.second - my.first + 1;
                    int len2 = sl.second - sl.first + 1;
                    ...
                    if (v1 < v2 || len2 < len1) {
                        ...
                        cout << l << ' ' << r << en;
                        cout << string(20,'-') << en;
                        cout << sl.first  + 1<< ' ' << sl.second  + 1<< en;
                        cout << v2 << ' ' << len2 << en;
                        cout << string(20,'-') << en;
                        cout << my.first + 1 << ' ' << my.second  + 1<< en;
                        cout << v1 << ' ' << len1 << en;
                        return;
                    }
                }
