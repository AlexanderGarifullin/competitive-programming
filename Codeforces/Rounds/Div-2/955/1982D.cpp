...
void multiSolve() {
    int t_; cin >> t_; while(t_--) {
        ...
        vvb isSnow(n+1, vb(m+1));
        for (int i = 1; i <= n; ++i) {
            str s; cin >> s;
            for (int j = 0; j < m; ++j) {
                isSnow[i][j+1] = s[j] == '0';
                if (isSnow[i][j+1]) sumSnow += a[i][j+1];
                else sumNotSnow += a[i][j+1];
            }
        }

        if (sumSnow == sumNotSnow) {
            cyes;
            continue;
        }

        vvi cntSnow(n+1, vi (m+1));
        for (int i = 1; i <= n; ++i) {
            for (int j = 1; j <= m; ++j) {
                cntSnow[i][j] = isSnow[i][j];
                cntSnow[i][j] += cntSnow[i-1][j] + cntSnow[i][j-1] - cntSnow[i-1][j-1];
            }
        }
        ll g =0;
        ...
        for (int i = k; i <= n; ++i) {
            for (int j = k; j <= m; ++j) {
                int x = cntSnow[i][j] - cntSnow[i-k][j] - cntSnow[i][j-k] + cntSnow[i-k][j-k];
                int y = k * k - x;
                g = __gcd(g, 1ll * abs(x-y));
            }
        }
        ll diff = abs(sumSnow - sumNotSnow);
        if (g == 0) {
            cno;
            continue;
        }
