...
//    freopen("output.txt", "wt", stdout);
    ...
    str s; cin >> s;
    int res = 0;
    function<int(char,char)> get = [](char x, char y) {
        int xx = x -'a';
        int yy = y - 'a';
        if (yy < xx) swap(xx, yy);
        int r = yy - xx;
    ...
    int add = 1e9;
    int leftClose = -1, rightClose = -1, leftFar = -1, rightFar= -1;
    if (pos < n/2) {
        for (int i = pos+1; i < n/2; ++i) {
            if (s[i] != s[n-1-i]) {
                if (rightClose == -1) rightClose = i;
                rightFar = i;
            }
        }
        for (int i = pos-1; i >= 0; --i) {
            if (s[i] != s[n-1-i]) {
                if (leftClose == -1) leftClose = i;
                leftFar = i;
            }
        }
        if (leftClose == -1 && rightClose == -1) {
            add = 0;
        } else {
            if (leftFar != -1) {
                add = min(add, n -1 - leftFar - pos);
            } else {
                add = min(add, rightFar - pos);
            }
            if (rightFar != -1) {
                add = min(add, pos + rightFar + 1);
            } else {
                add = min(add, pos - leftFar);
            }
            if (rightFar != -1 && leftFar != -1) {
                add = min(add, min(pos - leftFar, rightFar - pos) + rightFar - leftFar);
            }
        }
    } else {
        ...
        if (leftClose == -1 && rightClose == -1) {
            ...
            if (leftFar != -1) {
                add = min(add, n - pos + n -1 - leftFar);
            } else {
               add = min(add, rightFar - pos);
            }

            if (rightFar != -1) {
                add = min(add, pos - (n-1-rightFar));
            } else {
               add = min(add, pos - leftFar);
            }
