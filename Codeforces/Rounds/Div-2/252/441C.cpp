...
i32 main() {
    ...
    int already = 0;
    vvpii res;
    for (int i = 0; i < n && already < k-1; ++i) {
        for (int j = 0; j < m && already < k-1; ++j) {
            if (!use[i][j]) {
                if (j+1<m && !use[i][j+1]) {
                    already++;
                    use[i][j] = use[i][j+1] = true;
                    vpii vv = {{i+1,j+1}, {i+1,j+2}};
                    field[i][j] = field[i][j+1] = ++comp;
                    res.pb(vv);
                }
                if (!use[i][j] && already < k && i + 1 < n && !use[i+1][j]){
                    already++;
                    use[i][j] = use[i+1][j] = true;
                    field[i][j] = field[i+1][j] = ++comp;
                    vpii vv = {{i+1,j+1}, {i+2,j+1}};
                    res.pb(vv);
                }
            }
        }
