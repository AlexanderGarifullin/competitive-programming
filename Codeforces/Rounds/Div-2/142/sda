...
void fill_fact(vector<unsigned long long> &fact){
    fact[0] = 1;
    for (long long i = 1; i < isz(fact) ; ++i) {
        fact[i] = fact[i -1] * i;
    }
}

void fill_c(vector<vector<unsigned long long>> &C, vector<unsigned long long> &fact, int n){
    for (int i = 0; i <= n; ++i) {
        for (int j = 0; j <= i; ++j) {
...
//    freopen("comb.out", "wt", stdout);
    ...
    vpii v(n);
    for (int i = 0; i < n; ++i) {
        cin >> v[i].f >> v[i].second;
    }
    sort(all(v),[](pii p1, pii p2){return p1.first < p2.first;});
    for (int i = 0; i < n; ++i) {
        if (s <= v[i].first) {
            s = -1;
            break;
