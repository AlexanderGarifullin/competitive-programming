   ...
   ios_base::sync_with_stdio(false); cin.tie(nullptr); cout.tie(nullptr);
    ...
    int t; cin >> t; while(t--) {
        ...
        for (int i = 0; i < k / 2; ++i) {
            int add = 1e9;
            for (int j = 0; j < 26; ++j) {
                if (index_cnt[i][j] == 0) continue;
                for (int l = 0; l < 26; ++l) {
                    if (index_cnt[k-i-1][l] == 0) continue;
                    int cur;
                    if (j == l) {
                        cur = 2*p - index_cnt[i][j] - index_cnt[k-1-i][l];
                    } else {
                        cur = 2 * p - max(index_cnt[i][j], index_cnt[k-1-i][l]);
                    }
                    add = min(add, cur);
                }
            }
        ...
        if (k & 1) {
            int add = 1e9;
            for (int i = 0; i < 26; ++i) {
                if (index_cnt[k/2][i] == 0) continue;
                add = min(add, p - index_cnt[k/2][i]);
            }
            res += add;
        }
        cout << res << en;
