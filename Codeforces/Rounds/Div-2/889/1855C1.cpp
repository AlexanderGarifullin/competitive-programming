  ...
  //    freopen("choose.out", "wt", stdout);
    ...
    while(t--){
        ...
        if (*mmax > 0) {
            // 9
            for (int i = 0; i < 9; ++i) {
                a[mmax - a.begin()] += a[mmax - a.begin()];
                op.push_back({mmax - a.begin() + 1, mmax - a.begin() + 1});
            }
            // 40
            int predJ = mmax - a.begin();
            for (int i = 0; i < n; ++i) {
                op.push_back({i + 1, predJ + 1});
                a[i] += a[predJ];
                op.push_back({i + 1, predJ + 1});
                a[i] += a[predJ];
                predJ = i;
            ...
            cout << isz(op) << en;
            for (const auto &[l, r]: op) cout << l << ' ' << r << en;
        }
        else {
            for (int i = 0; i < n; ++i) {
                a[i] = -a[i];
            }
            mmax = max_element(all(a));
            // 9
            for (int i = 0; i < 9; ++i) {
                a[mmax - a.begin()] += a[mmax - a.begin()];
                op.push_back({mmax - a.begin() + 1, mmax - a.begin() + 1});
