...
i32 main() {
    ...
    bool ok = true;
    set<int> under_a_upper_b;
    set<int> under_a_upper_a;
    set<int> under_b_upper_a;
    set<int> under_b_upper_b;
    for (int i = 0; i < n; ++i) {
        if (s[i] == 'a') {
            if (t[i] == 'a') {
                under_a_upper_a.ins(i);
            } else {
                under_b_upper_a.ins(i);
            }
        } else {
            if (t[i] == 'a') {
                under_a_upper_b.ins(i);
            } else {
                under_b_upper_b.ins(i);
            }
        }
    }
    vpii v;
    ...
    for (int i = 0; i < n; ++i) {
        if (s[i] == t[i]) {
            under_a_upper_b.erase(i);
            under_a_upper_a.erase(i);
            under_b_upper_a.erase(i);
            under_b_upper_b.erase(i);
        } else {
            if (s[i] == 'a') {
                auto it = under_b_upper_a.upper_bound(i);
                if (it == under_b_upper_a.end()) {
                    if (use[i]) {
                        ok = false;
                        break;
                    }
                    ...
                    v.pb({i+1,j+1});
                    under_b_upper_a.erase(i);
                    under_b_upper_a.erase(j);
                    s[i] = 'b'; t[j] = 'a';
                    under_b_upper_b.ins(i);
                    under_a_upper_a.ins(j);
                }
            } else {
                auto it = under_a_upper_b.upper_bound(i);
                if (it == under_a_upper_b.end()) {
                    if (use[i]) {
                        ok = false;
                        break;
                    }
                    ...
                    v.pb({i+1,j+1});
                    under_a_upper_b.erase(i);
                    under_a_upper_b.erase(j);
                    s[i] = 'a'; t[j] = 'b';
                    under_b_upper_b.ins(j);
                    under_a_upper_a.ins(i);
                }
            }
            under_a_upper_b.erase(i);
