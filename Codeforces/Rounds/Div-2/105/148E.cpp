...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = 0; j < sz[i]; ++j) {
            pref[j] = a[j] + (j?pref[j-1]:0);
        }
        for (int j = sz[i] - 1; j >= 0; --j) {
            suf[j] = a[j] + (j+1<sz[i]?suf[j+1]:0);
        }
        for (int left = 0; left <= sz[i]; ++left) {
            for (int right = 0; right + left <= sz[i]; ++right) {
                int val = 0;
                if (left) val += pref[left-1];
                if (right) val += suf[sz[i] - right];
                precalc[i][left + right] = max(precalc[i][left + right], val);
            }
        }
    }
    vvl dp(n+1, vl(m+1, -1e9));
    dp[0][0] = 0;
    for (int i = 1; i <= n; ++i) {
        for (int j = 0; j <= m; ++j) {
            dp[i][j] = dp[i-1][j];
        ...
        for (int j = 0; j <= m; ++j) {
            for (int k = 0; k <= min(j, sz[i-1]); ++k) {
                dp[i][j] = max(dp[i][j], dp[i-1][j - k] + precalc[i-1][k]);
            }
        }
    }
    cout << dp[n][m];
