...
//    freopen("bees.out", "wt", stdout);
    ...
    int t_; cin >> t_; while(t_--) {
        ...
        function<void(int)> dfs = [&](int curV) {
            for (int next : g[curV]) {
                if (used[next]) continue;
                parent[next] = curV;
                used[next] = true;
                if (left[curV] == -1) left[curV] = next;
                else right[curV] = next;
                dfs(next);
                dp[curV] += dp[next];
            }
        };
        ...
        function<void(int)> dfs2 = [&](int curV) {
            if (left[curV] != -1 && right[curV] != -1) {
                dfs2(left[curV]);
                dfs2(right[curV]);
                dp2[curV] = max(dp2[left[curV]] + dp[right[curV]] - 1, dp2[right[curV]] + dp[left[curV]] -1);
            } else if (left[curV] != -1) {
                dfs2(left[curV]);
                dp2[curV] = dp[left[curV]] -1;
            } else if (right[curV] != -1) {
                dfs2(right[curV]);
                dp2[curV] = dp[right[curV]] -1;
            }
        };
