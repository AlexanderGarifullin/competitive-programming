...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            if (v[i][j] == 'B') {
                if (a.first == -1 || i + j > a.first + a.second) a = make_pair(i,j);
                if (b.first == -1 || i + j < b.first + b.second) b = make_pair(i,j);
                if (c.first == -1 || i - j > c.first - c.second) c = make_pair(i,j);
                if (d.first == -1 || i - j < d.first - d.second) d = make_pair(i,j);
            }
        }
    }
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            int maxDist = 0;
            for (auto [x,y] : good) {
                maxDist = max(maxDist, abs(i - x) + abs(j - y));
            }
            if (maxDist < best) {
                best = maxDist;
                x = i; y = j;
