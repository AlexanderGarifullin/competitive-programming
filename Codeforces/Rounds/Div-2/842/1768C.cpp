https://codeforces.com/contest/1768/problem/C#include <bits/stdc++.h>
...
i32 main() {
    ...
    int t; cin >> t; while(t--) {
        ...
        vi q(n, -1), p(n, -1);
        for (int i = 0; i < n; ++i) {
            auto itFreeFromP = setp.find(pairs[i].first);
            auto itFreeFromQ = setq.find(pairs[i].first);
            if (itFreeFromP != setp.end()) {
                p[pairs[i].second] = *itFreeFromP;
                setp.erase(itFreeFromP);
            } else if ( itFreeFromQ != setq.end()) {
                q[pairs[i].second] = *itFreeFromQ;
                setq.erase(itFreeFromQ);
            } else {
                ok = false;
                break;
            }
        ...
        sort(all(freePos));
        for (int i = 0; i < isz(freePos); ++i) {
            auto it = setp.begin();
            if (*it > freePos[i].first) {
                ok = false;
                break;
            }
            p[freePos[i].second] = *it;
            setp.erase(it);
        }
        freePos = {};
        for (int i = 0; i < n; ++i) {
