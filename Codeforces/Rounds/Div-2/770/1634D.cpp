...
void solve() {
    ...
    for (int i = 3; i + 1 <= n; i+=2) {
        vi pos = {good.first, good.second, i, i+1};
        vpii values(4);
        for (int j = 0; j < 4; ++j) {
            vi cords = pos;
            cords.erase(cords.begin() + j);
            values[j] = make_pair(ask(cords[0], cords[1], cords[2]), pos[j]);
        }
        sort(all(values));
        good = make_pair(values[0].second, values[1].second);
    }

    if (n % 2) {
        int add = 1;
        while(add == good.first || add == good.second) add++;
        vi pos = {good.first, good.second, n, add};
        vpii values(4);
        for (int j = 0; j < 4; ++j) {
            vi cords = pos;
            cords.erase(cords.begin() + j);
