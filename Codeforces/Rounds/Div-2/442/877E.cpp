...
struct SegmentTree {
    ...
    void push(int v, int l, int r) {
        ...
        tree[2*v] = m - l  - tree[2*v];
        tree[2*v+1] = r - m  - tree[2*v+1];
    }

    void update(int v, int vl, int vr, int ql, int qr) {
        if (ql >= vr || qr <= vl) {
            return;
        }
        if  (ql <= vl && vr <= qr) {
        ...
        push(v, vl, vr);
        int m = (vl + vr) / 2;
        update(2*v, vl, m, ql, qr);
        update(2*v+1, m, vr, ql, qr);
        tree[v] = tree[2*v] + tree[2 * v + 1];
    }


    int get(int v, int vl, int vr, int ql, int qr) {
        if (ql >= vr || qr <= vl) {
            return 0;
        }
        if (ql <= vl && vr <= qr) {
        ...
        push(v, vl, vr);
        int m = (vl + vr) / 2;
        return get(2*v, vl, m, ql, qr) + get(2*v+1, m, vr, ql, qr);
    }
};
...
void solve() {
    ...
    tout = vi(n+1);
    dfs(1);
    // для каждой вершины мы знаем его отрезок + значения на вершинках
    SegmentTree segmentTree(n);
    int q; cin >> q; while(q--) {
        str s; cin >> s;
        int v; cin >> v;
        if (s == "get") {
            cout << segmentTree.get(1, 1, n+1, tin[v], tout[v]+1) << en;
        } else {
            segmentTree.update(1, 1, n+1, tin[v], tout[v]+1);
        }
    }
}
