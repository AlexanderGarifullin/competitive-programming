...
void solve() {
    ...
    ll ans = 0;
    if (c[1] > c[2] + c[6]) c[1] = c[2] + c[6];
    if (c[2] > c[1] + c[3]) c[2] = c[1] + c[3];
    if (c[3] > c[2] + c[4]) c[3] = c[2] + c[4];
    if (c[4] > c[3] + c[5]) c[4] = c[3] + c[5];
    if (c[5] > c[4] + c[6]) c[5] = c[4] + c[6];
    if (c[6] > c[1] + c[5]) c[6] = c[1] + c[5];
    if (x > 0 && y > 0) {
        ll d = min(x,y);
        ans += d * c[1];
        x -= d; y -= d;
