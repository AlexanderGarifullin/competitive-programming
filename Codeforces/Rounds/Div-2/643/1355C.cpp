...
//    freopen("output.txt", "wt", stdout);
    ...
    ll r = 0;
    // z < x + y
    for (int sum = 2; sum <= b + c; ++sum) {
        int rz = min(d, sum - 1); // Правая граница z
        int nz = max(0, rz - c + 1); // Количество z
        // Сколько способов выбрать х и у
        // sum - x >= b ----> x <= sum - b
        // sum - x <= c ----> x >= sum - c
        ...
        int right = min(sum-b, b);
        int left = max(sum - c, a);
        int nx = max(0, right - left + 1);
        r += 1ll * nx * nz;
    }
    cout << r;
}
