...
//    freopen("output.txt", "wt", stdout);
    ...
    int n,m; cin >> n >> m;
    vi a(1<<n); fin(a);
    map<pii, int> segment_value;
    function<void(int,int, bool)> get = [&](int l, int r, bool isXor) {
        if (l +1 == r) {
            segment_value[{l,r}] = a[l];
            return;
        }
        int m = (l+r)/2;
        get(l, m, !isXor);
        get(m, r, !isXor);
        int v1 = segment_value[{l,m}];
        int v2 = segment_value[{m, r}];
        int v3;
        if (isXor) {
            v3 = v1^v2;
        } else {
        ...
        segment_value[{l,r}] = v3;
    };
    get(0,isz(a), n%2==0);
    for (int i = 0; i < m; ++i) {
        int p,b; cin >> p >> b;
        p--;
        function<int(int,int, bool)> calc = [&](int l, int r, bool isXor){
            if (l + 1 == r) {
//                cout << l << ' '<< r << ' ' << b << ' ' << isXor << en;
                segment_value[{l,r}] = b;
                return b;
            ...
            if (p < mid) {
                v1 = calc(l, mid, !isXor);
                segment_value[{l, mid}] = v1;
                v2 = segment_value[{mid, r}];
            } else {
                v1 = segment_value[{l, mid}];
                v2 = calc(mid, r, !isXor);
                segment_value[{mid, r}] = v2;
            }
//            cout << isXor << en;
//            cout << l << ' ' << mid << ' ' << v1 << en;
//            cout << mid << ' ' << r << ' ' << v2 << en;
