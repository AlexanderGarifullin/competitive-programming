...
void solve() {
    ...
    int shift = accumulate(all(b), 0);
    shift *= k;
    vvi dp(n + 1, vi(shift + 1 + 100 * 100 + 1, -1e9));
    dp[0][shift] = 0;
    for (int i = 1; i <= n; ++i) {
        ...
        for (int j = 0; j < isz(dp[i]); ++j) {
            if (0 <= j - a[i-1] + k * b[i-1] && j - a[i-1] + k * b[i-1] < isz(dp[i])) {
                dp[i][j] = max(dp[i][j], dp[i-1][j - a[i-1] + k * b[i-1]] + a[i-1]);
            }
        }
    }
    if (dp[n][shift] == 0) dp[n][shift] = -1;
    cout << dp[n][shift];
}
