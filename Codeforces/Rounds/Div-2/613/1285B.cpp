...
i32 main() {
    ...
    int t; cin >> t; while(t--) {
        int n; cin >> n; vi a(n); fin(a);
        ll s = accumulate(all(a), 0ll);
        //cout << s << en;
        int minel = *min_element(all(a));
        int maxel = *max_element(all(a));
        if (maxel <= 0) cno;
        else if (minel > 0) cyes;
        else {
            ll mm = maxel;
            vl pref(n), suf(n);
            for (int i = 0; i < n; ++i) {
            ...
            for (int i = 0; i < n; ++i) {
                prefmin[i] = pref[i];
                if (i > 0) prefmin[i]=min(prefmin[i],prefmin[i-1]);
            }
            for (int i = 0; i < n; ++i) {
                if (i == 0) {
                    ll cur = s - suf[i+1];
                    ...
                    if (i == n -1 && prefmin[i-1] > 0) continue;
                    ll cur = s - min(0ll,prefmin[i-1]);
