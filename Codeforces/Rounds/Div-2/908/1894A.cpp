    ...
    int t; cin >> t; while(t--) {
        ...
        for (int x = 1; x <= n; ++x) {
            for (int y = 1; y <= n; ++y) {
                ...
                if (can && cnt1 == 0 && cnt2 == 0 && max(cntWin1,cntWin2) == y) {
                    if (cntWin1 > cntWin2) st.insert('A');
                    else if (cntWin1 < cntWin2) st.insert('B');
                    else {
                        st.insert('B');st.insert('A');
                    }
                }
            }
        }

        if (isz(st) == 1) cout << *st.begin() << en;
        else cout << '?' << en;
    }
}
 
