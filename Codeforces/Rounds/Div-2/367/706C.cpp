...
//    freopen("bees.out", "wt", stdout);
    ...
    vi c(n); fin(c);
    vs v(n);fin(v);
    // минимальное кол-во энерегии, чтобы была остортировано и при этом
    // 0 = текущая не переворачиваем
    // 1 = текущую перевернули
    vvl dp(2,vl(n,-1));
    ...
    for (int i = 1; i < n; ++i) {
        if (v[i] >= v[i-1] && dp[0][i-1] != -1) {
            dp[0][i]=dp[0][i-1];
        }
        str pr = v[i-1];
        reverse(all(pr));
        if (v[i] >= pr && dp[1][i-1] != -1 && (dp[0][i] == -1 ||
        dp[0][i] > dp[1][i-1])) {
            dp[0][i]= dp[1][i-1];
        }
        str cur = v[i];
        reverse(all(cur));
        if (cur >= v[i-1] && dp[0][i-1] != -1) {
            dp[1][i] = dp[0][i-1] + c[i];
        }
        if (cur >= pr && dp[1][i-1] != -1 && (dp[1][i] == -1 ||
            dp[1][i] > dp[1][i-1] + c[i])) {
            dp[1][i] = dp[1][i-1] + c[i];
        }
    }
    
    if (dp[0][n-1]==-1 && dp[1][n-1]==-1) cmo;
    else if (dp[0][n-1]==-1 || dp[1][n-1]==-1) cout << max(dp[0][n-1],dp[1][n-1]);
