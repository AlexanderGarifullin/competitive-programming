...
//    freopen("output.txt", "wt", stdout);
    ...
    // 0 = i = index tree
    // 1 = j = beauty
    // 2 = k = i-tree has color k
    vvvl dp(101, vvl(101, vl(101, 1e18)));
    ...
    for (int i = 2; i <=n; ++i) {
        for (int beauty = 1; beauty <= k; ++beauty) {
            if (c[i] == 0) {
                for (int color1 = 1; color1 <= m; ++color1) {
                    dp[i][beauty][color1] = min(dp[i][beauty][color1],
                                                dp[i-1][beauty][color1] + cost[i][color1]);
                    for (int color2 = 1; color2 <= m; ++color2) {
                        if (color2 == color1) continue;
                        dp[i][beauty][color1] = min(dp[i][beauty][color1],
                                                    dp[i-1][beauty-1][color2] + cost[i][color1]);
                    }
                }
            } else {
                dp[i][beauty][c[i]] = min(dp[i][beauty][c[i]],
                                          dp[i-1][beauty][c[i]]);
                for (int color2 = 1; color2 <= m; ++color2) {
                    if (color2 == c[i]) continue;
                    dp[i][beauty][c[i]] = min(dp[i][beauty][c[i]],
                                                   dp[i-1][beauty-1][color2]);
                }
            }
        }
    }
