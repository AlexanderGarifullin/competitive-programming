...
bool check1(int ind, vi &a) {
    return a[ind-1] > a[ind] && a[ind] < a[ind+1];
}

bool check2(int ind, vi &a) {
    return a[ind-1] < a[ind] && a[ind] > a[ind+1];
}
    ...
    int t_; cin >> t_; while(t_--) {
        ...
        if (n>=3) {
            if (n == 3) {
                ...
                if (check1(1,a) || check2(1,a) || check1(n-2,a) || check2(n-2,a)) {
                    res = c-1;
                }
                for (int i = 1; i < n-1; ++i) {
                    vi v;
                    ...
                    int cc =c;
                    for (int j = 1; j < isz(v)-1; ++j) {
                        if (check1(j,v) || check2(j,v)) cc--;
                    }
                    int with = cc;
                    if (isz(v) == 5) {
                        v[2]=v[1];
