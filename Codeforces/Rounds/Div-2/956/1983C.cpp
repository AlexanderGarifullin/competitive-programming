...
vi solve(vl & prefGood, vl & prefLeft, vl & prefRight, int t1, int t2, int t3) {
    vi res= {};
    ll sum = prefGood[1] - prefGood[0];
    int l = 1;
    int r = 1;
    int n = isz(prefGood);
    while (l < n-1 && r < n-1) {
        while(3 *sum < all) {
            r++;
            if (r == n-1) break;
            sum += prefGood[r] - prefGood[r-1];
        }
        if (3 * sum < all) return res;
        if (r == n-1) break;
        ll s1 = prefLeft[l-1];
        ll s2 = prefRight.back() - prefRight[r];
        if (3*s1 >= all && 3 * s2 >= all) {
            res = vi(6);
            res[2*t1 - 1] = r+1;
            res[2*t1 - 2] = l+1;
            res[2*t2 - 1] = l;
...
void CBO(){
    ...
    vi v = solve(prefa, prefb, prefc, 1, 2, 3);
    if (!v.empty()) {
        fout(v);
        return;
    }

    v = solve(prefb, prefa, prefc, 2, 1, 3);
    if (!v.empty()) {
        fout(v);
        return;
    }
    v = solve(prefc, prefa, prefb, 3, 1, 2);
    if (!v.empty()) {
        fout(v);
        return;
    }
