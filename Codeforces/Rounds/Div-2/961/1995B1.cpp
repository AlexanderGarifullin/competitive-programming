...
void CBO() {
    ...
    ll best =0;
    for (auto [value, cn] : cnt) {
        ll canCnt = m / value;
        while ((1+canCnt<= cn) &&(1+canCnt) * value <= m) canCnt++;
        best = max(best, min(cn, canCnt) * value);

        ll v2 = value - 1;
        if (cnt.contains(v2)) {
            for (int cntX1 = 0; cntX1 <= cnt[v2]; ++cntX1) {
                ll sum = cntX1 * v2;
                if (sum <= m) {
                    ll needSum = m - sum;
                    canCnt = needSum / value;
                    while ((1+canCnt <= cn) && (1+canCnt) * value <= needSum) canCnt++;
                    best=max(best, sum + min(canCnt, cn) * value);
                }
            }
        }
    }
