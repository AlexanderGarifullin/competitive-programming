...
void CBO() {
    ...
    vpll v(n);
    for (int i = 0; i < n; ++i) {
        v[i] = make_pair(a[i], c[i]);
    }
    sort(all(v));
    ll best=0;

    for (int i = 0; i < n; ++i) {
        ll canCnt = m / v[i].first;
        while ((1+canCnt<= v[i].second) &&(1+canCnt) * v[i].first <= m) canCnt++;
        best = max(best, min(v[i].second, canCnt) * v[i].first);

        if (i + 1 < n && v[i].first + 1 == v[i+1].first) {
            ll a = v[i].first;
            ll c1 = v[i].second;
            ...
            ll can2 = min(c2, ost / b);
            ost -= can2 * b;

            best=max(best, can1 * a + can2 * b);

            ll free = c2 - can2;
            ll dif = min(free, min(ost, can1));
