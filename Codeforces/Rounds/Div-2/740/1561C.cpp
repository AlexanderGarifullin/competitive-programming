...
i32 main() {
    ...
    int t; cin >> t; while(t--) {
        ...
        sort(all(v),[&](vi v1, vi v2){
            int minPower1=minpower[v1.back()], minPower2 =minpower[v2.back()];
            return minPower1 < minPower2 ||minPower1 < minPower2 && isz(v1) > isz(v2);
        });
        int left = -1;
        int right = 1e9 + 1;
        function<bool(int x)> ok = [&](int power){
            for (int i = 0; i < n; ++i) {
                for (int j = 0; j < isz(v[i]) - 1; ++j) {
                    if (v[i][j] >= power) return false;
                    power++;
                }
            }
            return true;
