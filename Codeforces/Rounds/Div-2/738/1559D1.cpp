...
i32 main() {
    ...
    for (int i = 1; i < n; ++i) {
        for (int j = i+1; j <= n; ++j) {
            if (id1[i] == id1[j] || id2[i] == id2[j]) continue;
            v.pb({i,j});
            int x = id1[i];
            int y = id1[j];
            if (isz(list1[x]) > isz(list1[y])) swap(x,y);
            for (int cur : list1[x]) {
                list1[y].insert(cur);
                id1[cur] = y;
            }
            ...
            x = id2[i];
            y = id2[j];
            if (isz(list2[x]) > isz(list2[y])) swap(x,y);
            for (int cur : list2[x]) {
                list2[y].insert(cur);
                id2[cur] = y;
            }
