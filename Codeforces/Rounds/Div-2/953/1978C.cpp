...
vl solve(ll n, ll k) {
    ...
    for (int i = 0; i < n; ++i) {
        int dif = mmax - mmin;
        if (2 * dif <= k) {
            swap(p1[mmin - 1], p1[mmax - 1]);
            k -= 2 *dif;
            mmin++;
            mmax--;
        } else break;
    }
    swap(p1[mmin-1], p1[mmin-1 + k / 2]);
    return p1;
}
        ...
        for (int i = 0; i < 10; ++i) {
            for (int k = 0; k < 10; ++k) {
                ...
                do {
                    vl pp = solve(n, k);
                    vl ppp = slow(n, k);
                    if (pp.empty() && ppp.empty()) continue;
                    if (pp.empty() || ppp.empty()) {
                        cout << 1 << en;
                        cout << n << ' ' << k << en;
                        fout(pp);
                        fout(ppp);
