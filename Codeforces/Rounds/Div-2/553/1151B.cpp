...
//    freopen("bees.out", "wt", stdout);
    ...
    vector<vector<vector<int>>> have(n, vvi(11, vi(2)));
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < 11; ++j) {
            for (int x : v[i]) {
                if (x & (1<<j)) have[i][j][1] = 1;
                else have[i][j][0] = 1;
            }
        }
    }
    ...
    for (int i = 0; i < 11; ++i) {
        ...
        int onlyhavenot = 0;
        for (int j = 0; j < n; ++j) {
            if (have[j][i][0] && have[j][i][1]) two++;
            else if (have[j][i][0]) onlyhavenot++;
            else onlyhave++;
        }
        if (onlyhave % 2 || onlyhave % 2 == 0 && two > 0) {
            ok = true;
            if (onlyhave % 2) {
                // take only have
                for (int j = 0; j < n; ++j) {
                    if (have[j][i][0] && have[j][i][1]) {
                        for (int k = 0; k < m; ++k) {
                            if ((v[j][k] & (1<<i)) == 0) {
                                ind.pb(k+1);
                                break;
