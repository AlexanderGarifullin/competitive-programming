    ...
    if (r != curR) {
        for (ull i = 0; i < 100 && curR <= r; ++i) {
            ...
            if (curR > r) {
                ...
                if (i % 2) {
                    // even
                    lastEven = lastEven + 2 * good;
                } else {
                    // odd
                    lastOdd = lastOdd + 2 * good;
                }
                break;
            } else {
                if (i % 2) {
                    // even
                    lastEven = lastEven + 2 * len;
                } else {
                    // odd
                    lastOdd = lastOdd + 2 * len;
                }
            }
            if (curR == r) break;
        }
    }
//    cout << lastOdd << ' ' << sum(lastOdd) << en;
//    cout << lastEven << ' ' << sum(lastEven) << en;
    return (sum(lastOdd) + sum(lastEven)) % mod;
}
