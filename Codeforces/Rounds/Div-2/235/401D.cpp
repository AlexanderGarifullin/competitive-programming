...
int const T = 10 + 5, MASK = 3e4 + 20, M = 100 + 10;
int mult[T], cnt[T], base[T], m;
ll n, dp[MASK][M];
...
void solve() {
    ...
    for (int i = 0 ; i < 10 ; i ++){
        base[i] = cnt[i] + 1;
        mult[i + 1] = base[i] * mult[i];
    }

    for (int i = 1 ; i < 10 ; i ++) if (cnt[i]) dp[mult[i]][i % m] ++;

    for (int mask = 0 ; mask < mult[10] ; mask ++)
        for (int i = 0 ; i < 10 ; i ++)
            for (int j = 0 ; j < m ; j ++)
                if (mask / mult[i] % base[i] != cnt[i]) dp[mask + mult[i]][(j * 10 + i) % m] += dp[mask][j];
