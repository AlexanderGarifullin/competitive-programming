...
void solve() {
    ...
    int r=0,l=0;
    int best=0;
    vector<multiset<int>> maxValues(m);
    while(r<n){
        int need = 0;
        for (int i = 0; i < m; ++i) {
            maxValues[i].insert(a[r][i]);
            need += *maxValues[i].rbegin();
        }
        while(l < r && need > k) {
            for (int i = 0; i < m; ++i) {
                need -= *maxValues[i].rbegin();
                maxValues[i].erase(maxValues[i].find(a[l][i]));
                if (!maxValues[i].empty()) need += *maxValues[i].rbegin();
            }
            l++;
        }
        if (need <= k && r - l + 1 > best) {
            best = r - l + 1;
            for (int i = 0; i < m; ++i) {
                ans[i] = *maxValues[i].rbegin();
            }
        }
        r++;
    }
