...
int smartsolve(int n, str s) {
    ...
    str sss;
    for (int i = 0; i < n; ++i) {
        if (color[i]==0) sss += s[i];
    }
    if (!is_sorted(all(sss))) {
        return -1;
    }
    reverse(all(ss));
    int j = 0;
...
int stupidsolve(int n, str s) {
    for (int i = 0; i <= 100; ++i) {
        if (is_sorted(all(s))) return i;
        vi ind;
        int start = 0;
        for (int j = 25; j >=0 ; --j) {
            char c ='a';
        ...
        str ss = s;
        for (int j = 0; j < isz(ind); ++j) {
            s[ind[j]] = ss[ind[(isz(ind)+j-1)%isz(ind)]];
        }
    }
    return -1;
}
...
//                s[j]+=add;
//            }
//            int my = smartsolve(n, s);
//            int ans = stupidsolve(n, s);
//            if (my == ans) {
//                cout << "OK" << en;
//            } else {
//                cout << "WA" << en;
