...
//    freopen("output.txt", "wt", stdout);
    ...
    function<void(int)> dfs = [&](int curV) {
        ...
        if (comp.back().empty()) {
            ...
            maximum.back() = max(maximum.back(), curV);
            minimum.back() = min(minimum.back(), curV);
        }
        comp.back().pb(curV);
    ...
    vpii segments;
    for (int i = 0; i < isz(comp); ++i) {
        segments.pb({minimum[i], maximum[i]});
    }
    sort(all(segments));
    int l = segments[0].first;
    int r = segments[0].second;
    for (int i = 0; i < isz(comp)-1; ++i) {
        auto[ll,rr] = segments[i+1];
        if (ll < r) {
            res++;
            r = max(r, rr);
        } else {
