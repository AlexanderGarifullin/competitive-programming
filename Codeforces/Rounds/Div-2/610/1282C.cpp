...
void CBO(){
    ...
    vpii tt(n);
    for (int i = 0; i < n; ++i) {
        tt[i] = make_pair(ti[i], type[i]);
    }
    sort(all(tt));
    vpii timeCntBad = {make_pair(tt[0].first-1, 0)};
    vi cntAll = {0};
    int cntBadPred = tt[0].second;
    int cntToAdd = 1;
    for (int i = 1; i < isz(tt); ++i) {
        auto [time, ty] = tt[i];
        if (timeCntBad.back().first + 1 == time) {
            cntBadPred += tt[i].second;
            cntToAdd++;
        } else {
            timeCntBad.emplace_back(time-1, cntBadPred);
            cntBadPred += tt[i].second;
            cntAll.pb(cntToAdd);
            cntToAdd++;
        }
    ...
    int cntAllBad = accumulate(all(type), 0);
    int cntAllGood = n - cntAllBad;

    for (int i = 0; i < isz(timeCntBad); ++i) {
        auto [time, cntBad] = timeCntBad[i];
        int cnt = cntAll[i];
       // cout << time << ' ' << cntBad << ' ' << cnt << en;
        ll needTime = 1ll * cntBad * b + 1ll * (cnt - cntBad) * a;
        if (time >= needTime) {
            int add =0;
            int cntGood = cnt - cntBad;

            int freeBad = cntAllBad - cntBad;
            int freeGod = cntAllGood - cntGood;

            int freeTime = time - needTime;

            int addGroup = min(freeTime/a, freeGod);

            add += addGroup;
            freeTime -= a * addGroup;

            int addBadGroup = min(freeTime/b, freeBad);

            add += addBadGroup;
            freeTime -= b * addBadGroup;

            best = max(best, add + cnt);
        }
    }
    cout << best << en;
}
