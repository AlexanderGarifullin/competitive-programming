...
void solve() {
    int n,m; cin >> n >> m;
    vector<vector<tiii>> g(n+1);
    for (int i = 0; i < m; ++i) {
        int a,b,w; cin >> a >> b >> w;
        g[a].emplace_back(b, w, i+1);
        g[b].emplace_back(a, w, i+1);
    }
    int start; cin >> start;
    ll answ = 0;
    vi v;
    ll inf = 1e18;
    vl dist(n+1, inf);
    // dist, lastEdgeCost, lastEdgeId, vertex
    pqsmalleststart<tuple<ll, int, int ,int>> q;
    q.push({0,0,0,start});
    while(!q.empty()) {
        auto [curLen, lastEdgeCost, lastEdgeId, vertex] = q.top();
        q.pop();
        if (dist[vertex] != inf) continue;
        dist[vertex] = curLen;
        if (lastEdgeId != 0) {
            answ += lastEdgeCost;
            v.push_back(lastEdgeId);
        }
        for (auto [nextV, cost, id] : g[vertex]) {
            if (dist[nextV] == inf) {
                q.push({curLen + cost, cost, id, nextV});
            }
        }
    }
    cout << answ << en;
