...
i32 main() {
   ...
   while(t--){
      ...
      vi a(n+1);
       ...
       function<void(int, int)> get_sums = [&](int l, int r){
           sums.ins(pref[r] - pref[l-1]);
           auto it = upper_bound(all(a), (a[r] + a[l]) / 2);
           if (it - a.begin() == r + 1) return;
           get_sums(l, it - a.begin() - 1);
           get_sums(it - a.begin(), r);
