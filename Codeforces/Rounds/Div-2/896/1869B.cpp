...
i32 main() {
    ...
    int t; cin >> t; while(t--) {
        ...
        a--;
        b--;
        ll res = abs(cords[a].first - cords[b].first) + abs(cords[a].second - cords[b].second);


        ll minLenFromAtoBig = 1e18, minb = 1e18;
        for (int i = 0; i < k; ++i) {
            minLenFromAtoBig = min(minLenFromAtoBig, abs(cords[a].first - cords[i].first) + abs(cords[a].second - cords[i].second));
            minb = min(minb, abs(cords[b].first - cords[i].first) + abs(cords[b].second - cords[i].second));
        }



        if (k) res = min(res, minLenFromAtoBig + minb);
