...
void solve() {
    ...
    for (int len = n-1; len >= 0; --len) {
        // текущий символ должен быть больше чем s[len]
        // len = префикс какой длины совпадает
        cnt[s[len] - 'a']--;
        // на префиксе остались числа cnt
        // хотим выбрать суффикс
        for (int j = (s[len] - 'a') + 1; j < C; ++j) {
            int sum = 0;
            vi need(C);
            for (int i = 0; i < C; ++i) {
                need[i] = (k - ((cnt[i] + (i == j)) % k)) % k;
                sum += need[i];
            }
            int lenSuffix = n - len - 1;
            if (sum > lenSuffix) continue;
            // s[0..len) + j + suffix
            // хотим построить суффикс
            str ans = s.substr(0, len);
            ans += char('a' + j);
            int free = lenSuffix - sum;
            need[0] += free;
            for (int i = 0; i < C; ++i) {
                ans += string(need[i], char('a' + i));
            }
            cout << ans << en;
            return;
        }
