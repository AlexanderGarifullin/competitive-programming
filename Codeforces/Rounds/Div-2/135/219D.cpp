...
int get(int cur, int pred) {
    int c = 0;
    for (auto [next, type] : g[cur]) {
        if (next == pred) continue;
        c += (type == 2) + get(next, cur);
    }
    return c;
}

void dfs(int cur, int pred, int aa, int bb) {
    for (auto [next, type] : g[cur]) {
        if (next == pred) continue;
        if (type == 1) {
            a[next] = aa + 1;
