...
vi mysolve(int n, int m,str s, vector<pair<int,char>> op) {
    vi res;
    int cntGroup=0;
    for (int i = 0; i < n-1; ++i) {
        if (s[i] == '.' && s[i+1] == '.') {
...
vi solve(int n, int m, str s, vector<pair<int, char>> op) {
    vi res;
    int cnt = 0;
    ...
    for (int i = 0; i < m; ++i) {
        auto [pos, c] = op[i];
        pos--;
        if (pos > 0 && s[pos] == s[pos - 1] && s[pos] == '.') cnt--;
        if (pos + 1 < n && s[pos] == s[pos + 1] && s[pos] == '.') cnt--;
        s[pos] = c;
        if (pos > 0 && s[pos] == s[pos - 1] && s[pos] == '.') cnt++;
        if (pos + 1 < n && s[pos] == s[pos + 1] && s[pos] == '.') cnt++;
        res.pb(cnt);
...
//            op[i]={x,c};
//        }
//        vi r1 = mysolve(n,m,s,op);
//        vi r2 = solve(n,m,s,op);
//        for (int i = 0; i < m; ++i) {
//            s[op[i].first-1]=op[i].second;
//            if (r1[i] != r2[i]) {
//                cout << n << ' ' << n << en;
//                cout << s << en;
//                for (auto [x,y] : op) {
