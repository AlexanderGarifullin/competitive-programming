...
void CBO(){
    ...
    set<pll> q;
    for (int i = 0; i < n; ++i) {
        q.ins(make_pair(c[i], i));
    }
    vb use(n);
    for (int i = 0; i < n; ++i) {
        auto [len, v] = *q.begin(); q.erase(q.begin());
        best += len; use[v] = true;
        if (link[v] == -1) {
            b.pb(v+1);
        } else {
            e.emplace_back(link[v]+1, v+1);
        }
        for (int j = 0; j < n; ++j) {
            if (use[j]) continue;
            ll cl = abs(cords[j].first - cords[v].first) +
                    abs(cords[j].second - cords[v].second);
            cl *= (k[j] + k[v]);
            if (cl < c[j]) {
                q.erase(make_pair(c[j], j));
                c[j] = cl;
                link[j] = v;
                q.insert(make_pair(c[j], j));
            }
        }
    }
