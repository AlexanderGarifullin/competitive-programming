...
int n;
vi a, divisors, ans, cnt;
vvi g;

void dfs(int u, int p, int gcd) {
    ans[u] = gcd;
    for (int i = 0; i < isz(divisors); ++i) {
        if (a[u] % divisors[i] != 0) cnt[i]--;
        if (cnt[i] >= -1) {
            ans[u] = max(ans[u], divisors[i]);
        }
    }
    for (int v : g[u]) {
        if (v != p) dfs(v, u, __gcd(gcd, a[v]));
    }
...
void solve() {
    ...
    for (int i = 1; i <= a[0]; ++i) {
        if (a[0] % i == 0) divisors.pb(i);
    }
    cnt.resize(isz(divisors));
    for (int i = 0; i < n-1; ++i) {
        int a,b;cin >> a >> b; a--; b--;
        g[a].pb(b); g[b].pb(a);
    }
