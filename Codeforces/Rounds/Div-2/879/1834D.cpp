...
void solve() {
    ...
    int maxLeft = *max_element(all(l));
    int minRight = *min_element(all(r));
    int ans= 2 * (maxLen - minLen);
    for (int i = 0; i < n; ++i) {
        int len = r[i] - l[i] + 1;
        if (maxLeft > r[i] || minRight < l[i]) ans=max(ans, 2 * len);
        else {
            int dist = max(0, r[i] - maxLeft + 1);
            ans = max(ans, 2 * (len - dist));
            dist = max(0, minRight - l[i] + 1);
            ans = max(ans, 2 * (len - dist));
        }
    }
    cout << ans << en;
