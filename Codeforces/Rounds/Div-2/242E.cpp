...
struct SegemtTree{
    ...
    void build(int v, int l, int r) {
        if (l + 1 == r) {
            ...
            build(vr, mid, r);
            for (int i = 0; i < LG; ++i) {
                tree[i][v] = tree[i][vl] + tree[i][vr];
            }
        }
    }


    void push(int v, int left, int right, int val) {
        lazy[v] ^= val;
        for (int i = 0; i < LG; ++i) {
            if (val & (1<<i)) {
                tree[i][v] = right - left - tree[i][v];
            }
        }
    }

    void shift(int v, int left, int right) {
        int mid = (left + right) / 2;
        push(2*v, left, mid, lazy[v]);
        push(2*v + 1, mid, right, lazy[v]);
        lazy[v] = 0;
    }

    ll get(int v, int left, int right, int qLeft, int qRight) {
        ...
        int vLeft = 2 * v;
        int vRight = 2 * v + 1;
        return get(vLeft, left, mid, qLeft, qRight) + get(vRight, mid, right, qLeft, qRight);
    }

    void update(int v, int left, int right, int qLeft, int qRight, int val) {
        if (qRight <= left || right <= qLeft) {
            return;
        }
        if (qLeft <= left && right <= qRight) {
            push(v, left, right, val);
            return;
        }
        shift(v, left, right);
        int mid = (left + right ) /2;
        int vLeft = 2 * v;
        int vRight = 2 * v + 1;
        update(vLeft, left, mid, qLeft, qRight, val);
        update(vRight, mid, right, qLeft, qRight, val);
        for (int i = 0; i < LG; ++i) {
           tree[i][v] = tree[i][vLeft] + tree[i][vRight];
        }
    }
};
...
void solve() {
    ...
    for (int i = 0; i < m; ++i) {
        ...
        if (t == 1) {
            int l,r; cin >> l >> r;
            l--;
            cout << segemtTree.get(1, 0, n, l, r) << en;
        } else {
            int l,r,x; cin >> l >> r >> x;
            l--;
            segemtTree.update(1, 0, n, l, r, x);
        }
    }
}
