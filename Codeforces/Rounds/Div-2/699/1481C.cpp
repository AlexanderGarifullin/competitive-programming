...
//    freopen("bees.out", "wt", stdout);
    ...
    int t_; cin >> t_; while(t_--){
        ...
        fin(a); fin(b); fin(c);
        vi res(m,-1);
        map<int, vi> color_index;
        for (int i = 0; i < m; ++i) {
            color_index[c[i]].pb(i);
        }
        bool ok = 1;
        for (int i = 0; i < n; ++i) {
            if (a[i] != b[i]) {
                if (color_index.find(b[i]) == color_index.end()) {
                    ok = false;
                    break;
                }
                int x = color_index[b[i]].back();
                color_index[b[i]].pop_back();
                res[x] = i+1;
                if (color_index[b[i]].empty()) {
                    color_index.erase(b[i]);
                }
            }
        }
