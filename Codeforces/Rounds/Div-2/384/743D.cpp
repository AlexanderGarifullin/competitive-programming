...
void dfs1(int cur, int pred = -1) {
    sum[cur] = a[cur];
    for (int next : g[cur]) {
        if (next == pred) continue;
        dfs1(next,cur);
        sum[cur] += sum[next];
        maxsum[cur] = max(maxsum[cur], maxsum[next]);
    }
    maxsum[cur] = max(maxsum[cur], sum[cur]);
}

void dfs2(int cur, int pred = -1, ll maxSumOut = -1e18) {

    if (maxSumOut != -1e18) {
        best = max(best, maxSumOut + maxsum[cur]);
    }
    
    vpll sumIndex;
    for (int next: g[cur]) {
        if (next == pred) continue;
        sumIndex.emplace_back(maxsum[next], next);
    }
    sort(all(sumIndex), [](pll p1, pll p2){
    ...
    for (int next: g[cur]) {
        if (next == pred) continue;
        ll maxOut = -1e18;
        if (sumIndex[0].second != next) maxOut = sumIndex[0].first;
        else if (isz(sumIndex)>1) maxOut = sumIndex[1].first;
        dfs2(next, cur, max(maxSumOut, maxOut));
    }
}
