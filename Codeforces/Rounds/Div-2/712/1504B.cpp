...
i32 main() {
    ...
    int t; cin >> t; while(t--) {
        ...
        vi ind;
        for (int i = 0; i < j + 1; ++i) {
            if (pref_0[i] == pref_1[i]) ind.push_back(i);
        }
        reverse(all(ind));
        for (int i = 0; i < isz(ind); ++i) {
            int Finish = ind[i];
            int Start = 0;
            if (i + 1 < isz(ind)) Start = ind[i+1] + 1;
            str sub1 = a.substr(Start, Finish - Start + 1);
            str sub2 = b.substr(Start, Finish - Start + 1);
            if (sub1 != sub2) {
