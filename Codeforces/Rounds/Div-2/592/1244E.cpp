...
void solve() {
    ...
    function<ll(int)> cntForMaximum = [&](int R) {
        int posStart = upper_bound(all(a), R) - a.begin();
        return suf[posStart] - 1ll * R * (n - posStart + 1);
    };
    function<ll(int)> cntForMinimum = [&](int L) {
        int posStart = lower_bound(all(a), L) - a.begin();
        return 1ll * L * (posStart-1) - pref[posStart - 1];
    };
    for (int i = 1; i <= n; ++i) {
        int L = a[i];
        int posStart = lower_bound(all(a), L) - a.begin();
        ll need = 1ll * L * (posStart-1) - pref[posStart - 1];
        if (need > k) break;
        int l = L - 1;
        int r = a.back();
        while(l + 1 < r) {
            int m = (l + r)/2;
            if (cntForMaximum(m) + need <= k) {
                r = m;
            } else l = m;
        }
        ans = min(ans, r - L);
    ...
    for (int i = n; i >=1; --i) {
        int R = a[i];
        int posStart = upper_bound(all(a), R) - a.begin();
        ll need = suf[posStart] - 1ll * R * (n - posStart + 1);
        if (need > k) break;
        int l = a[0];
        int r = R + 1;
        while (l + 1 < r) {
            int mid = (l + r)/2;
            if (cntForMinimum(mid) + need <= k) {
                l = mid;
            } else r = mid;
        }
        ans = min(ans, R - l);
