...
ll solve(int n, int m) {
    ...
    ll leftPos=max(0,n-m), rightPos=n+m;
    for (int i = 0; i < 33; ++i) {
        bool ok = false;
        ll blockSize = (1ll<<(i+1));
        ll x1 = (1+leftPos + blockSize - 1)/blockSize;
        ll x2 = (1+rightPos + blockSize - 1)/blockSize;
        if (x1 != x2) {
            ok = true;
        }
        else {
            ll ost = (1+rightPos) % (blockSize);
            if (ost == 0) ost = blockSize;
            ok = ost > (1ll << i);
        }
        if (ok) r += 1ll<<i;
    }
