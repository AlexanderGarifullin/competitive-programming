...
i32 main() {
    ...
    int t; cin >> t; while(t--) {
        ...
        int l = 0, r = 1;
        multiset<int> mst;
        for (int x : g[1]) mst.insert(x);

        while(l < n){
            while(r < n && mst.find(perm[r]) == mst.end()) {
                for (int x : g[perm[r]]) mst.insert(x);
                r++;
            }
            int len = r - l;
            res += 1ll * len * (len+1) / 2;
            if (r == n) break;
            while (mst.find(perm[r]) != mst.end()) {
                for (int x : g[perm[l]]) {
                    mst.erase(mst.find(x));
                    g[perm[r]].erase(perm[l]);
                }
                l++;
            }
            int len1 = r - l;
