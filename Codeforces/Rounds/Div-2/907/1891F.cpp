...
void dfs(int v, vvi & g, vector<vpii> & add, vi & time, vl & val, SegmentTree & tree) {
    for (auto [t, x] : add[v]) {
        tree.update(1, 0, tree.n, t, x);
    }
    val[v] = tree.get(1, 0, tree.n, time[v], tree.n);

    for (int next : g[v]) {
        dfs(next, g, add, time, val, tree);
    }
