...
i32 main() {
    ...
    int t; cin >> t; while(t--) {
        ...
        vi b1,b2,b3,b4,b5,b6;
        for (int i = 0; i < n; ++i) {
            if (i == 0) b1.pb(a[i]);
            else if (i < 2) b2.pb(a[i]);
            else if (i < 4) b3.pb(a[i]);
            else if (i < 8) b4.pb(a[i]);
            else if (i < 16) b5.pb(a[i]);
            else b6.pb(a[i]);
        }
        function<bool(vi)> c = [](vi a){
            for (int i = 0; i < isz(a) - 1; ++i) {
                if (a[i] > a[i+1]) return false;
            }
            return true;
        };
        bool ok = c(b1) && c(b2) && c(b3) && c(b4) && c(b5) && c(b6);
        ifyn(ok);
    }
}
