...
//    freopen("output.txt", "wt", stdout);
    ...
    int t_; cin >> t_; while(t_--) {
        ...
        int countcnt = count(all(cnt), maxcnt);
        //want abc...abc...abc...
        // n - countcnt => abc...abc... (sub last block)
        // (n - countcnt) / (maxcnt -1) -> abc... x times (-1 cause we del last block)
        // (n - countcnt) / (maxcnt -1)   -1 , because we want bc...
        if (maxcnt ==1 )cout << 0 << en;
        else cout << (n - countcnt) / (maxcnt - 1) - 1 << en;
    }
}
