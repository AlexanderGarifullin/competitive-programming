...
i32 main() {
    ...
    int t; cin >> t; while(t--) {
        ...
        vi prefmax(n),sufmax(n);
        for (int i = 0; i < n; ++i) {
            prefmax[i] = a[i];
            if (i > 0) prefmax[i] = max(prefmax[i], prefmax[i-1]);
        }
        vi v1, v2;
        for (int i = n-1; i >= 0; --i) {
            sufmax[i] = a[i];
        ...
        for (int i = 0; i < n-1; ++i) {
            v2.pop_back();
            v1.pb(a[i]);
            if (prefmax[i] == i + 1 && sufmax[i+1] == n - i - 1) {
                if (ok(v1) && ok(v2)) {
                    res.pb({prefmax[i], sufmax[i+1]});
                }
            }
        }
        ...
        for (int i = 0; i < isz(res); ++i) {
            cout << res[i].first << ' ' << res[i].second << en;
        }
    }
}
