...
i32 main() {
    ...
    function<ll(int,int)> C=[&](int n, int k) {
        if (k < 0 || k > n || n < 0) return 0ll;
        return (fact[n] * ifact[k] % mod) * ifact[n-k] % mod;
    };
    int other = n - cntBig - cntLess - 1;
    int cntLessX = x - 1;
    int cntMoreX = n - x;
    ll x1 = C(cntLessX, cntLess) * fact[cntLess] % mod;
    ll x2 = C(cntMoreX, cntBig) * fact[cntBig] % mod;
    ll res = (fact[other] * x1 % mod) * x2 % mod;
    cout << res << en;
}
