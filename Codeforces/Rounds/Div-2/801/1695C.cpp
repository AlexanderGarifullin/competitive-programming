...
//    freopen("output.txt", "wt", stdout);
    ...
    int t_; cin >> t_; while(t_--) {
        ...
        vvi minlen(n,vi(m,1e9)), maxlen(n,vi(m,-1e9));
        minlen[0][0]=maxlen[0][0]=dp[0][0];
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < m; ++j) {
                if (i != 0) {
                    minlen[i][j] = min(minlen[i][j], minlen[i-1][j] + dp[i][j]);
                    maxlen[i][j] = max(maxlen[i][j], maxlen[i-1][j] + dp[i][j]);
                }
                if (j != 0) {
                    minlen[i][j] = min(minlen[i][j], minlen[i][j-1] + dp[i][j]);
                    maxlen[i][j] = max(maxlen[i][j], maxlen[i][j-1] + dp[i][j]);
                }
            }
        }
        ifyn(in(0,minlen[n-1][m-1],maxlen[n-1][m-1]));
    }
}
