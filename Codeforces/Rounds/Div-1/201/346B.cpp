...
int const N = 100 + 10, inf = 1e9;
int kmp[N], dp[N][N][N];

void build(int i, int j, int k) {
    if (!dp[i][j][k]) return;
    if (dp[i][j][k] == dp[i-1][j][k]) {
        build(i-1,j,k); return;
    }
    if (dp[i][j][k] == dp[i][j-1][k]) {
        build(i,j-1,k);return;
    }
    ans += s[i-1];
    for (int l = 0; l < v; ++l) {
        int c = l;
        while(c && virus[c] != s[i-1]) c = kmp[c];
        if (virus[c] == s[i-1]) c++;
        if (c == k && dp[i-1][j-1][l] + 1 == dp[i][j][k]) {
            build(i-1,j-1,l);
            return;
        }
    }
...
void solve() {
    cin >> s >> t >> virus;
    n= isz(s);m=isz(t);v=isz(virus);
    int c = 0;
    for (int i = 1; i < v; ++i) {
        while(c && virus[c] != virus[i]) c = kmp[c];
        if (virus[c] == virus[i]) c++;
        kmp[i+1] = c;
    }
    ...
    for (int i = 0; i <= n; i ++)
        for (int j = 0; j <= m; j ++)
            for (int k = 0; k < v; k ++){
                ...
                dp[i][j+1][k] = max(dp[i][j+1][k], val);
                if (i < n && j < m && s[i] == t[j]){
                    int cur = k;
                    while (cur && s[i] != virus[cur]) cur = kmp[cur];
                    if (s[i] == virus[cur]) dp[i+1][j+1][cur+1] = max(dp[i+1][j+1][cur+1], val+1);
                    else dp[i+1][j+1][0] = max(dp[i+1][j+1][0], val+1);
                }
            }

    int T = 0;
    for (int k = 0; k < v; ++k) {
        if (dp[n][m][k] > dp[n][m][T]) T = k;
    }
    build(n,m,T);
    reverse(all(ans));
    if (ans.empty()) ans += '0';
