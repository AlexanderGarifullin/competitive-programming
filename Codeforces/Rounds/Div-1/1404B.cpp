...
void CBO() {
    ...
    vi dp1(n+1,-1);
    dp1[a]=0;
    function<void(int,int)> dfs = [&](int cv, int pv = -1) {
        for (int next:g[cv]) {
            if (next==pv|| dp1[next] != -1) continue;
            dp1[next] = 1 + dp1[cv];
            dfs(next, cv);
        }
    };
    ...
    int mm = max_element(all(dp1)) - dp1.begin();
    vi dp2(n+1, -1);
    dp2[mm] = 0;
    function<void(int,int)> dfs2 = [&](int cv, int pv = -1) {
        for (int next:g[cv]) {
            if (next==pv|| dp2[next] != -1) continue;
            dp2[next] = 1 + dp2[cv];
            dfs2(next, cv);
        }
    };
    dfs2(mm,-1);
    int d = *max_element(all(dp2));
