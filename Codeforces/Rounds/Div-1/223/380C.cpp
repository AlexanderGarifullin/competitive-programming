...
struct SegTree {
        ...
        void build(int v, int l, int r) {
            ...
            int mid = (l+r)/2;
            int vl = 2*v; int vr = 2*v+1;
            build(vl, l,mid);
            build(vr,mid,r);
            int add = min(open[vl], close[vr]);
            good[v] = good[vl] + good[vr] + add;
            open[v] = open[vl] + open[vr] - add;
            close[v] = close[vl] + close[vr] - add;
        }

        void query(int v, int L, int R, int l, int r) {
            if (R <= l || r <= L) return;
            if (l <= L && R <= r) {
                int add = min(cntOpen, close[v]);
                ans += good[v] + add;
                cntOpen += open[v] - add;
                cntClose += close[v] - add;
                return;
            }
...
void solve() {
    ...
    while(m--) {
        int l,r; cin >> l >> r;
        l--; r--;
        st.ans = 0; st.cntOpen = 0; st.cntClose = 0;
        st.query(1, 0, n, l, r+1);
        cout << 2*st.ans << en;
    }
}
