...
//    freopen("bees.out", "wt", stdout);
    ...
    int t_; cin >> t_; while (t_--){
        ...
        function<void(int,int)> dfs =[&](int cur, int pred) {
            for (int next : g[cur]) {
                if (next == pred) continue;
                dfs(next, cur);
                // cur is l
                dp0[cur] += max(abs(l[cur] - l[next]) + dp0[next],
                                abs(l[cur] - r[next]) + dp1[next]);
                // cur is r
                dp1[cur] += max(abs(r[cur] - l[next]) + dp0[next],
                                abs(r[cur] - r[next]) + dp1[next]);
            }
        };
        dfs(1,1);
        cout << max(dp0[1], dp1[1])<<en;
    }
}
