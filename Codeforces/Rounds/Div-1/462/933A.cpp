...
void svo(){
    int n; cin >> n;
    vi v(n); fin(v);
    int best= max(count(all(v), 1), count(all(v), 2));

    vvi dpPref(3, vi(n));

    for (int i = 0; i < n; ++i) {
        if (i>0) {
            dpPref[1][i] = dpPref[1][i-1];
            dpPref[2][i] = dpPref[2][i-1];
            if (v[i] == 2) {
                dpPref[2][i] = max(dpPref[1][i-1], dpPref[2][i-1]);
            }
        }
        dpPref[v[i]][i]++;
    }
    ...
    for (int i = n-1; i >=0 ; --i) {
        if (i < n-1) {
            dpSuf[2][i] = dpSuf[2][i+1];
            dpSuf[1][i] = dpSuf[1][i+1];
            if (v[i] == 1) {
                dpSuf[1][i] = max(dpSuf[1][i+1], dpSuf[2][i+1]);
            }
        }
        dpSuf[v[i]][i]++;
    }

    best = max(best, dpPref[2][n-1]);
    vector<vector<vector<int>>> dp(n+1,vector<vector<int>>(n, vector<int>(3)));

    for (int len = 1; len <= n; ++len) {
        for (int start = 0, end = start + len - 1; end < n; ++start, ++end) {
            if (len == 1) {
                dp[len][start][v[start]]++;
            } else {
                int lastOne = dp[len-1][start][1];
                int lastTwo = dp[len-1][start][2];
                dp[len][start][1] = lastOne;
                dp[len][start][2] = lastTwo;
                if (v[end] == 1) {
                    dp[len][start][1] = max(lastOne, lastTwo);
                }
                dp[len][start][v[end]]++;
            }
            int cur = 0;
            if (start>0) cur += dpPref[1][start-1];
            if (end < n-1) cur += dpSuf[2][end+1];
            cur += max(dp[len][start][1], dp[len][start][2]);
            best=max(best, cur);
            cur = 0;
            if (start>0) cur += dpPref[1][start-1];
            if (end < n-1) cur += dpSuf[1][end+1];
            cur += dp[len][start][1];
            best=max(best, cur);
        }
    }
