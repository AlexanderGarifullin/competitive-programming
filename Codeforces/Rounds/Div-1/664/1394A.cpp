...
ll solve(int n, int d, int m, vi a) {
    ll ans = 0;
    vi good,bad;
    for (int i = 0; i < n; ++i) {
    ...
    sort(all(good));
    sort(rall(bad));
    ll sumGood = accumulate(all(good), 0ll);
    ll ans1 = sumGood;
    if (isz(bad)) {
        int can = (isz(bad) + d) / (d+1);
        for (int i = 0; i < can; ++i) {
            ans1 += bad[i];
        }
        ll badSum = 0;
        vl prefGood(isz(good) + 1);
        for (int i = 1; i <= isz(good); ++i) {
            prefGood[i] = good[i-1] + prefGood[i-1];
        }
        for (int i = 0; i < isz(bad); ++i) {
            badSum += bad[i];
            int ostBad = isz(bad) - (i+1);
            ll want = 1ll * i * d;
            want -= ostBad;
            if (want > isz(good)) break;
            ll cur = badSum + prefGood.back() - (want>=0 ? prefGood[want] : 0);
            ans = max(cur, ans);
        }
    }
    return max(ans1, ans);
