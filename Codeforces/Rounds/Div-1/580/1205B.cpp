...
void CBO() {
    ...
    for (int i = 0; i < maxbit; ++i) {
        if (v[i].empty() || isz(v[i]) == 1) continue;
        for (int val : v[i]) {
            if (!old_id.contains(val)) {
                old_id[val] = id++;
            }
        }
        assert(isz(v[i]) == 2);
        int id1 = old_id[v[i][0]];
        int id2 = old_id[v[i][1]];
    ...
    for (auto [ban1, ban2] : e) {
        vi len(n+1, 1e9);
        len[ban1] = 0;
        queue<pii> q;
        q.push({ban1, ban1});
        while(!q.empty()) {
            auto [cur, pred] = q.front();
            q.pop();
            for (int next : g[cur]) {
                if (next == pred) continue;
                if (cur == ban1 && next == ban2 ||
                    cur == ban2 && next == ban1)
                    continue;
