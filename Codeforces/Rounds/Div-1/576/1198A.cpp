...
//    freopen("output.txt", "wt", stdout);
    ...
    int curK = isz(val_cnt);
    int x = 8 * i / n;
    int curval = __lg(curK);
    if ((1ll<<curval) < curK) curval++;
    if (curval <= x) {
        cout << 0;
        return 0;
    }
    int minel = (1<<x);
    // мы хотим сделать так, чтобы кол-во различных элементов было <= minel
    vi v;
    int res = n;
    for (auto [k,cnt] : val_cnt) {
        v.pb(cnt);
    }
    int nn = isz(v);
    vi pref(1+nn);
    ...
    for (int j = 0; j <= nn; ++j) {
        int need = nn - j - minel;
        auto it = cnt_sum.lower_bound(need);
        int del = pref[j] + it->second;
        res=min(res,del);
    }
    cout<<res<<en;
}
