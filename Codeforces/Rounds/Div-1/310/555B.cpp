...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        ll l,r; cin >> l >> r;
        if (i) vec.pb(make_pair(make_pair(r - pl, l - pr), i - 1));
        pl = l; pr = r;
    }
    set<pll> st;
    for (int i = 0; i < m; ++i) {
        ll x; cin >> x;
        st.ins(make_pair(x, i));
    }
    sort(all(vec));
    vi res(n);
    for (int i = 0; i < n-1; ++i) {
        auto it = st.lower_bound(make_pair(vec[i].first.second, -1));
        if (it == st.end() || it->first > vec[i].first.first) {
            cout << "No\n";
            return;
        }
        res[vec[i].second] = it->second;
        st.erase(it);
    }
    cout << "Yes\n";
    for (int i = 0; i < n-1; ++i) {
