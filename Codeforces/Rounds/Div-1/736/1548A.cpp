...
i32 main() {
    ...
    for (int i = 0; i < m; ++i) {
        ...
        edges[v]++;
        if (u < v) weaker[v]++;
        else if (v < u) weaker[u]++;
    }

    set<int> strongest;
    for (int i = 1; i <= n; ++i) {
        if (weaker[i] == edges[i]) strongest.insert(i);
    }
    ...
    int q; cin >> q; while(q--) {
        ...
        if (t == 1) {
            ...
            if (u > v) weaker[u]++;
            if (v > u) weaker[v]++;
            if (weaker[u] == edges[u]) strongest.insert(u);
            else strongest.erase(u);

            if (weaker[v] == edges[v]) strongest.insert(v);
            else strongest.erase(v);
        } else if (t == 2) {
            int u, v; cin >> u >> v;
            edges[u]--; edges[v]--;
            if (u > v) weaker[u]--;
