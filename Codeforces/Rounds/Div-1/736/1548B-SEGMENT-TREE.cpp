...
void build_tree(int v, int left, int right) {
    if (left+1==right) {
        ...
        segTree[v] = __gcd(segTree[v_left], segTree[v_right]);
    }
}

void build(int n){
    treeSize = n;
    segTree = vl(treeSize<<2);
    build_tree(1, 0, treeSize);
}
...
void get_tree(int v, int left, int right) {
    if (start_ <= left && right <= end_) {
        curValue = __gcd(curValue, segTree[v]);
    } else {
        int mid = (left+right)/2;
        int v_left=2*v,v_right =v_left+1;
        if (start_ < mid) get_tree(v_left, left, mid);
        if (mid < end_) get_tree(v_right, mid, right);
    }
}
