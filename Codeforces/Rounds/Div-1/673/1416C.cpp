...
struct Trie {
        ...
        void add(int val, int pos) {
            ...
            for (int i = MAXBIT; i >= 0; --i) {
                int bit = ((val >> i) & 1);
                if (tree[bit][v] == 0) tree[bit][v] = id++;
                v = tree[bit][v];
                g[v].push_back(pos);
            }
        }

        void calc(int v, int bit = 29) {
            int l = tree[0][v];
            int r = tree[1][v];
            if (l) calc(l, bit-1);
            if (r) calc(r, bit-1);
            ...
            int p = 0;
            for (auto x : g[l]) {
                while(p < isz(g[r]) && g[r][p] < x) {
                    p++;
                }
                res += p;
            }
            dp[0][bit] += res;
            dp[1][bit] += isz(g[l]) * 1ll * isz(g[r]) - res;
        }
    };
...
void solve() {
    ...
    ll cnt = 0;
    ll x = 0;
    for (int i = 0; i <= trie.MAXBIT; ++i) {
        cnt += min(trie.dp[0][i], trie.dp[1][i]);
        if (trie.dp[1][i] < trie.dp[0][i]) {
            x += (1<<i);
        }
    }
    cout << cnt << ' ' << x << en;
