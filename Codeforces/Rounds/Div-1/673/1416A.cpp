...
i32 main() {
    ...
    int t; cin >> t; while(t--) {
        ...
        vi a(n); fin(a);
        vi ans(n+1,1e9);
        map<int,int> last_indexes;
        map<int,int> min_len;
        for (int i = 0; i < n; ++i) {
            int len = i+1;
            if (last_indexes.find(a[i]) != last_indexes.end()) {
                len = i - last_indexes[a[i]];
            }
            last_indexes[a[i]] = i;
            min_len[a[i]] = max(min_len[a[i]], len);
        }
        for (auto [v, ind] : last_indexes) {
            int len = n - ind;
            min_len[v] = max(min_len[v], len);
        }
        for (auto [value, len] : min_len) {
            ans[len] = min(ans[len], value);
        }
        for (int i = 1; i <= n; ++i) {
