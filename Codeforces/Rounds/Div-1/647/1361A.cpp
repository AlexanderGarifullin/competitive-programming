...
void add (int curV) {
    ...
    if (st.find(colors[curV]) != st.end()) {
        ok = false;
        return;
    }
    int need = 1;
    while(st.find(need) != st.end()) need++;
    ok = need == colors[curV];
}
...
//    freopen("output.txt", "wt", stdout);
    ...
    colors.resize(n+1); use.resize(n+1);
    for (int i = 1; i <= n; ++i) {
        cin >> colors[i];
        levelVertex[colors[i]].pb(i);
    }
    for (auto [level, v]:levelVertex) {
        if (!ok) break;
        for (int curV : v) {
            add(curV);
            if (!ok) break;
