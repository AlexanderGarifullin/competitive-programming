...
i32 main() {
    ...
    //  freopen("output.txt", "wt", stdout);
     ...
     int t; cin >> t; while(t--){
        ...
        sort(all(x),[](pii p1, pii p2){ return abs(p1.second) < abs(p2.second);});
        sort(all(y),[](pii p1, pii p2){ return abs(p1.first) < abs(p2.first);});
        ld answ = 0;
        for (int i = 0; i < n; ++i) {
            answ += sqrtl((ld)(x[i].first - y[i].first) * (x[i].first - y[i].first) + (ld)(x[i].second - y[i].second) * (x[i].second - y[i].second));
        }
