...
void CBO() {
    ...
    vi p(n); fin(p);
    int ans=0;
    vi lenLeft(n), lenRight(n);
    vi lenLeftUp(n), lenRightUp(n);
    vi maxLenLeft(n), maxLenRight(n);
    for (int i = 0; i < n; ++i) {
        lenLeft[i] = 1;
        lenLeftUp[i] = 1;
        if (i>0 && p[i] > p[i-1]) lenLeft[i] += lenLeft[i-1];
        if (i>0 && p[i] < p[i-1]) lenLeftUp[i] += lenLeftUp[i-1];
        maxLenLeft[i] = max(lenLeft[i], lenLeftUp[i]);
        if (i>0) maxLenLeft[i] = max(maxLenLeft[i],
                                     maxLenLeft[i-1]);
    }
    for (int i = n-1; i >=0 ; --i) {
        lenRight[i] = lenRightUp[i] = 1;
        if (i<n-1 && p[i] > p[i+1]) lenRight[i] += lenRight[i+1];
        if (i<n-1 && p[i] < p[i+1]) lenRightUp[i] += lenRightUp[i+1];
        maxLenRight[i] = max(lenRight[i], lenRightUp[i]);
        if (i<n-1) {
            maxLenRight[i] = max(maxLenRight[i], maxLenRight[i+1]);
        }
    }

    for (int i = 1; i < n-1; ++i) {
        if (p[i] > p[i-1] && p[i] > p[i+1]) {
            int canLen = lenLeft[i];
            int canRight = lenRight[i];
            int canTwo = 1;
            int j1 = i - canLen + 1;
            int j2 = i + canRight - 1;
            if (j1 >= 0) canTwo = max(canTwo, maxLenLeft[j1]);
            if (j2 < n) canTwo = max(canTwo, maxLenRight[j2]);
            if (canTwo >= max(canLen, canRight)) continue;
            if (canLen == canRight && canLen % 2) {
                ans++;
            }
        }
    }
