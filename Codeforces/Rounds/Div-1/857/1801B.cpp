...
//    freopen("output.txt", "wt", stdout);
    ...
    int t_; cin >> t_; while(t_--) {
        ...
        int mmax = -1;
        for (int i = 1; i < n; ++i) {
            best = min(best, abs(v[i].first - v[0].second));
        }
        for (int i = 0; i < n; ++i) {
            st.ins(v[i].first);
        }
        ...
        for (int i = 0; i < n; ++i) {
            ...
            best = min(best, abs(bb - mmax));
            
            auto it = st.lower_bound(bb);
            if (it != st.end() && *it >= mmax) {
                best = min(best, abs(*it - bb));
            }
            if (it != st.begin() && *prev(it) >= mmax) {
                best = min(best, abs(*prev(it) - bb));
            }
