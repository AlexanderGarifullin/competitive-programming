...
void CBO(){
    int n; cin >> n;
    vi a(n); fin(a);
    int cntPos = count(all(a), 1);
    if (cntPos == 1) {
        cmo;
        return;
    }
    function<ll(int)> calc = [&](int k ) {
        vi temp;
        ll val=0;
        for (int i = 0; i < n; ++i) {
            if (a[i] == 1) {
    ...
    ll res = 1e18;
    for (int i = 1; i * i <= cntPos; ++i) {
        if (cntPos % i == 0) {
            if (i != 1) res = min(res, calc(i));
            if (i != cntPos / i) res = min(res, calc(cntPos / i));
        }
    }
    cout << res<<en;
}
