...
//    freopen("output.txt", "wt", stdout);
    ...
    int t_; cin >> t_; while(t_--) {
        ...
        vi a(n1);fin(a); sort(all(a));
        reuniq(a);
        vi b(n2); fin(b); sort(all(b));
        reuniq(b);
        vi c(n3); fin(c); sort(all(c));
        reuniq(c);
        n1=isz(a); n2 = isz(b); n3 = isz(c);
        ll res = 2e18;
        function<void(vi,vi,vi)> brute = [&](vi aa, vi bb, vi cc){
            for (int i = 0; i < isz(aa); ++i) {
                int it1 = lower_bound(all(bb), aa[i]) - bb.begin();
                int it2 = lower_bound(all(cc), aa[i]) - cc.begin();
                int it1pred = it1;
                int it2pred = it2;
                it1pred--; it2pred--;
                vi v1 =  {it1pred, it1};
                ...
                for (int j : v1) {
                    for (int k : v2) {
                        if (0 <= j && j < isz(bb) && 0 <= k && k < isz(cc)) {
                            ll x = aa[i];
                            ll y = bb[j];
                            ll z = cc[k];
                            ll cur = (x-y) * (x-y)+ (y-z) * (y-z)+ (x-z) * (x-z);
                            res=min(res,cur);
                        }
                    }
                }
