...
//    freopen("output.txt", "wt", stdout);
    ...
    int t_; cin >> t_; while(t_--) {
        ...
        vi path = {a};
        vi ansPath;
        function<void(int,int,int)> dfs2 = [&](int cur, int end, int pred) {
            if (cur == end) {
                ansPath = path;
                return;
            }
        ...
        int maxH = 0;
        function<void(int,int, int)> dfs = [&](int cur, int pred, int curH) {
            maxH=max(maxH,curH);
            for (int next:g[cur]) {
                if (next==pred) continue;
                dfs(next, cur, curH+1);
        ...
        ll mid = ansPath[(isz(ansPath) - 1)/2];
        dfs(mid, -1, 0);
        ll r = 2ll * (n-1) - maxH + isz(ansPath) / 2;
        cout << r << en;
    }
}
