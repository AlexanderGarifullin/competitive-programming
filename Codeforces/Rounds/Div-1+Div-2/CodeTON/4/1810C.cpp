...
void voidsolve(){
    ll n, numberDelete, numberAdd;
    cin >> n >> numberDelete >> numberAdd;
    map<int, int> map;
    vi b(n);
    for (int i = 0; i < n; ++i) {
        cin >> b[i];
    ...
    ll mine = 0;
    for (auto &[x, cnt] : map) {
        mine += (cnt - 1) * numberDelete;
    }
    n = isz(b);
    ll addcost = 0;
    if (b[0] !=  1) addcost = b[0] - 1;
    addcost *= numberAdd;
    ll deletecost = 0;
    deletecost = (n-1) * numberDelete;
    answ = addcost + deletecost;
    for (int i = 1; i < n; ++i) {
        deletecost -= numberDelete;
        if (b[i] != b[i-1] - 1) {
            addcost += (b[i] - b[i-1] - 1 ) * numberAdd;
        }
        answ = min(answ, deletecost + addcost);
    }

    cout << mine + min(answ, n * numberDelete + numberAdd) << en;
}
