...
i32 main() {
   ...
   int t; cin >> t; while(t--) {
        ...
        for (int i = 0; i < n; ++i) {
            if (a[i] == b[0]) {
                int maximumleftmove = min(isz(b) -1, n-1-i);
                for (int moveleft = 0; moveleft <= maximumleftmove; ++moveleft) {
                    bool ok = true;
                    int  j = 1;
                    for (int curi = i + 1; curi <= i + moveleft && j < isz(b); ++curi) {
                        if (a[curi] != b[j]) {
                            ok = false;
                            break;
                        }
                    ...
                    int moveright = isz(b) - 1 - moveleft;
                    int leftindex = i + moveleft - moveright;
                    if (leftindex < 0) continue;
                    for (int curi = i + moveleft-1; curi >= leftindex && j < isz(b); --curi) {
                        if (a[curi] != b[j]) {
                            ok = false;
                            break;
                        }
