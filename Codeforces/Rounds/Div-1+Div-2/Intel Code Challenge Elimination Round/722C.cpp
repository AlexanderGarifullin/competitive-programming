...
//    freopen("output.txt", "wt", stdout);
    ...
    for (int i = 0; i < n; ++i) {
        auto it = start_end.lower_bound(p[i]-1);
        if (it == start_end.end() || it != start_end.end() && it->first > p[i]-1) it--;
        int x1 = it->first;
        int y1 = p[i]-1-1;
        int x2 = p[i];
        int y2 = it->second;
        sums.erase(sums.find(-(pref[y2] - (x1 != 0 ? pref[x1-1] : 0))));
        start_end.erase(it);
        if (y1 >= x1) {
            start_end[x1]=y1;
            sums.insert(-(pref[y1] - (x1 != 0 ? pref[x1-1] : 0)));
        }
        if (y2 >= x2) {
            start_end[x2]=y2;
            sums.insert(-(pref[y2] - (x2 != 0 ? pref[x2-1] : 0)));
        }
        if (sums.empty()) cout << 0 <<en;
        else cout << -(*sums.begin())<<en;
    }
}
