...
//    freopen("output.txt", "wt", stdout);
    ...
    for (int i = 0; i < n; ++i) {
        ...
        else if (level[a[i]] == id + 1) {
            id++;
            if (!cur.empty()) v.pb(cur);
            cur = {a[i]};
        } else {
            ok=0;break;
        }
    ...
    if (ok && !cur.empty()) {
        v.pb(cur);
        for (int i = 1; i < isz(v) && ok; ++i) {
            int pred = 0;
            for (int j = 0; j < isz(v[i]); ++j) {
                bool add = 0;
                while(!add && pred<isz(v[i-1])) {
                    if (g[v[i - 1][pred]].find(v[i][j]) != g[v[i - 1][pred]].end()) {
                        g[v[i - 1][pred]].erase(v[i][j]);
                        g[v[i][j]].erase(v[i - 1][pred]);
                        if (g[v[i - 1][pred]].empty()) {
                            pred++;
                        }
                        add = 1;
                    } else {
