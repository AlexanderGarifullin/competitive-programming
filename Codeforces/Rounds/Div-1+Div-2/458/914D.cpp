...
void query(int v, int lv, int rv, int l, int r, int g) {
    if (r <= lv || rv <= l) return;
    if (l <= lv && rv <= r) {
        if (tree[v] % g) {
            badNodes.emplace_back(lv, rv);
        }
        return;
    }
    int m = (lv + rv) / 2;
    query(2*v, lv, m, l, r, g);
    query(2*v+1, m, rv, l, r, g);
}
...
bool solve(int l, int r, int x) {
    ...
    query(1, 0, n, l, r, x);
    while(true) {
        if (badNodes.empty()) return true;
        if (isz(badNodes) > 1) return false;
        auto [badNodeL, badNodeR] = badNodes[0];
        l = badNodeL; r = badNodeR;
