...
void svo(){
    ...
    vvi dp(n+1, vi(n+1,-1));
    function<int(int,int)> solve = [&](int lastCake, int cntAlice) {
        if (lastCake < 0 || cntAlice < 0) {
            return -2;
        }

        if (lastCake == 0) {
            if (cntAlice == 0) {
                return dp[lastCake][cntAlice] = 0;
            } else {
                return dp[lastCake][cntAlice] = -2;
            }
        }

        if (dp[lastCake][cntAlice] != -1) {
            return dp[lastCake][cntAlice];
        }

        if (cnt[lastCake]) {
            dp[lastCake][cntAlice] = solve(lastCake-1, cntAlice-1);
        } else {
            dp[lastCake][cntAlice] = solve(lastCake-1, cntAlice);
        }

        if (solve(lastCake-1, cntAlice) != -2 && solve(lastCake-1, cntAlice) + cnt[lastCake] <= cntAlice) {
            if (dp[lastCake][cntAlice] == -2 || solve(lastCake-1, cntAlice) + cnt[lastCake] <dp[lastCake][cntAlice]) {
                dp[lastCake][cntAlice] = cnt[lastCake] + solve(lastCake-1, cntAlice);
            }
        }
