    ...
    int t_; cin >> t_; while(t_--) {
        ...
        for (int i = 0; i < n; ++i) {
            if (i == 0) {
                ...
                while(1) {
                    if (level[st.top()] == deleteLevel) st.pop();
                    else if (a[i] == a[st.top()]+1) {
                        level[i] = level[st.top()];
                        str s = res[st.top()];
                        ...
                        if (a[i] == 1) {
                            level[i] = level[st.top()] + 1;
                            res[i] = res[st.top()] + ".1";
                            st.push(i);
                            deleteLevel = -1;
                            break;
                        } else {
                            deleteLevel = level[st.top()];
                            st.pop();
                        }
                    }
                }
