...
//    freopen("output.txt", "wt", stdout);
    ...
    function<int(int,int)> getOr = [](int i, int j){
        cout << "or " << i << ' ' << j << endl;
        int x; cin >> x;
        return x;
    };

    function<int(int,int)> getAnd = [](int i, int j){
        cout << "and " << i << ' ' << j << endl;
        int x; cin >> x;
        return x;
    };
    ...
    for (int i = 0; i + 2< n; i+=3) {
        ...
        and23 = getAnd(i2,i3);
        and13 = getAnd(i1,i3);
        a[i2-1] = (or12 + and12 + or23 + and23 - or13 - and13) / 2;
        a[i1-1] = or12 + and12 - a[i2-1];
        a[i3-1] = or23 + and23 - a[i2-1];
    }
    if (n%3 == 1) {
        int or1n = getOr(1,n);
        int and1n = getAnd(1,n);
        a[n-1] = or1n + and1n - a[0];
    } else if (n % 3 == 2){
        int or1n = getOr(1,n);
        int and1n = getAnd(1,n);
        a[n-1] = or1n + and1n - a[0];

        or1n = getOr(1,n-1);and1n = getAnd(1,n-1);
        a[n-2] = or1n + and1n - a[0];
    }
