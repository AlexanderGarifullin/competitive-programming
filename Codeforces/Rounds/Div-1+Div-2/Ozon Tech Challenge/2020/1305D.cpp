...
bool dfs2(int v, int p, int i, int j) {
    for (int next : g[v]) {
        if (next == p) continue;
        if (next == i || next == j) return true;
        if (dfs2(next, v, i, j)) return true;
    }
    return v == i || v == j;
}

void dfs3(int v, int p, set<int> & all) {
    for (int next : g[v]) {
        if (next == p) continue;
        dfs3(next, v, all);
    }
...
void solve() {
    ...
    while(isz(have) > 1) {
        ...
        d1[*have.begin()] = 0; dfs(*have.begin(), -1, d1);
        int j = max_element(all(d1)) - d1.begin();
        d2[j]=0; dfs(j, -1, d2);
        int i = max_element(all(d2)) - d2.begin();
